"""
H-1176 Money Flow Index XS Paper Trade Runner

Long high MFI assets (strong inferred order flow).
Backtest: IS Sharpe 1.762, WF 4/4 PERFECT, SH 1.79/1.75, p=0.015, H-012 corr 0.085.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Money Flow Index: RSI on typical_price × volume, 14d."""
    if highs_df is None or lows_df is None:
        return closes_df * 0
    tp = (highs_df + lows_df + closes_df) / 3
    mf = tp * volumes_df
    mf_diff = mf.diff()
...
def run():
    print("=== H-1176 Money Flow Index XS Paper Trade Runner ===")
    ...
    if len(closes) > 0 and str(closes.index[-1].date()) == today_utc:
        ...
        for df_name in [volumes, opens, highs, lows]:
            pass
