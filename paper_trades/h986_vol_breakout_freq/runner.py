"""
H-986 Vol Breakout Frequency XS Paper Trade Runner

Count of days where absolute return exceeds 2-sigma in 60-day lookback.
More breakouts = more dynamic/active asset. Long active, short dormant.
Backtest: IS Sharpe 2.874, WF 3/4, SH p=0.0001, H-012 corr -0.04.
Best params: P=60, T=2.0, R=7, N=4. SESSION BEST.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        ...
        if sigma == 0:
            continue
        count = np.sum(np.abs(r - mu) > CONFIG["threshold_sigma"] * sigma)
        signal[col] = count
...
def run():
    print("=== H-986 Vol Breakout Frequency XS Paper Trade Runner ===")
