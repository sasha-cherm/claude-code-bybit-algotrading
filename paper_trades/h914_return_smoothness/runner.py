"""
H-914 Return Smoothness XS Paper Trade Runner

R-squared of cumulative returns vs linear trend, signed by slope.
High R2 = smooth trend. Low = choppy.

Backtest: IS Sharpe 1.471, WF 4/5, SH p=0.040, H-012 corr -0.019.
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        ...
        if np.std(cum_r) == 0:
            continue
        slope, intercept, r_val, p_val, std_err = sp_stats.linregress(x, cum_r)
        r2 = r_val ** 2
        signal[col] = r2 * np.sign(slope)
    sig_row = signal
...
def run():
    print("=== H-914 Return Smoothness XS Paper Trade Runner ===")
