"""
H-1257 True Range Trend XS Paper Trade Runner

ATR(10)/ATR(30). Expanding vs contracting true range.
Backtest: IS Sharpe 1.096, WF 4/4 PERFECT, SH 1.01/1.23, p=0.129, H-012 corr -0.002.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """True Range Trend XS."""
    tr1 = highs_df - lows_df
    tr2 = (highs_df - closes_df.shift(1)).abs()
    tr3 = (lows_df - closes_df.shift(1)).abs()
    true_range = pd.DataFrame(np.maximum(tr1.values, np.maximum(tr2.values, tr3.values)),
                              index=closes_df.index, columns=closes_df.columns)
    atr_10 = true_range.rolling(10).mean()
    atr_30 = true_range.rolling(30).mean()
    return atr_10 / (atr_30 + 1e-10)
...
def run():
    print("=== H-1257 True Range Trend XS Paper Trade Runner ===")
