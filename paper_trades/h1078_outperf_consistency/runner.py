"""
H-1078 Outperformance Consistency XS Paper Trade Runner

Assets consistently above XS median → long. Consistently below → short.
Backtest: IS Sharpe 1.239, WF 3/4, SH p=0.084, H-012 corr -0.014.
...
def compute_signal(closes_df):
    """Outperformance consistency: fraction of trailing 20d where asset beat XS median."""
    returns = closes_df.pct_change()
    xs_median = returns.median(axis=1)
    outperf = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
...
def run():
    print("=== H-1078 Outperformance Consistency XS Paper Trade Runner ===")
