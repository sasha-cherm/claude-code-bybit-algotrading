"""
H-1091 Overnight Return Share XS Paper Trade Runner

Assets earning more overnight returns (close-to-open) → long.
Captures institutional/smart money accumulation patterns.
Backtest: IS Sharpe 1.905, IS 100% (8/8), WF 4/4 PERFECT, SH p=0.008, H-012 corr -0.016.
Params: R=7, N=4, dir=high_long.
"""
...
def compute_signal(closes_df, opens_df):
    """Overnight return share: rolling 20d sum of overnight returns / abs sum."""
    overnight_ret = opens_df / closes_df.shift(1) - 1
    total_abs = overnight_ret.abs().rolling(20).sum()
    overnight_sum = overnight_ret.rolling(20).sum()
    return overnight_sum / total_abs.clip(lower=1e-10)
...
def run():
    print("=== H-1091 Overnight Return Share XS Paper Trade Runner ===")
    ...
    if latest_date == state.get("last_daily_date"):
        ...
        signal_df = compute_signal(closes, opens)
