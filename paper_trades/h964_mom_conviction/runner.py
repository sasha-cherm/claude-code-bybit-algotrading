"""
H-964 Momentum Conviction XS Paper Trade Runner

Momentum penalized by path roughness (max DD / abs return). Smooth momentum paths preferred.
Backtest: IS Sharpe 2.074, WF 4/4, SH p=0.005, H-012 corr -0.006.
Best params: M=60, D=60, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    cum_ret = ret.rolling(lb).sum()
    signal = pd.Series(np.nan, index=closes.columns)
    for col in closes.columns:
        r = ret[col].iloc[-lb:].values
        if not np.all(np.isfinite(r)):
            continue
        cr = cum_ret[col].iloc[-1]
        ...
        roughness = min(roughness, 2.0)
        conviction = cr * max(0, 1 - roughness)
        signal[col] = conviction
...
def run():
    print("=== H-964 Momentum Conviction XS Paper Trade Runner ===")
