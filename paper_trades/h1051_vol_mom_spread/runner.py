"""
H-1051 Volume Momentum Spread XS Paper Trade Runner

Difference between short and mid-term volume ratios vs long-term. Accelerating volume interest.
Backtest: IS Sharpe 1.581, WF 3/4, SH p=0.033, H-012 corr 0.070.
Best params: P=65, R=5, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    vm_short = volumes.rolling(5).mean() / volumes.rolling(60).mean()
    vm_mid = volumes.rolling(20).mean() / volumes.rolling(60).mean()
    vm_short = vm_short.replace([np.inf, -np.inf], np.nan)
    vm_mid = vm_mid.replace([np.inf, -np.inf], np.nan)
    spread = vm_short - vm_mid
    signal = spread.iloc[-1].dropna()
    sig_row = signal
...
def run():
    print("=== H-1051 Volume Momentum Spread XS Paper Trade Runner ===")
