"""
H-929 Volume-Weighted Momentum XS Paper Trade Runner

Sum of (return_i * relative_volume_i) over lookback.
Weighs returns by relative volume — high-volume moves count more.

Backtest: IS Sharpe 1.753, WF 4/5, SH p=0.015, H-012 corr 0.006.
...
def compute_rankings(daily_dict):
    ...
    vw_ret = returns * rel_vol
    sig_row = vw_ret.rolling(lb).sum().iloc[-1]
...
def run():
    print("=== H-929 VW Momentum XS Paper Trade Runner ===")
