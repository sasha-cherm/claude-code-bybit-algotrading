"""
H-437 Paper Trade Runner: HL Spread Proxy Factor

Market-neutral strategy: rank 14 crypto assets by rolling average of
hourly (high-low)/close as a bid-ask spread proxy (computed from previous
day's hourly bars). LOW spread → long (tight spread = liquid = quality).

Backtest: IS 95.8% (low_long direction). Best LB30_R5_N4
Sharpe 0.870, +49.5% ann, -55.4% DD.
WF 5/6 mean 1.049. Split-half H1=0.068, H2=1.461.
Corr H-012 -0.183 — NEGATIVE, excellent diversifier.
"""
...
def compute_hl_spread_daily(df_1h_sym):
    """Compute daily avg (high-low)/close from hourly bars."""
    df = df_1h_sym.copy()
    if df.index.tz:
    ...
    daily_spread = df.groupby("date")["hl_spread"].mean()
    return daily_spread.sort_index()


def compute_rankings(hourly_dict):
    """Rank assets by rolling HL spread. LOW → long (tight spread = quality)."""
    scores = {}
    for sym, df_1h in hourly_dict.items():
        spread_series = compute_hl_spread_daily(df_1h)
        if spread_series is None or len(spread_series) < CONFIG["lookback"]:
            continue
        rolling_mean = spread_series.rolling(CONFIG["lookback"],
...
def run():
    print("=== H-437 HL Spread Proxy Paper Trade Runner ===")
