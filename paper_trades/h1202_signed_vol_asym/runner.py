"""
H-1202 Signed Volume Asymmetry XS XS Paper Trade Runner

Long assets where volume correlates with positive returns (buying pressure), short where volume correlates with negative returns.
Backtest: IS Sharpe 1.631, WF 4/4, SH 2.21/0.85, p=0.024, H-012 corr 0.002.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    """Signed Volume Asymmetry: corr(returns, volume) over 20d window."""
    ...
    for col in closes_df.columns:
        result[col] = returns[col].rolling(20).corr(volumes_df[col])
    return result
...
def run():
    print("=== H-1202 Signed Volume Asymmetry XS XS Paper Trade Runner ===")
