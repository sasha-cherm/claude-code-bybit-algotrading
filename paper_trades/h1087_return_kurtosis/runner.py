"""
H-1087 Return Kurtosis XS Paper Trade Runner

Assets with LOW excess kurtosis (thin tails, stable returns) → long.
High kurtosis (fat tails, jumpy) → short.
Backtest: IS Sharpe 1.198, WF 3/4, SH p=0.097, H-012 corr 0.019.
Params: R=7, N=4, dir=low_long.
"""
...
def compute_signal(closes_df):
    """Rolling 30d excess kurtosis of daily returns."""
    returns = closes_df.pct_change()
    kurtosis = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    for col in closes_df.columns:
        kurtosis[col] = returns[col].rolling(30).apply(
            lambda x: sp_stats.kurtosis(x.dropna()) if len(x.dropna()) > 10 else np.nan, raw=False)
    return kurtosis
...
def run():
    print("=== H-1087 Return Kurtosis XS Paper Trade Runner ===")
