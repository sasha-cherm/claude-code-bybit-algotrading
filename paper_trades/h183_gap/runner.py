"""
H-183 Paper Trade Runner: Gap Factor / Overnight Sentiment (14 Assets)

Market-neutral strategy: rank assets by rolling mean overnight gap
(open_t - close_{t-1}) / close_{t-1}. Contrarian: long assets with
most negative gaps (overnight pessimism = buying opportunity),
short assets with most positive gaps. Rebalance every 5 days.

Backtest: IS 100% positive (30/30), best Sharpe 1.778 (+85.3% ann, -38.0% DD).
WF 5/6 positive, mean OOS 1.771. Split-half H1=1.826, H2=2.040.
Correlation: H-012 0.468, H-076 -0.050, H-160 0.190.
"""
...
CONFIG = {
    "lookback": 10,          # 10-day rolling gap window
    "rebal_freq": 5,         # rebalance every 5 days
    "n_long": 4,             # long bottom 4 (most negative gaps = contrarian buy)
    "n_short": 4,            # short top 4 (most positive gaps)
    "direction": "neg_gap_long",
...
# ── Data loading ─────────────────────────────────────────────────────
def load_daily_ohlc() -> tuple:
    """Load 1d OHLCV, return (opens, closes) DataFrames."""
    opens_dict = {}
    closes_dict = {}
    for sym in ASSETS:
        try:
...
# ── Signal generation ────────────────────────────────────────────────
def compute_rankings(opens: pd.DataFrame, closes: pd.DataFrame,
                     date_idx: int) -> dict:
    """
    Rank assets by rolling mean overnight gap.
    neg_gap_long: long bottom-N (most negative gap), short top-N (most positive).
    """
    ...
    for sym in closes.columns:
        if sym not in opens.columns:
            continue
        o = opens[sym].iloc[:date_idx]
        c = closes[sym].iloc[:date_idx]
        if len(o) < lookback + 1:
            continue
        # Gap = (open_t - close_{t-1}) / close_{t-1}
        gap = (o.iloc[1:].values - c.iloc[:-1].values) / c.iloc[:-1].values
        # Rolling mean of last `lookback` gaps
        if len(gap) < lookback:
            continue
        avg_gap = np.mean(gap[-lookback:])
    ...
    ranked = pd.Series(signals).sort_values(ascending=True)  # most negative first

    if CONFIG["direction"] == "neg_gap_long":
        longs = ranked.index[:n_long]     # most negative gaps (contrarian buy)
        shorts = ranked.index[-n_short:]  # most positive gaps
    else:
        longs = ranked.index[-n_long:]    # most positive gaps
        shorts = ranked.index[:n_short]   # most negative gaps
...
# ── Main runner ──────────────────────────────────────────────────────
def run():
    print("=== H-183 Gap Factor Paper Trade Runner ===")
    ...
    if days_since >= CONFIG["rebal_freq"]:
        ...
        new_weights = compute_rankings(opens, closes, date_idx)
        if not new_weights:
