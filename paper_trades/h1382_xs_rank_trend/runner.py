"""
H-1382 XS Rank Trend XS Paper Trade Runner

Slope of asset's XS-rank-percentile (of 10d cumulative return) over 30d.
High = asset is climbing ranks → long. Low = slipping → short.
Backtest: IS Sharpe 1.332, WF 3/4, SH 1.25/1.47, p=0.065, H-012 corr 0.005.
...
def compute_signal(closes_df):
    """XS Rank Trend: 30d slope of 10d-cum-return rank percentile."""
    returns = closes_df.pct_change()
    rank_pct = returns.rolling(10).sum().rank(axis=1, pct=True)
    return rank_pct.apply(slope_30)
...
def run():
    print("=== H-1382 XS Rank Trend XS Paper Trade Runner ===")
