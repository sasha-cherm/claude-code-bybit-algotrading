"""
H-353 Paper Trade Runner: Volume Persistence Factor

Market-neutral strategy: rank 14 crypto assets by autocorrelation of
hourly volumes within each day. High persistence = sustained institutional
engagement (trending volume). Low persistence = sporadic bursts (retail).
Long high-persistence, short low-persistence.

Backtest: IS 100% high_long (30/30). Best LB5_R3_N4 Sharpe 2.501
(WF 5/6 mean 2.526). Split-half H1=1.592, H2=1.035.
Corr H-012 0.196, H-076 -0.030. Very low correlation with existing strategies.
"""
        ...
        try:
            ...
            for date, hg in df_1h.groupby("date"):
                ...
                if len(vol) >= 10 and np.std(vol) > 0:
                    ...
                    daily_persistence[pd.Timestamp(date)] = float(ac) if np.isfinite(ac) else 0.0
                else:
                    daily_persistence[pd.Timestamp(date)] = 0.0
            features_dict[sym] = pd.Series(daily_persistence, name="vol_persistence")
...
def run():
    print(f"=== H-353 Volume Persistence Paper Trade Runner ===")
