"""
H-864 Conditional Momentum XS Paper Trade Runner

Momentum computed only on high-volume days — filters noise.

Backtest: IS Sharpe 1.608, WF 4/5, SH p=0.025, H-012 corr 0.000.
Best params: W=20, R=5, N=4.
"""
...
def compute_rankings(daily_dict):
    """Compute signal and return target weights."""

    returns = pd.DataFrame({s: d["close"].pct_change() for s, d in daily_dict.items()}).dropna(how="all").ffill().dropna()
    volumes = pd.DataFrame({s: d["volume"] * d["close"] for s, d in daily_dict.items()}).dropna(how="all").ffill().dropna()
    common_idx = returns.index.intersection(volumes.index)
    common_cols = returns.columns.intersection(volumes.columns)
    ...
    for i in range(lb + 5, len(returns)):
        for col in returns.columns:
            v = volumes[col].iloc[i - lb:i]
            r = returns[col].iloc[i - lb:i]
            median_v = v.median()
            hv_rets = r[v > median_v]
            if len(hv_rets) > 5:
                signal.iloc[i, signal.columns.get_loc(col)] = hv_rets.sum()
    sig_row = signal.iloc[-1].dropna()

    n_required = CONFIG["n_long"] + CONFIG["n_short"]
    if len(sig_row) < n_required:
        return {}
...
def run():
    print("=== H-864 Conditional Momentum XS Paper Trade Runner ===")
