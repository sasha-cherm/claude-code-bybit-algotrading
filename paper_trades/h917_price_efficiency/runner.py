"""
H-917 Price Efficiency XS Paper Trade Runner

Net price change / sum(abs(daily changes)).
1.0 = perfectly straight move. Near 0 = choppy.

Backtest: IS Sharpe 1.594, WF 3/5, SH p=0.026, H-012 corr -0.012.
Best params: P=14, R=7, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(returns) < lb + 5:
        return {}
    net = closes.pct_change(lb).iloc[-1]
    abs_sum = returns.abs().rolling(lb).sum().iloc[-1].replace(0, np.nan)
    sig_row = net / abs_sum
...
def run():
    print("=== H-917 Price Efficiency XS Paper Trade Runner ===")
