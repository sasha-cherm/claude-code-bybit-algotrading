"""
H-899 Volume Trend Persistence XS Paper Trade Runner

Proportion of days with above-average volume over lookback.
Sustained high volume = institutional interest / accumulation.

Backtest: IS Sharpe 1.560, WF 5/5 PERFECT, SH p=0.030, H-012 corr -0.048.
Best params: P=20, R=7, N=4.  SESSION BEST.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(volumes) < lb + 5:
        return {}
    sma = volumes.rolling(lb).mean()
    above = (volumes > sma).astype(float)
    persistence = above.rolling(lb).sum() / lb
    sig_row = persistence.iloc[-1].dropna()
...
def run():
    print("=== H-899 Volume Trend Persistence XS Paper Trade Runner ===")
