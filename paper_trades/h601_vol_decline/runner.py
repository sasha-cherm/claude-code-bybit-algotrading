"""
H-601 Paper Trade Runner: Volume Decline Rate Factor

Market-neutral strategy: rank 14 crypto assets by recent dollar volume
trend (10d avg / 20d avg). HIGH ratio = rising volume → long.
Signal uses LAGGED data (up to yesterday) — no look-ahead.

Backtest: IS Sharpe 0.965, Ann +39.3%, DD -32.1%.
WF 4/6 positive (mean 1.482). Split-half 0.731/1.321 PASS.
Param robust 60/60 (100%), best 1.843.
Corr H-012: 0.054 (near-zero — excellent diversifier).
"""
...
def compute_rankings(daily_dict):
    """Rank assets by volume decline rate. HIGH ratio (rising vol) -> long."""
    scores = {}
    lb = CONFIG["lookback"]
    half_lb = lb // 2
    ...
    for sym, daily in daily_dict.items():
        ...
        dv = d["volume"] * d["close"]
        vol_recent = dv.iloc[-half_lb:].mean()
        vol_longer = dv.iloc[-lb:].mean()

        if vol_longer > 0 and np.isfinite(vol_recent) and np.isfinite(vol_longer):
            scores[sym] = vol_recent / vol_longer
...
def run():
    print("=== H-601 Volume Decline Rate Paper Trade Runner ===")
