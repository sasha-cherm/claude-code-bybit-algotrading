"""
H-215 Paper Trade Runner: Dollar Volume Trend Factor (14 Assets)

Market-neutral strategy: rank assets by OLS slope of log(dollar_volume) over
a lookback window. Long assets with steepest increasing dollar volume trend
(growing interest), short assets with steepest decreasing (fading interest).
Rebalance every 3 days.

Backtest: IS 94.4% positive (34/36), mean Sharpe 0.705, best LB15_R3_N4
Sharpe 1.668 (+65.2% ann, -21.8% DD). WF 4/6 positive, mean OOS 0.016.
Split-half H1=2.388, H2=1.565. Corr H-012 0.148.
"""
...
def compute_rankings(closes: pd.DataFrame, volumes: pd.DataFrame,
                     date_idx: int) -> dict:
    """
    Rank assets by OLS slope of log(dollar_volume) over lookback window.
    Steepest positive slope (growing interest) → long.
    Steepest negative slope (fading interest) → short.
    """
    ...
    for sym in closes.columns:
        if sym not in volumes.columns:
            continue
        dv_window = dv[sym].iloc[date_idx - lookback: date_idx].values
        if np.any(np.isnan(dv_window)) or np.any(dv_window <= 0):
            continue
        log_dv = np.log(dv_window)
        y_dm = log_dv - log_dv.mean()
    ...
    if len(slopes) < n_long + n_short:
        return {}

    ranked = pd.Series(slopes).sort_values(ascending=False)
...
def run():
    print("=== H-215 Dollar Volume Trend Paper Trade Runner ===")
