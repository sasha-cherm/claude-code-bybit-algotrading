"""
H-726 Paper Trade Runner: Max Drawdown Factor (Cross-Sectional)

Rank 14 crypto assets by rolling max drawdown over lookback window.
Long most beaten-down (deepest DD) → mean reversion.
Short least beaten-down → overbought.

Backtest: IS Sharpe 0.980, Ann +44.6%, DD -35.5%.
WF 6/6 (perfect). 100% param robust (60/60).
SH 0.570/1.379 (both positive). H-012 corr 0.332.
"""
...
def compute_rankings(daily_dict):
    """Rank assets by rolling max drawdown. Most beaten-down → long."""
    scores = {}
    lb = CONFIG["lookback"]
    for sym, daily in daily_dict.items():
        window = daily["close"].iloc[-(lb + 1):-1]
    ...
    ranked = pd.Series(scores).sort_values()
...
def run():
    print("=== H-726 Max Drawdown Factor Paper Trade Runner ===")
