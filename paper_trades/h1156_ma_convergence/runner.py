"""
H-1156 MA Convergence XS Paper Trade Runner

Long assets where EMA20 > EMA50 (strong trend), short where EMA20 < EMA50.
Backtest: IS Sharpe 1.258, WF 3/4, SH 1.38/1.12, p=0.082, H-012 corr 0.012.
...
def compute_signal(closes_df, volumes_df):
    """MA Convergence: (EMA20 - EMA50) / EMA50."""
    result = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    for col in closes_df.columns:
        ema20 = closes_df[col].ewm(span=20, adjust=False).mean()
        ema50 = closes_df[col].ewm(span=50, adjust=False).mean()
        result[col] = (ema20 - ema50) / ema50.clip(lower=1e-10)
    return result
...
def run():
    print("=== H-1156 MA Convergence XS Paper Trade Runner ===")
