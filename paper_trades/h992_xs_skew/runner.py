"""
H-992 XS Skew Position Paper Trade Runner

Each asset's return percentile within cross-section. Long right-tail (outperformers),
short left-tail. Short-term (10d) XS positioning.
Backtest: IS Sharpe 1.236, WF 4/5, SH p=0.084, H-012 corr 0.021.
Best params: P=10, R=7, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    p = CONFIG["return_period"]
    if len(closes) < p + 5:
        return {}
    returns = closes.pct_change(p)
    latest_ret = returns.iloc[-1].dropna()
    if len(latest_ret) < CONFIG["n_long"] + CONFIG["n_short"]:
        return {}
    # Percentile rank within cross-section
    signal = pd.Series(np.nan, index=latest_ret.index)
    for col in latest_ret.index:
        pctile = (latest_ret < latest_ret[col]).sum() / (len(latest_ret) - 1)
        signal[col] = pctile
...
def run():
    print("=== H-992 XS Skew Position Paper Trade Runner ===")
