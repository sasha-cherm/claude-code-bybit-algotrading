"""
H-1116 Dispersion-Timed Momentum XS Paper Trade Runner

Momentum signal scaled by cross-sectional return dispersion.
High dispersion → amplify momentum signal; low dispersion → dampen.
Backtest: IS Sharpe 1.293, WF 2/3, SH 1.97/0.31, p=0.080, H-012 corr 0.635.
...
def compute_signal(closes_df):
    """Dispersion-timed momentum: z-scored 60d momentum * (1 + dispersion_z)."""
    returns = closes_df.pct_change()
    mom60 = returns.rolling(60).sum()

    mu = mom60.mean(axis=1)
    sigma = mom60.std(axis=1).clip(lower=1e-10)
    z_mom = mom60.sub(mu, axis=0).div(sigma, axis=0)
    ...
    disp_weight = 1 + xs_disp_z.clip(-2, 2)
    result = z_mom.mul(disp_weight, axis=0)
    return result
...
def run():
    print("=== H-1116 Dispersion-Timed Momentum XS Paper Trade Runner ===")
