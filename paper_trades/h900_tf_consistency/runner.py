"""
H-900 Timeframe Consistency XS Paper Trade Runner

Sign agreement of returns across 3d/7d/14d/30d.
+4 = all timeframes up, -4 = all down.

Backtest: IS Sharpe 1.460, WF 3/4, SH p=0.044, H-012 corr -0.004.
Best params: R=7, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    consistency = np.sign(r3) + np.sign(r7) + np.sign(r14) + np.sign(r30)
    sig_row = consistency.iloc[-1].dropna()
...
def run():
    print("=== H-900 Timeframe Consistency XS Paper Trade Runner ===")
