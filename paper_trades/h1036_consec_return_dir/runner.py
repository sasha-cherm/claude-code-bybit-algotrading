"""
H-1036 Consecutive Return Direction XS Paper Trade Runner

Length of current consecutive win/loss streak. Short long streaks, long short streaks (contrarian).
Backtest: IS Sharpe 1.808, WF 4/5, SH p=0.012, H-012 corr -0.036.
Best params: P=20, R=5, N=5, dir=low_long.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        streak = 0
        for j in range(min(20, len(closes) - 1)):
            r = returns[col].iloc[-(j+1)]
...
def run():
    print("=== H-1036 Consecutive Return Direction XS Paper Trade Runner ===")
