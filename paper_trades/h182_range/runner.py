"""
H-182 Paper Trade Runner: High-Low Range Factor (14 Assets)

Market-neutral strategy: rank assets by normalized intraday range
(rolling mean of (high - low) / close). Long narrowest-range assets
(quiet accumulation), short widest-range (panic/volatility).
Rebalance every 5 days.

Backtest: IS 90% positive (27/30), best Sharpe 0.912 (+47.2% ann, -62.1% DD).
WF 5/6 positive, mean OOS 1.506. Split-half H1=0.416, H2=2.397.
Correlation: H-012 0.200, H-076 -0.154, H-160 0.101.
"""
...
CONFIG = {
    "lookback": 30,          # 30-day rolling range window
    "rebal_freq": 5,         # rebalance every 5 days
    "n_long": 3,             # long bottom 3 (narrowest range)
    "n_short": 3,            # short top 3 (widest range)
    "direction": "narrow_long",
...
def load_daily_ohlc() -> tuple:
    """Load 1d OHLCV, return (closes, highs, lows) DataFrames."""
    closes_dict = {}
    highs_dict = {}
    lows_dict = {}
    for sym in ASSETS:
...
def compute_rankings(closes: pd.DataFrame, highs: pd.DataFrame,
    ...
    """
    Rank assets by rolling mean of normalized HL range.
    narrow_long: long bottom-N (narrowest), short top-N (widest).
    """
    ...
    for sym in closes.columns:
        if sym not in highs.columns or sym not in lows.columns:
            continue
        h = highs[sym].iloc[:date_idx]
        l = lows[sym].iloc[:date_idx]
        ...
        avg_range = norm_range.iloc[-lookback:].mean()
        if not np.isnan(avg_range):
            signals[sym] = avg_range
    ...
    ranked = pd.Series(signals).sort_values(ascending=True)  # lowest range first

    if CONFIG["direction"] == "narrow_long":
        longs = ranked.index[:n_long]     # narrowest
        shorts = ranked.index[-n_short:]  # widest
    else:
        longs = ranked.index[-n_long:]    # widest
        shorts = ranked.index[:n_short]   # narrowest
...
# ── Main runner ──────────────────────────────────────────────────────
def run():
    print("=== H-182 High-Low Range Paper Trade Runner ===")
