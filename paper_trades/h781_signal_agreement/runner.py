"""
H-781 Paper Trade Runner: Signal Agreement XS

Long assets where multiple factors agree (momentum, short-term mom, vol mom, low vol).

Backtest: IS Sharpe 1.293, Ann +63.6%, WF 4/5. SH p=0.072.
H-012 corr -0.008. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    ...
    mom60 = closes.pct_change(60).iloc[-2]
    mom20 = closes.pct_change(20).iloc[-2]
    vol_mom = volumes.pct_change(20).iloc[-2]
    ret = closes.pct_change()
    vol = ret.rolling(20).std().iloc[-2]
    ...
    for sym in closes.columns:
        ...
        agreement = 0
        vals = {
            "mom60": mom60.get(sym, np.nan),
            "mom20": mom20.get(sym, np.nan),
            "vol_mom": vol_mom.get(sym, np.nan),
            "vol": vol.get(sym, np.nan),
        }
        if any(not np.isfinite(v) for v in vals.values()):
            continue

        mom60_rank = mom60.rank(pct=True).get(sym, 0.5)
        mom20_rank = mom20.rank(pct=True).get(sym, 0.5)
        vol_mom_rank = vol_mom.rank(pct=True).get(sym, 0.5)
        vol_rank = vol.rank(pct=True).get(sym, 0.5)
...
def run():
    print("=== H-781 Signal Agreement Paper Trade Runner ===")
