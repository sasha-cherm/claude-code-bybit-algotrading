"""
H-012 Paper Trade Runner: Cross-Sectional Momentum (14 Crypto Assets)

Market-neutral strategy: rank 14 assets by 60-day return, long top 4,
short bottom 4. Rebalance every 5 days using lagged (t-1) ranking.

Internal simulation — called each session. Only acts on new daily bars
and when a rebalance is due (every 5 calendar days since first rebalance).

Position sizing: equal weight 0.25 per slot (1.0 long + 1.0 short = 2.0 gross).
Fees: 0.1% taker on turnover at rebalance + 2bps slippage.
"""
...
CONFIG = {
    "lookback": 60,          # 60-day return for ranking
...
def load_state() -> dict:
    ...
    return {
        ...
        "positions": {},       # {symbol: {"weight": 0.25, "entry_price": ..., "size": ...}}
        "equity_history": [],
...
def compute_rankings(closes: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by lookback-day return using data up to date_idx - 1 (lagged).
    ...
    # Use yesterday's (lagged) data for ranking
    lagged_idx = date_idx - 1
    current_prices = closes.iloc[lagged_idx]
    past_prices = closes.iloc[lagged_idx - lookback]

    returns = (current_prices / past_prices - 1).dropna()
    if len(returns) < CONFIG["n_long"] + CONFIG["n_short"]:
        return {}
...
# ── Main runner ──────────────────────────────────────────────────────
def run():
    print("=== H-012 XSMom Paper Trade Runner ===")
    ...
    date_idx = len(closes) - 1  # index of latest bar
    ...
    if days_since >= CONFIG["rebal_freq"]:
        ...
        if not new_weights:
            ...
            # Close positions that are no longer in the basket
            total_fees = 0.0
            trades_this_rebal = 0
            ...
            for sym, pos in old_positions.items():
                if sym in current_prices.index:
                    ...
                    # Only charge fee if position is changing
            ...
            for sym, weight in new_weights.items():
                ...
                # Fee on new position (only if changed)
...
def report():
    ...
    print("=== H-012 XSMom Performance Report ===")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="H-012 XSMom Paper Trade")
