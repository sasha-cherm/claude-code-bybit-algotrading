"""
H-242 Paper Trade Runner: Intraday Momentum Concentration Factor (14 Assets)

Market-neutral strategy: rank assets by intraday return concentration (fraction
of daily absolute return from the single highest-return hour). High concentration
(spike-driven) -> long. Low concentration -> short.

Backtest: IS 100% high_conc_long (24/24). Best LB10_R5_N4 Sharpe 2.014
(+73.2% ann, -28.6% DD). WF 6/6 mean OOS 1.802.
Split-half H1=2.112, H2=2.298. Corr H-012 0.14, H-031 0.24.
"""
...
def load_data():
    """Load hourly data, compute daily concentration, and daily closes."""
    closes_dict = {}
    conc_dict = {}
    for sym in ASSETS:
        try:
            ...
            # Compute hourly returns and daily concentration
            ...
            for date, grp in grouped:
                ...
                if total > 1e-10 and len(grp) >= 12:
                    daily_conc[pd.Timestamp(date)] = grp.max() / total
            conc_dict[sym] = pd.Series(daily_conc).sort_index()
        except Exception as e:
            print(f"  {sym}: failed: {e}")

    closes = pd.DataFrame(closes_dict).dropna(how="all").ffill().dropna()
    # Strip timezone from closes to align with tz-naive concentration index
    if closes.index.tzinfo is not None:
        closes.index = closes.index.tz_localize(None)
    conc = pd.DataFrame(conc_dict).reindex(closes.index).ffill()
    return closes, conc


def compute_rankings(closes: pd.DataFrame, conc: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by rolling avg intraday concentration.
    High concentration (spike-driven) -> long.
    """
    ...
    # High concentration -> long (high_conc_long direction)
...
def run():
    print("=== H-242 Intraday Concentration Paper Trade Runner ===")
    ...
    if days_since >= CONFIG["rebal_freq"]:
        ...
        new_weights = compute_rankings(closes, conc, date_idx)
        if not new_weights:
