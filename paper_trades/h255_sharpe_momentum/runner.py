"""
H-255 Paper Trade Runner: Risk-Adjusted Momentum (Rolling Sharpe) Factor

Market-neutral strategy: rank 14 crypto assets by rolling Sharpe ratio
(return/vol). Long high-Sharpe (quality momentum), short low-Sharpe.

Backtest: IS 93.3% high_sharpe_long (28/30). Best LB14_R7_N3 Sharpe 1.552
(+75.2% ann, -26.3% DD). WF 5/6 mean OOS 0.964.
Split-half H1=1.963, H2=1.447. Corr H-012 0.46.
"""
...
def compute_rankings(closes: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by rolling Sharpe ratio.
    High Sharpe (quality momentum) -> long.
    """
    ...
    # date_idx is in closes space; rets has 1 fewer row
    ri = date_idx - 1
    if ri < lookback or ri >= len(rets):
        return {}

    # Compute rolling Sharpe for each asset at this date
    sharpe_vals = {}
    for col in rets.columns:
        ...
        if sigma > 1e-12:
            sharpe_vals[col] = mu / sigma

    valid = pd.Series(sharpe_vals)
    ...
    # High Sharpe -> long (high_sharpe_long direction)
...
def run():
    print("=== H-255 Risk-Adjusted Momentum Paper Trade Runner ===")
