"""
H-1071 Hourly Range Expansion XS Paper Trade Runner

Assets with higher max hourly range / avg hourly range → long (explosive moves = trending).
Backtest: IS Sharpe 1.365, WF 4/5, SH p=0.056, H-012 corr 0.020.
...
def compute_range_expansion(hourly_dict):
    """Compute max hourly range / avg hourly range for each asset."""
    signals = {}
    for sym, df in hourly_dict.items():
        ...
        for date, group in df.groupby("date"):
            ...
            if len(ranges) < 10 or ranges.mean() <= 0:
                continue
            daily_expansion[pd.Timestamp(date)] = ranges.max() / ranges.mean()
        if daily_expansion:
            signals[sym] = pd.Series(daily_expansion)
    if not signals:
        return pd.DataFrame()
...
def run():
    print("=== H-1071 Range Expansion XS Paper Trade Runner ===")
    ...
    if len(hourly_dict) < 8:
        save_state(state)
        return state
    signal_df = compute_range_expansion(hourly_dict)
