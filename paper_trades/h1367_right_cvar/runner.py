"""
H-1367 Right CVaR Trend XS Paper Trade Runner

Mean of TOP 3 returns in recent 20d minus mean of top 3 in prior 20d.
High = right CVaR expanding (upside tail growing). Long these.
Backtest: IS Sharpe 1.308, WF 4/4 PERFECT, SH 1.27/1.42, p=0.070, H-012 corr -0.021.
...
def cvar_rolling(series, window=20, n_k=3, tail="right"):
...
def compute_signal(closes_df):
    """Right CVaR Trend: mean top3 recent 20d - mean top3 prior 20d."""
    returns = closes_df.pct_change()
    r_recent = returns.apply(lambda col: cvar_rolling(col, 20, 3, "right"))
    r_prior = returns.shift(20).apply(lambda col: cvar_rolling(col, 20, 3, "right"))
...
def run():
    print("=== H-1367 Right CVaR Trend XS Paper Trade Runner ===")
