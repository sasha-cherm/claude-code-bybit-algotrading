"""
H-997 Momentum-Vol Interaction XS Paper Trade Runner

Return / volatility — risk-adjusted momentum (return per unit of risk).
Long efficient movers (high return/vol), short noisy losers (low return/vol).
Backtest: IS Sharpe 1.576, WF 3/5, SH p=0.029, H-012 corr 0.04.
Best params: M=20, V=20, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    vol = returns.rolling(vp).std().iloc[-1]
    vol = vol.replace(0, np.nan)
    signal = (mom / vol).dropna()
    if len(signal) < CONFIG["n_long"] + CONFIG["n_short"]:
        return {}
...
def run():
    print("=== H-997 Momentum-Vol Interaction XS Paper Trade Runner ===")
