"""
H-1223 Lempel-Ziv Complexity XS Paper Trade Runner

Low LZ complexity (more compressible return sign patterns) → outperforms.
Backtest: IS Sharpe 1.093, WF 2/4, SH 1.06/1.17, p=0.130, H-012 corr -0.047.
...
def _lempel_ziv(s):
    n = len(s)
    if n == 0: return 0
    complexity = 1; l = 1; k = 1; k_max = 1
    while l + k <= n:
        if s[l + k - 1] == s[k - 1]:
            k += 1
        else:
            k_max = max(k_max, k); k = 1
            if k_max > l:
                complexity += 1; l += k_max; k_max = 1
            else:
                complexity += 1; l += 1; k_max = 1
    if k > 1: complexity += 1
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Lempel-Ziv Complexity of return sign sequence over 20d."""
...
def run():
    print("=== H-1223 Lempel-Ziv Complexity XS Paper Trade Runner ===")
    ...
    if latest_date == state.get("last_daily_date"):
        ...
        if days_since >= CONFIG["rebal_freq"]:
            ...
            if new_weights:
                ...
                save_log(log)
                print(f"  LONG:  {log[-1]['longs']}"); print(f"  SHORT: {log[-1]['shorts']}")
        else:
