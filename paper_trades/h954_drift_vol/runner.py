"""
H-954 Drift-to-Volatility XS Paper Trade Runner

mean(daily return) / std(daily returns). Directional strength relative to noise.
Backtest: IS Sharpe 1.598, WF 3/5, SH p=0.026, H-012 corr -0.017.
Best params: P=14, R=7, N=3. SESSION BEST.
"""
...
def compute_rankings(daily_dict):
    ...
    # Drift-to-Volatility: mean(return) / std(return)
    mean_ret = ret.rolling(lb).mean().iloc[-1]
    std_ret = ret.rolling(lb).std().iloc[-1].replace(0, np.nan)
...
def run():
    print("=== H-954 Drift-to-Volatility XS Paper Trade Runner ===")
