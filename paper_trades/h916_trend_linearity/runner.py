"""
H-916 Trend Linearity XS Paper Trade Runner

R-squared of close prices vs time, signed by slope direction.
High R2 = strong linear trend.

Backtest: IS Sharpe 1.521, WF 4/5, SH p=0.034, H-012 corr -0.030.
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        p = closes[col].iloc[-lb:].values
        if not np.all(np.isfinite(p)):
            continue
        slope, intercept, r_val, p_val, std_err = sp_stats.linregress(x, p)
        r2 = r_val ** 2
...
def run():
    print("=== H-916 Trend Linearity XS Paper Trade Runner ===")
