"""
H-1047 Volume-Price Agreement XS Paper Trade Runner

Correlation between volume and |returns| over 20 days. High agreement = efficient pricing.
Backtest: IS Sharpe 1.914, WF 3/5, SH p=0.008, H-012 corr -0.018.
Best params: P=20, R=3, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        if col not in volumes.columns:
            continue
        r = abs_returns[col].iloc[-20:].dropna().values
        v = volumes[col].iloc[-20:].dropna().values
        mn = min(len(r), len(v))
        if mn < 10:
            continue
...
def run():
    print("=== H-1047 Volume-Price Agreement XS Paper Trade Runner ===")
