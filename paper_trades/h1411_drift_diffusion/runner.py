"""
H-1411 Drift/Diffusion Ratio XS Paper Trade Runner

mean(r_t) / std(r_t) over 15 days — classic risk-adjusted return rate.
High = positive drift relative to diffusion. Long high.
Backtest: IS Sharpe 1.278, WF 3/4 (final 3 folds positive), SH 0.96/1.02, p=0.035, H-012 corr 0.328.
Params: R=3, N=4, lb=15, dir=high_long.
"""
...
CONFIG = {"rebal_freq": 3, "n_long": 4, "n_short": 4, "lb": 15, "initial_capital": 10_000.0,
...
def compute_signal(closes_df):
    """Drift/Diffusion: mean(r) / std(r) over lb days."""
    rets = closes_df.pct_change()
    return rets.rolling(CONFIG["lb"]).mean() / (rets.rolling(CONFIG["lb"]).std() + 1e-12)
...
def run():
    print("=== H-1411 Drift/Diffusion Ratio XS Paper Trade Runner ===")
