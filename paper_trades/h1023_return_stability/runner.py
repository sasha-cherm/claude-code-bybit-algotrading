"""
H-1023 Return Stability XS Paper Trade Runner

Inverse coefficient of variation of weekly returns.
Low CV = stable/consistent returner. Long stable, short erratic.
Backtest: IS Sharpe 1.296, WF 4/4, SH p=0.074, H-012 corr -0.051.
Best params: P=30, R=5, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    lb = CONFIG["lookback"]
    sp = CONFIG["sub_period"]
    if len(closes) < lb + 5:
        return {}
    returns = closes.pct_change(sp)
    n_periods = lb // sp
    signal = pd.Series(np.nan, index=closes.columns)
    for col in closes.columns:
        weekly_rets = []
        for k in range(n_periods):
            idx = len(returns) - 1 - k * sp
            if idx >= 0:
        ...
        if len(weekly_rets) < 4:
            continue
        mean_ret = abs(np.mean(weekly_rets))
        std_ret = np.std(weekly_rets)
        if mean_ret > 0 and std_ret > 0:
            signal[col] = -std_ret / mean_ret  # negative CV: high = stable
...
def run():
    print("=== H-1023 Return Stability XS Paper Trade Runner ===")
