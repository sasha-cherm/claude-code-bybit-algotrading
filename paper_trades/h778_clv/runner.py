"""
H-778 Paper Trade Runner: Close Location Value XS

Long assets closing near daily highs, short those closing near lows.

Backtest: IS Sharpe 1.506, Ann +60.6%, WF 4/4 PERFECT. SH p=0.039.
H-012 corr -0.034. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    ...
    for sym, daily in daily_dict.items():
        ...
        clv = (c - l) / rng
        avg_clv = clv.iloc[-window:].mean()
        if np.isfinite(avg_clv):
            scores[sym] = avg_clv
...
def run():
    print("=== H-778 Close Location Value Paper Trade Runner ===")
