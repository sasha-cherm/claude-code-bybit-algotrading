"""
H-197 Paper Trade Runner: Amihud Illiquidity Factor (14 Assets)

Market-neutral strategy: rank assets by Amihud (2002) illiquidity measure.
Amihud = mean(|return| / dollar_volume) over lookback window.
Long the most liquid assets (low Amihud), short the most illiquid.
"Flight to liquidity" factor in crypto.
Rebalance every 3 days.

Backtest: IS 100% positive (30/30), mean Sharpe 1.537, best LB10_R3_N4 Sharpe 1.895
(+89.8% ann, -29.7% DD). WF 5/6 positive, mean OOS 1.387.
Split-half H1=1.911, H2=2.290. Corr H-012 0.488, H-076 0.000.
"""
...
CONFIG = {
    "lookback": 10,          # 10-day rolling Amihud window (best param)
    "rebal_freq": 3,         # rebalance every 3 days
    "n_long": 4,             # long bottom 4 (lowest Amihud = most liquid)
    "n_short": 4,            # short top 4 (highest Amihud = most illiquid)
...
def compute_rankings(closes: pd.DataFrame, volumes: pd.DataFrame,
                     date_idx: int) -> dict:
    """
    Rank assets by Amihud illiquidity ratio.
    Amihud = mean(|return| / dollar_volume) over lookback window.
    Low Amihud = most liquid (large-cap, high-volume) -> long.
    High Amihud = most illiquid (small-cap, thin) -> short.
    """
    ...
    for sym in closes.columns:
        ...
        abs_rets = c.pct_change().abs()
        # Dollar volume
        dollar_vol = v * c

        # Amihud ratio: |return| / dollar_volume
        # Scale by 1e6 to avoid tiny numbers
        amihud = (abs_rets / dollar_vol) * 1e6

        # Rolling mean over lookback
        avg_amihud = amihud.iloc[-lookback:].mean()
        if not np.isnan(avg_amihud) and avg_amihud > 0:
            signals[sym] = avg_amihud
    ...
    ranked = pd.Series(signals).sort_values(ascending=True)  # lowest Amihud first

    longs = ranked.index[:n_long]      # lowest Amihud = most liquid -> long
    shorts = ranked.index[-n_short:]   # highest Amihud = most illiquid -> short
...
# ── Main runner ──────────────────────────────────────────────────────
def run():
    print("=== H-197 Amihud Illiquidity Paper Trade Runner ===")
