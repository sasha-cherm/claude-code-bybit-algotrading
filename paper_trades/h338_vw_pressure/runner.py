"""
H-338 Paper Trade Runner: Volume-Weighted Directional Pressure

Market-neutral strategy: rank 14 crypto assets by volume-weighted
directional pressure (sum of volume × sign(return) / total volume
computed from hourly data, averaged over lookback). High pressure
captures net buying activity weighted by volume → long.

Backtest: IS 100% high_long (24/24). Best LB10_R3_N4 Sharpe 2.136
(WF 6/6 mean 2.390). Split-half H1=1.842, H2=2.428.
Corr H-012 0.289 — novel volume-flow signal.
"""
...
def compute_hourly_features():
    """Compute volume-weighted directional pressure from hourly data."""
    features = {}
    for sym in ASSETS:
        try:
            ...
            for date, group in df_1h.groupby("date"):
                if len(group) < 18:  # need most of the day
                    continue

                vol = group["volume"].values
                ret = group["hour_return"].values
...
def compute_rankings(closes: pd.DataFrame, features: dict, date_idx: int) -> dict:
    """
    Rank assets by volume-weighted directional pressure (avg over lookback).
    High pressure -> long (high_long direction).
    """
...
def run():
    print("=== H-338 VW Directional Pressure Paper Trade Runner ===")
    ...
    print("Computing hourly VW directional pressure features...")
