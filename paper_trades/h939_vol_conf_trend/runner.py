"""
H-939 Volume-Confirmed Trend XS Paper Trade Runner

Momentum * volume trend ratio. Amplifies momentum when volume is rising.
Backtest: IS Sharpe 1.182, WF 4/5, SH p=0.101, H-012 corr 0.001.
Best params: M=20, V=10, R=5, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    mp = CONFIG["mom_period"]
    vp = CONFIG["vol_period"]
    if len(closes) < mp + 10:
        return {}
    # Momentum
    mom = closes.pct_change(mp).iloc[-1]
    # Volume trend: short-term avg / longer-term avg
    vol_short = volumes.rolling(vp).mean().iloc[-1]
    vol_long = volumes.rolling(mp).mean().iloc[-1].replace(0, np.nan)
    vol_ratio = vol_short / vol_long
    # Interaction: momentum * volume ratio
    signal = mom * vol_ratio
...
def run():
    print("=== H-939 Volume-Confirmed Trend XS Paper Trade Runner ===")
