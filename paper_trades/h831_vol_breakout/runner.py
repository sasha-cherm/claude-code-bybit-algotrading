"""
H-831 Paper Trade Runner: Volume-Confirmed Breakout XS

Price position in N-day range × volume ratio.
Near N-day high + above-average volume = confirmed breakout.

Backtest: IS Sharpe 1.274, Ann +50.7%, WF 4/5. SH p=0.078.
H-012 corr 0.006. 94% param robust.
Best params: lb=30, R=3, N=4.
"""
...
def compute_rankings(daily_dict):
    """Rank by volume-confirmed breakout: price position in range × volume ratio."""
    lb = CONFIG["lookback"]
    ...
    high_n = closes.rolling(lb).max()
    low_n = closes.rolling(lb).min()
    range_n = high_n - low_n
    position = (closes - low_n) / range_n.replace(0, np.nan)  # 1 = at high, 0 = at low
    vol_ratio = volumes / volumes.rolling(lb).mean().replace(0, np.nan)

    # Signal: proximity to high × volume confirmation
    signal = (position * vol_ratio).iloc[-1].dropna()
...
def run():
    print("=== H-831 Volume-Confirmed Breakout Paper Trade Runner ===")
