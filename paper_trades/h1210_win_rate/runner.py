"""
H-1210 Win Rate XS Ranking XS Paper Trade Runner

Long assets with high win rate (% positive days), short low win rate. Anti-momentum (corr -0.093).
Backtest: IS Sharpe 1.284, WF 4/4 PERFECT, SH 0.65/2.05, p=0.076, H-012 corr -0.093.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    """Win Rate: % of positive return days over 20d."""
    returns = closes_df.pct_change()
    pos_days = (returns > 0).astype(float)
    return pos_days.rolling(20).mean()
...
def run():
    print("=== H-1210 Win Rate XS Ranking XS Paper Trade Runner ===")
