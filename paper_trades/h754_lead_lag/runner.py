"""
H-754 Paper Trade Runner: Lead-Lag Signal XS

Long assets whose lagged returns predict BTC direction (high lead-lag corr).
Short assets with negative lead-lag correlation.

Backtest: IS Sharpe 1.232, Ann +61.7%, WF 4/4. SH p=0.089.
H-012 corr -0.014. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    """Rank by lead-lag correlation: corr(asset_ret(t-1), BTC_ret(t))."""
    if "BTC/USDT" not in daily_dict:
        return {}
    ...
    btc_ret = returns.get("BTC/USDT")
    if btc_ret is None or len(btc_ret) < CONFIG["corr_window"] + 5:
        return {}

    scores = {}
    window = CONFIG["corr_window"]
    for sym in ASSETS:
        if sym == "BTC/USDT" or sym not in returns:
            continue
        lagged = returns[sym].shift(1)
        common = btc_ret.index.intersection(lagged.dropna().index)
        if len(common) < window + 5:
            continue
        btc_tail = btc_ret.loc[common].iloc[-window:]
        lag_tail = lagged.loc[common].iloc[-window:]
        mask = np.isfinite(btc_tail.values) & np.isfinite(lag_tail.values)
        if mask.sum() < window // 2:
            continue
        corr = np.corrcoef(btc_tail.values[mask], lag_tail.values[mask])[0, 1]
        if np.isfinite(corr):
            scores[sym] = corr
...
def run():
    print("=== H-754 Lead-Lag Paper Trade Runner ===")
