"""
H-1043 Return Predictability (R-squared) XS Paper Trade Runner

R-squared of linear fit to log prices over 20d. High R2 = smooth predictable trend.
Backtest: IS Sharpe 1.313, WF 5/5, SH p=0.068, H-012 corr 0.008.
Best params: P=20, R=5, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        y = log_prices[col].iloc[-20:].values
        if np.sum(np.isfinite(y)) < 16:
            continue
        y_clean = np.where(np.isfinite(y), y, np.nanmean(y))
        slope, intercept, r_value, p_value, std_err = sp_stats.linregress(x, y_clean)
        signal[col] = r_value ** 2 * np.sign(slope)
...
def run():
    print("=== H-1043 Return Predictability (R-squared) XS Paper Trade Runner ===")
