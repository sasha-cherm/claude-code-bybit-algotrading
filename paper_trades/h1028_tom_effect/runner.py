"""
H-1028 Turn-of-Month Effect XS Paper Trade Runner

Average TOM return premium per asset (last 3 + first 3 days of month vs mid). Long high TOM premium assets.
Backtest: IS Sharpe 1.979, WF 3/4, SH p=0.008, H-012 corr -0.031.
Best params: P=180, R=7, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        window = returns[col].iloc[-180:].dropna()
        if len(window) < 60:
            continue
        tom_rets = []
        non_tom_rets = []
        ...
        if len(tom_rets) > 5 and len(non_tom_rets) > 5:
            signal[col] = np.mean(tom_rets) - np.mean(non_tom_rets)
...
def run():
    print("=== H-1028 Turn-of-Month Effect XS Paper Trade Runner ===")
