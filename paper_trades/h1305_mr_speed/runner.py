"""
H-1305 Mean Reversion Speed XS Paper Trade Runner

AR(1) coefficient on log prices over 60d. High = trending, outperforms.
Backtest: IS Sharpe 1.659, WF 3/4, SH 1.44/2.06, p=0.022, H-012 corr -0.030.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """H-1305 Mean Reversion Speed — AR(1) coeff on log prices, 60d."""
    log_prices = np.log(closes_df)
    ...
    for col in closes_df.columns:
        ...
        for i in range(60, len(series)):
            y = series.iloc[i-59:i+1].values
            x = series.iloc[i-60:i].values
            if np.std(x) < 1e-15:
                result.iloc[i][col] = 1.0
                continue
            slope, _, _, _, _ = scipy_stats.linregress(x, y)
            result.at[result.index[i], col] = slope
    return result
...
def run():
    print("=== H-1305 Mean Reversion Speed XS Paper Trade Runner ===")
