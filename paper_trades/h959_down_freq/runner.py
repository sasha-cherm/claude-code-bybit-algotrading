"""
H-959 Down Day Frequency XS Paper Trade Runner

Fraction of negative return days in lookback. Fewer down days = healthier trend.
Backtest: IS Sharpe 1.289, WF 5/5, SH p=0.074, H-012 corr 0.030.
Best params: P=20, R=5, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    neg_count = ret.rolling(lb).apply(lambda x: np.sum(x < 0), raw=True)
    signal = -(neg_count / lb).iloc[-1]  # fewer down days = higher signal
...
def run():
    print("=== H-959 Down Day Frequency XS Paper Trade Runner ===")
