"""
H-1216 Return Per Unit Volume XS Paper Trade Runner

Low return-per-volume assets outperform (more liquid/efficient). Contrarian direction.
Backtest: IS Sharpe 1.452, WF 4/4 PERFECT, SH 1.28/1.63, p=0.045, H-012 corr -0.023.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    """Return Per Unit Volume: sum(|ret|) / sum(vol) over 20d."""
    returns = closes_df.pct_change()
    abs_ret_sum = returns.abs().rolling(20).sum()
    vol_sum = volumes_df.rolling(20).sum().clip(lower=1)
    return abs_ret_sum / vol_sum
...
def run():
    print("=== H-1216 Return Per Unit Volume XS Paper Trade Runner ===")
