"""
H-854 Paper Trade Runner: Close Location Value XS

CLV = (Close - Low) / (High - Low), averaged over window.
High CLV = consistently closing near highs = bullish accumulation.

Backtest: IS Sharpe 1.267, WF 4/5, SH p=0.078.
H-012 corr 0.005. 94% param robust.
...
def compute_rankings(daily_dict):
    """Rank by Close Location Value (CLV)."""
    lb = CONFIG["lookback"]
    ...
    signal = clv.rolling(lb).mean().iloc[-1].dropna()
...
def run():
    print("=== H-854 Close Location Value Paper Trade Runner ===")
