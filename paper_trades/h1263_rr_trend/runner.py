"""
H-1263 Risk-Return Trend XS Paper Trade Runner

20d rolling Sharpe minus 40d rolling Sharpe. Improving risk-reward.
Backtest: IS Sharpe 1.521, WF 4/4 PERFECT, SH 1.22/1.98, p=0.035, H-012 corr -0.036.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    for col in closes_df.columns:
        r = returns[col]
        sh20 = r.rolling(20).mean() / (r.rolling(20).std() + 1e-10) * np.sqrt(365)
        sh40 = r.rolling(40).mean() / (r.rolling(40).std() + 1e-10) * np.sqrt(365)
        result[col] = sh20 - sh40
    return result
...
def run():
    print("=== H-1263 Risk-Return Trend XS Paper Trade Runner ===")
