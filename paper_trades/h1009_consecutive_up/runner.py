"""
H-1009 Consecutive Up Days XS Paper Trade Runner

Maximum streak of consecutive positive daily returns in 20-day lookback.
Long assets with longer winning streaks (trend persistence).
Backtest: IS Sharpe 1.490, WF 4/5, SH p=0.039, H-012 corr 0.042.
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        streak_data = up[col].iloc[-lb:].values
        max_streak = 0
        current = 0
        for v in streak_data:
            if v > 0:
                current += 1
                max_streak = max(max_streak, current)
            else:
                current = 0
        signal[col] = max_streak
...
def run():
    print("=== H-1009 Consecutive Up Days XS Paper Trade Runner ===")
