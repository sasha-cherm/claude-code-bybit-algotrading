"""
H-1018 Price Compression XS Paper Trade Runner

|mean return| / std return over 20-day window.
High = directional (trending). Low = noisy.
Backtest: IS Sharpe 1.432, WF 4/5, SH p=0.047, H-012 corr -0.017.
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        rets = returns[col].iloc[-lb:].dropna().values
        if len(rets) < 10:
            continue
        mean_ret = np.mean(rets)
        std_ret = np.std(rets)
        if std_ret > 0:
            signal[col] = abs(mean_ret) / std_ret
...
def run():
    print("=== H-1018 Price Compression XS Paper Trade Runner ===")
