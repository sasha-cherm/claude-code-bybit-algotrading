"""
H-470 Paper Trade Runner: First-Hour Return Factor

Market-neutral strategy: rank 14 crypto assets by rolling average of
first-hour return (computed from previous day's hourly bars).
HIGH first-hour return → long (strong opening = continuation).

Backtest: IS 100% (high_long). Best LB20_R7_N4.
Sharpe 1.564, +88.0% ann, -24.5% DD.
WF 4/6 mean 0.365. Split-half H1=1.665, H2=0.411.
Corr H-012 0.267 — moderate.
"""
...
def compute_first_hour_ret_daily(df_1h_sym):
    """Compute daily first-hour return from hourly bars."""
    df = df_1h_sym.copy()
    if df.index.tz:
    ...
    for date_ts, group in df.groupby("date"):
        ...
        h_open = group["open"].values
        if h_open[0] > 0:
            fhr = (h_close[0] - h_open[0]) / h_open[0]
            if np.isfinite(fhr):
                daily_fhr[date_ts] = fhr
    return pd.Series(daily_fhr).sort_index() if daily_fhr else None


def compute_rankings(hourly_dict):
    """Rank assets by rolling avg first-hour return. HIGH → long."""
    scores = {}
    for sym, df_1h in hourly_dict.items():
        fhr_series = compute_first_hour_ret_daily(df_1h)
        if fhr_series is None or len(fhr_series) < CONFIG["lookback"]:
            continue
        rolling_mean = fhr_series.rolling(CONFIG["lookback"],
...
def run():
    print("=== H-470 First-Hour Return Paper Trade Runner ===")
