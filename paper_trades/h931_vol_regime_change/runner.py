"""
H-931 Volume Regime Change XS Paper Trade Runner

Short-term avg volume / long-term avg volume.
High = volume spike (regime change). Captures sudden interest shifts.

Backtest: IS Sharpe 1.796, WF 3/4, SH p=0.013, H-012 corr 0.000.
Best params: S=5, L=30, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(volumes) < lb + 5:
        return {}
    short_avg = volumes.rolling(5).mean()
    long_avg = volumes.rolling(lb).mean().replace(0, np.nan)
    sig_row = (short_avg / long_avg).iloc[-1]
...
def run():
    print("=== H-931 Vol Regime Change XS Paper Trade Runner ===")
