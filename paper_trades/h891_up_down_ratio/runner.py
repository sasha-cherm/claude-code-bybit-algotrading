"""
H-891 Up/Down Day Ratio XS Paper Trade Runner

Proportion of positive return days over lookback period.
High ratio = consistent upward sentiment.

Backtest: IS Sharpe 1.233, WF 4/5, SH p=0.087, H-012 corr 0.015.
Best params: P=20, R=5, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in returns.columns:
        r = returns[col].iloc[-lb:]
        if len(r) == lb:
            sig_row[col] = (r > 0).sum() / lb
...
def run():
    print("=== H-891 Up/Down Day Ratio XS Paper Trade Runner ===")
