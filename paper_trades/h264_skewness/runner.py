"""
H-264 Paper Trade Runner: Return Skewness Factor

Market-neutral strategy: rank 14 crypto assets by return skewness over
lookback window. Long assets with high positive skew (breakout/momentum
phase), short assets with low/negative skew.

Backtest: IS 91.7% high_long (22/24). Best LB60_R3_N3 Sharpe 1.879.
WF 6/6 mean OOS 1.532. Split-half H1=2.398, H2=0.872. Corr H-012 0.400.
"""
...
def compute_rankings(closes: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by return skewness over lookback.
    High positive skew -> long (breakout/momentum phase).
    """
    ...
    # High skew -> long (high_long direction)
...
def run():
    print("=== H-264 Return Skewness Paper Trade Runner ===")
