"""
H-1081 Relative Volume Surprise XS Paper Trade Runner

Assets with volume surge relative to XS peers → long. Volume decline → short.
Backtest: IS Sharpe 1.923, WF 4/4 PERFECT, SH p=0.007, H-012 corr 0.043.
SESSION BEST — SH 1.97/1.90 remarkably stable across both halves.
...
def compute_signal(closes_df, volumes_df):
    """Relative volume surprise: asset's 5d/20d volume ratio minus XS mean."""
    vol_ratio = volumes_df.rolling(5).mean() / volumes_df.rolling(20).mean()
    xs_vol_mean = vol_ratio.mean(axis=1)
    return vol_ratio.sub(xs_vol_mean, axis=0)
...
def run():
    print("=== H-1081 Relative Volume Surprise XS Paper Trade Runner ===")
