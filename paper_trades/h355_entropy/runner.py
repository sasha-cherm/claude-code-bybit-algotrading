"""
H-355 Paper Trade Runner: Hourly Return Entropy Factor

Market-neutral strategy: rank 14 crypto assets by Shannon entropy of
discretized hourly returns. Low entropy = structured/trending intraday
price action (predictable, informed flow). High entropy = random walk
(noisy, unpredictable). Long low-entropy, short high-entropy.

Backtest: IS 100% low_long (30/30). Best LB14_R3_N3 Sharpe 1.657
(WF 5/6 mean 1.684). Split-half H1=0.353, H2=1.059.
Corr H-012 0.079, H-076 -0.020. Near-zero correlation with all strategies.
"""
        ...
        try:
            ...
            for date, hg in df_1h.groupby("date"):
                ...
                if len(ret) >= 10 and np.std(ret) > 0:
                    ...
                    daily_entropy[pd.Timestamp(date)] = float(entropy)
                else:
                    daily_entropy[pd.Timestamp(date)] = 0.0
            features_dict[sym] = pd.Series(daily_entropy, name="entropy")
...
def compute_rankings(closes, features_dict, date_idx) -> dict:
    ...
    # Low entropy -> long (structured/trending)
...
def run():
    print(f"=== H-355 Hourly Return Entropy Paper Trade Runner ===")
