"""
H-1333 Positive Return Concentration XS Paper Trade Runner

Gini coefficient of positive daily returns in 20d. Concentrated gains = few big up days.
Backtest: IS Sharpe 1.290, WF 2/4, SH 1.08/1.71, p=0.074, H-012 corr -0.025.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """H-1333 Positive Return Concentration — Gini of positive returns in 20d."""
    ...
    for col in closes_df.columns:
        ...
        for i in range(20, len(s)):
            chunk = s.iloc[i-20:i].values
            pos = chunk[chunk > 0]
            if len(pos) < 2:
                result.at[result.index[i], col] = 0
                continue
            pos = np.sort(pos)
            n = len(pos)
            idx_arr = np.arange(1, n + 1)
            result.at[result.index[i], col] = (2 * np.sum(idx_arr * pos) / (n * np.sum(pos))) - (n + 1) / n
    return result
...
def run():
    print("=== H-1333 Positive Return Concentration XS Paper Trade Runner ===")
