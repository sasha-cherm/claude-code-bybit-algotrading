"""
H-1340 Volume-Weighted Return XS Paper Trade Runner

Average daily return weighted by relative volume (vol / 20d avg vol). High = positive returns on high-volume days.
Backtest: IS Sharpe 1.196, WF 3/4, SH 1.61/0.65, p=0.098, H-012 corr -0.008.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """H-1340: Volume-weighted return — mean(return * rel_volume) over 20d."""
    returns = closes_df.pct_change()
    avg_vol = volumes_df.rolling(20).mean().replace(0, np.nan)
    rel_vol = volumes_df / avg_vol
    return (returns * rel_vol).rolling(20).mean()
...
def run():
    print("=== H-1340 Volume-Weighted Return XS Paper Trade Runner ===")
