"""
H-447 Paper Trade Runner: Hourly Volume Autocorrelation Factor

Market-neutral strategy: rank 14 crypto assets by rolling average of
hourly volume autocorrelation (computed from previous day's hourly bars).
HIGH vol autocorr → long (predictable volume = institutional patterns).

Backtest: IS 87.5% (high_long). Best LB15_R3_N3.
Sharpe 0.741, +38.9% ann, -48.8% DD.
WF 4/6 mean 0.859. Split-half H1=0.754, H2=1.391.
Corr H-012 0.039 — near-zero, excellent diversifier.
"""
...
def compute_vol_autocorr_daily(df_1h_sym):
    """Compute daily volume autocorrelation from hourly data."""
    df = df_1h_sym.copy()
    if df.index.tz:
    ...
    for date_ts, group in df.groupby("date"):
        h_vol = group["volume"].values
        if len(h_vol) >= 8:
            vac = np.corrcoef(h_vol[:-1], h_vol[1:])[0, 1]
            if np.isfinite(vac):
                daily_vac[date_ts] = vac
    return pd.Series(daily_vac).sort_index() if daily_vac else None


def compute_rankings(hourly_dict):
    """Rank assets by rolling avg vol autocorrelation. HIGH → long."""
    scores = {}
    for sym, df_1h in hourly_dict.items():
        vac_series = compute_vol_autocorr_daily(df_1h)
        if vac_series is None or len(vac_series) < CONFIG["lookback"]:
            continue
        rolling_mean = vac_series.rolling(CONFIG["lookback"],
...
def run():
    print("=== H-447 Vol Autocorrelation Paper Trade Runner ===")
