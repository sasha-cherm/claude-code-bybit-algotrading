"""
H-935 Trend Strength R² XS Paper Trade Runner

Signed R² of linear regression on returns. High = clean uptrend.
Backtest: IS Sharpe 1.243, WF 3/4, SH p=0.087, H-012 corr -0.043.
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        ...
        if not np.all(np.isfinite(r)):
            continue
        slope, intercept, r_val, p_val, std_err = sp_stats.linregress(x, r)
        r2 = r_val ** 2
...
def run():
    print("=== H-935 Trend Strength R² XS Paper Trade Runner ===")
