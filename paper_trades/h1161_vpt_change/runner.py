"""
H-1161 Volume-Price Trend Change XS Paper Trade Runner

Long assets with strong 20d VPT change (volume confirming price), short weak VPT.
Backtest: IS Sharpe 1.327, WF 3/4, SH 2.01/0.43, p=0.066, H-012 corr -0.021.
...
def compute_signal(closes_df, volumes_df):
    """VPT Change: 20d change in cumulative volume-price trend, normalized."""
    returns = closes_df.pct_change()
    result = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    vol_20 = volumes_df.rolling(20).mean().clip(lower=1)
    for col in closes_df.columns:
        if col in returns.columns and col in volumes_df.columns:
            vpt_cum = (returns[col] * volumes_df[col]).cumsum()
            result[col] = vpt_cum.diff(20) / vol_20[col]
    return result
...
def run():
    print("=== H-1161 Volume-Price Trend Change XS Paper Trade Runner ===")
