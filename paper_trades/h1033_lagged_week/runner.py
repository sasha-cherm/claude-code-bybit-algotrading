"""
H-1033 Lagged Week Return XS Paper Trade Runner

Prior week's XS return as signal. Weekly momentum carry-over.
Backtest: IS Sharpe 1.400, WF 4/5, SH p=0.050, H-012 corr -0.023.
Best params: P=7, R=5, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(closes) < CONFIG["lookback"] + 5:
        return {}
    ret7 = closes.pct_change(7)
    signal = ret7.iloc[-2].dropna()
    sig_row = signal
...
def run():
    print("=== H-1033 Lagged Week Return XS Paper Trade Runner ===")
