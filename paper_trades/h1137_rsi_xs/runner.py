"""
H-1137 RSI Cross-Sectional XS Paper Trade Runner

Long assets with highest 14-day RSI (momentum), short lowest RSI.
Backtest: IS Sharpe 1.552, WF 3/4, SH 1.59/1.54, p=0.032, H-012 corr 0.021.
Very stable split-half. Nearly zero momentum correlation.
...
def compute_signal(closes_df, volumes_df):
    """RSI(14) cross-sectional: rank by relative RSI level."""
    rsi_df = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    for col in closes_df.columns:
        delta = closes_df[col].diff()
...
def compute_rankings(signal_df):
    ...
    ranked = sig_row.sort_values(ascending=False)  # high_long: long highest RSI
    weights = {}
...
def run():
    print("=== H-1137 RSI Cross-Sectional XS Paper Trade Runner ===")
