"""
H-863 Win Rate XS Paper Trade Runner

Fraction of positive return days — long high win-rate, short low win-rate.

Backtest: IS Sharpe 1.207, WF 4/5, SH p=0.093, H-012 corr -0.022.
Best params: W=20, R=7, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    lb = CONFIG["lookback"]
    signal = (returns > 0).astype(float).rolling(lb).mean()
...
def run():
    print("=== H-863 Win Rate XS Paper Trade Runner ===")
