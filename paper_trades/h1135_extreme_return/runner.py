"""
H-1135 Extreme Return Reversal XS Paper Trade Runner

Long extreme losers (z < -2), short extreme winners (z > 2).
Signal: 5d return z-score (vs 20d vol), zeroed when |z| < 2.
Backtest: IS Sharpe 1.836, WF 4/4 PERFECT, SH 2.08/1.59, p=0.011, H-012 corr -0.002.
SESSION BEST. 100% IS positive.
Params: R=3, N=3, dir=low_long.
"""
...
CONFIG = {"rebal_freq": 3, "n_long": 3, "n_short": 3, "initial_capital": 10_000.0,
...
def compute_signal(closes_df, volumes_df):
    """Extreme Return Indicator: 5d return z-score, zeroed when |z| < 2."""
    returns = closes_df.pct_change()
    ret_5d = closes_df.pct_change(5)
    ret_std = returns.rolling(20).std() * np.sqrt(5)
    z_score = ret_5d / ret_std.clip(lower=1e-8)
    extreme = z_score.copy()
    extreme[z_score.abs() < 2] = 0
    return extreme
...
def compute_rankings(signal_df):
    ...
    ranked = sig_row.sort_values(ascending=True)  # low_long: long extreme losers (negative z)
    weights = {}
...
def run():
    print("=== H-1135 Extreme Return Reversal XS Paper Trade Runner ===")
    ...
    if latest_date == state.get("last_daily_date"):
        ...
        signal_df = compute_signal(closes, volumes)
        ...
        if days_since >= CONFIG["rebal_freq"]:
            ...
            if new_weights:
                ...
                log.append({"time": datetime.now(timezone.utc).isoformat(), "action": "rebalance",
                            "date": latest_date,
