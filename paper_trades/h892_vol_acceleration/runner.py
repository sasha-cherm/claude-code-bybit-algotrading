"""
H-892 Volume Acceleration XS Paper Trade Runner

Second derivative of volume: change in short/long volume ratio.
High acceleration = volume surge building.

Backtest: IS Sharpe 1.455, WF 3/4, SH p=0.045, H-012 corr -0.026.
Best params: Short=7, Long=30, R=7, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(volumes) < long_w + short_w + 5:
        return {}
    vol_mom = volumes.rolling(short_w).mean() / volumes.rolling(long_w).mean().replace(0, np.nan)
    accel = vol_mom.diff(short_w)
    sig_row = accel.iloc[-1].dropna()
...
def run():
    print("=== H-892 Volume Acceleration XS Paper Trade Runner ===")
