"""
H-902 Momentum Quality XS Paper Trade Runner

Sharpe-like measure of recent returns: mean(daily ret) / std(daily ret).
High quality = consistent positive returns, not just large total.

Backtest: IS Sharpe 1.598, WF 3/5, SH p=0.026, H-012 corr -0.017.
Best params: P=14, R=7, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(returns) < lb + 5:
        return {}
    mean_r = returns.rolling(lb).mean()
    std_r = returns.rolling(lb).std().replace(0, np.nan)
    quality = mean_r / std_r
    sig_row = quality.iloc[-1].dropna()
...
def run():
    print("=== H-902 Momentum Quality XS Paper Trade Runner ===")
