"""
H-1186 Up-Capture minus Down-Capture XS Paper Trade Runner

Long high up/down capture ratio (asymmetric upside participation).
Backtest: IS Sharpe 1.240, WF 4/4 PERFECT (all ~1.3), SH 1.23/1.28, p=0.086, H-012 corr 0.024.
Params: R=5, N=4, dir=high_long.
"""
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Up-Capture minus Down-Capture over 30d rolling window."""
    returns = closes_df.pct_change()
    mkt = closes_df.mean(axis=1).pct_change()
    result = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    for col in closes_df.columns:
        r = returns[col]
        up_mask = mkt > 0
        dn_mask = mkt < 0
        up_cap = pd.Series(np.where(up_mask, r / mkt.clip(lower=1e-10), 0), index=closes_df.index)
        dn_cap = pd.Series(np.where(dn_mask, r / mkt.clip(upper=-1e-10), 0), index=closes_df.index)
        result[col] = up_cap.rolling(30).mean() - dn_cap.rolling(30).mean()
    return result
...
def run():
    print("=== H-1186 Up-Capture minus Down-Capture XS Paper Trade Runner ===")
