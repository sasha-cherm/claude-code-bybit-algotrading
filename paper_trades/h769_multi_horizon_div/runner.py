"""
H-769 Paper Trade Runner: Multi-Horizon Divergence XS (Contrarian)

Short assets where short-term momentum diverges above long-term, long the opposite.
(When 5d mom >> 20d mom, asset is overextended → short)

Backtest: IS Sharpe 1.544, Ann +67.7%, WF 4/5. SH p=0.032.
...
def compute_rankings(daily_dict):
    ...
    for sym, daily in daily_dict.items():
        if len(daily) < CONFIG["long_window"] + 10:
            continue
        c = daily["close"]
        mom_short = c.iloc[-1] / c.iloc[-CONFIG["short_window"] - 1] - 1
        mom_long = c.iloc[-1] / c.iloc[-CONFIG["long_window"] - 1] - 1
        divergence = mom_short - mom_long
        if np.isfinite(divergence):
            scores[sym] = divergence
...
def run():
    print("=== H-769 Multi-Horizon Divergence Paper Trade Runner ===")
