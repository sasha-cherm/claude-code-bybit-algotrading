"""
H-786 Paper Trade Runner: Volume-Confirmed Strength XS

Composite: momentum + volume trend + up-volume ratio, z-scored and averaged.

Backtest: IS Sharpe 1.213, Ann +56.7%, WF 3/4. SH p=0.096.
H-012 corr -0.016. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    ...
    mom = closes.pct_change(w).iloc[-2]
    vol_trend = (volumes.rolling(w).mean() / volumes.rolling(w * 2).mean()).iloc[-2]
    ret = closes.pct_change()
    up_vol = ((ret > 0).astype(float) * volumes).rolling(w).sum()
    total_vol = volumes.rolling(w).sum()
    upvol_ratio = (up_vol / total_vol.replace(0, np.nan)).iloc[-2]

    z_mom = xs_zscore(mom)
    z_vol = xs_zscore(vol_trend)
    z_upvol = xs_zscore(upvol_ratio)

    composite = (z_mom + z_vol + z_upvol) / 3
...
def run():
    print("=== H-786 Volume-Confirmed Strength Paper Trade Runner ===")
