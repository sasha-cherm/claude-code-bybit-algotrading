"""
H-250 Paper Trade Runner: US Session Momentum Factor (14 Assets)

Market-neutral strategy: rank assets by fraction of daily absolute return
occurring during US trading hours (13:00-21:00 UTC). High US share
(institutional flow) -> long. Low US share -> short.

Backtest: IS 96.7% high_us_long (29/30). Best LB20_R7_N3 Sharpe 1.057
(+25.5% ann, -29.4% DD). WF 5/5 mean OOS 1.197.
Split-half H1=0.245, H2=2.557. Corr H-012 0.032, H-031 0.378, H-076 -0.227.
"""
...
def load_data():
    """Load hourly data, compute daily US session share, and daily closes."""
    closes_dict = {}
    us_share_dict = {}
    for sym in ASSETS:
        try:
            ...
            # Compute hourly returns and US session share
            h_rets = df_1h["close"].pct_change().dropna()
            if h_rets.index.tzinfo is not None:
                h_rets.index = h_rets.index.tz_localize(None)
            h_abs = h_rets.abs()
            hours = h_rets.index.hour
            grouped = h_abs.groupby(h_abs.index.date)
            daily_us = {}
            for date, grp in grouped:
                total = grp.sum()
                if total > 1e-10 and len(grp) >= 12:
                    us_mask = (grp.index.hour >= CONFIG["us_start_hour"]) & \
                              (grp.index.hour < CONFIG["us_end_hour"])
                    us_abs = grp[us_mask].sum()
                    daily_us[pd.Timestamp(date)] = us_abs / total
            us_share_dict[sym] = pd.Series(daily_us).sort_index()
    ...
    us_share = pd.DataFrame(us_share_dict).reindex(closes.index).ffill()
    return closes, us_share


def compute_rankings(closes: pd.DataFrame, us_share: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by rolling avg US session share.
    High US share (institutional) -> long.
    """
    ...
    avg_us = us_share.rolling(lookback, min_periods=max(lookback // 2, 5)).mean()
    cv = avg_us.iloc[date_idx]
    valid = cv.dropna()
    ...
    # High US share -> long (high_us_long direction)
...
def run():
    print("=== H-250 US Session Momentum Paper Trade Runner ===")
    ...
    if days_since >= CONFIG["rebal_freq"]:
        ...
        new_weights = compute_rankings(closes, us_share, date_idx)
        if not new_weights:
