"""
H-970 Higher Lows Count XS Paper Trade Runner

Count of days where low > prior day low. More higher lows = stronger uptrend structure.
Backtest: IS Sharpe 2.464, WF 5/5, SH p=0.001, H-012 corr -0.008.
Best params: P=10, R=5, N=3. SESSION BEST..
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        if col not in lows_df.columns:
            continue
        lo = lows_df[col].iloc[-lb:].values
...
def run():
    print("=== H-970 Higher Lows Count XS Paper Trade Runner ===")
