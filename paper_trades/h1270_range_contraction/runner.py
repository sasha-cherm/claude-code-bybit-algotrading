"""
H-1270 Range Contraction XS Paper Trade Runner

Fraction of days range < prev range in 20d. Low contraction = range expansion = outperforms.
Backtest: IS Sharpe 1.749, WF 4/4 PERFECT, SH 1.76/1.83, p=0.016, H-012 corr -0.036. SESSION BEST.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    daily_range = highs_df - lows_df
    contracting = (daily_range < daily_range.shift(1)).astype(float)
    return contracting.rolling(20).mean()
...
def run():
    print("=== H-1270 Range Contraction XS Paper Trade Runner ===")
