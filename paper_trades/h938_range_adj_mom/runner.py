"""
H-938 Range-Adjusted Momentum XS Paper Trade Runner

Momentum / average daily range. Normalizes momentum by activity level.
Backtest: IS Sharpe 1.585, WF 3/5, SH p=0.027, H-012 corr 0.018.
Best params: P=14, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(closes) < lb + 5:
        return {}
    # Range-adjusted momentum: momentum / avg daily range
    mom = closes.pct_change(lb).iloc[-1]
    daily_range = ((highs - lows) / closes).rolling(lb).mean().iloc[-1]
    daily_range = daily_range.replace(0, np.nan)
    signal = mom / daily_range
...
def run():
    print("=== H-938 Range-Adjusted Momentum XS Paper Trade Runner ===")
