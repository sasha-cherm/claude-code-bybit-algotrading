"""
H-733 Paper Trade Runner: Dollar Volume Change XS

Rank 14 crypto assets by 10-day change in dollar volume (price × volume).
Long increasing dollar volume, short decreasing.

Backtest: IS Sharpe 1.262 (1.887 on 11 assets), Ann +51.2%.
WF 5/6. SH 0.254/2.242 (BORDERLINE — first half weak).
97% param robust (70/72). H-012 corr 0.046. H-021 corr 0.540.
"""
...
def compute_rankings(daily_dict):
    """Rank by 10-day change in dollar volume."""
    scores = {}
    lb = CONFIG["lookback"]
    for sym, daily in daily_dict.items():
        window = daily.iloc[-(lb + 1 + lb):-1]
        if len(window) < 2 * lb * 0.7:
            continue
        dv = (window["close"] * window["volume"]).values
        if len(dv) >= 2 * lb:
            recent = np.mean(dv[-lb:])
            prior = np.mean(dv[-2 * lb : -lb])
            if prior > 0 and np.isfinite(recent) and np.isfinite(prior):
                scores[sym] = (recent - prior) / prior
...
def run():
    print("=== H-733 Dollar Volume Change Paper Trade Runner ===")
        ...
        if days_since >= CONFIG["rebal_freq"]:
            ...
            if not new_weights:
                ...
                for sym, weight in new_weights.items():
                    ...
                    if (
                        sym not in old_positions
                        or abs(
                            new_weights.get(sym, 0)
                            - old_positions.get(sym, {}).get("weight", 0)
                        )
                        > 0.01
                    ):
                        total_fees += CONFIG["fee_rate"] * notional
                ...
                log.append({
                    ...
                    "date": latest_date,
                    "longs": [
                        s for s, p in new_positions.items() if p["weight"] > 0
                    ],
                    "shorts": [
                        s for s, p in new_positions.items() if p["weight"] < 0
                    ],
