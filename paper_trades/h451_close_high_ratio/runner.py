"""
H-451 Paper Trade Runner: Close-to-High Ratio Factor

Market-neutral strategy: rank 14 crypto assets by rolling average of
hourly close-to-high ratio (computed from previous day's hourly bars).
HIGH ratio → long (consistent buying pressure, closes near highs).

Backtest: IS 100% (high_long). Best LB30_R5_N3.
Sharpe 1.325, +87.4% ann, -50.0% DD.
WF 5/6 mean 1.366. Split-half H1=0.867, H2=0.510.
Corr H-012 0.258 — moderate but acceptable.
"""
...
def compute_close_high_ratio_daily(df_1h_sym):
    """Compute daily avg (close-low)/(high-low) from hourly bars."""
    df = df_1h_sym.copy()
    if df.index.tz:
    ...
    daily_chr = {}
    for date_ts, group in df.groupby("date"):
        h_close = group["close"].values
        h_high = group["high"].values
        h_low = group["low"].values
        ...
        if valid.sum() >= 8:
            ...
            if np.isfinite(chr_val):
                daily_chr[date_ts] = chr_val
    return pd.Series(daily_chr).sort_index() if daily_chr else None


def compute_rankings(hourly_dict):
    """Rank assets by rolling avg close-high ratio. HIGH → long."""
    scores = {}
    for sym, df_1h in hourly_dict.items():
        chr_series = compute_close_high_ratio_daily(df_1h)
        if chr_series is None or len(chr_series) < CONFIG["lookback"]:
            continue
        rolling_mean = chr_series.rolling(CONFIG["lookback"],
...
def run():
    print("=== H-451 Close-High Ratio Paper Trade Runner ===")
