"""
H-814 Paper Trade Runner: Rank Velocity XS

Speed of rank position changes — fast-rising assets continue rising.
Rate of change of momentum rank positions (5-day rank change, smoothed).

Backtest: IS Sharpe 1.886, Ann +71.6%, WF 3/4. SH p=0.011.
H-012 corr 0.062. 100% param robust.
Best params: lb=60, R=10, N=4.
"""
...
def compute_rankings(daily_dict):
    """Rank assets by rank velocity — speed of rank position changes."""
    lb = CONFIG["lookback"]
    ...
    ret = closes.pct_change(lb)
    ranks = ret.rank(axis=1, pct=True)
    rank_change = ranks.diff(5)  # 5-day rank change
    smoothed = rank_change.rolling(lb // 2).mean()

    latest = smoothed.iloc[-1].dropna()
...
def run():
    print("=== H-814 Rank Velocity Paper Trade Runner ===")
