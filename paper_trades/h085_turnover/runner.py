"""
H-085 Paper Trade Runner: Turnover Velocity Factor (14 Crypto Assets)

Market-neutral strategy: rank 14 assets by turnover velocity
(short-term avg dollar volume / long-term avg dollar volume).
Long top N (highest volume surge = growing attention),
short bottom N (lowest volume surge = declining interest).
Rebalance every 7 days.

Backtest: 100% param robustness (48/48 positive), best Sharpe 2.08,
+109% ann, -23% DD. WF (param-selected) 3/4 positive, mean Sharpe 2.37.
Correlation with H-012 (momentum): 0.21.
"""
...
CONFIG = {
    "short_vol_window": 5,   # 5-day short-term volume average
    "long_vol_window": 20,   # 20-day long-term volume average (lookback)
    "rebal_freq": 7,         # rebalance every 7 days
    "n_long": 4,             # long top 4 (highest turnover surge)
    "n_short": 4,            # short bottom 4 (lowest turnover)
...
def load_daily_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    """Fetch 1h data for all assets, resample to daily closes and volumes."""
    daily_closes = {}
    daily_volumes = {}
    for sym in ASSETS:
        try:
            ...
            daily_volumes[sym] = daily["close"] * daily["volume"]
        except Exception as e:
            print(f"  {sym}: failed to load: {e}")

    closes = pd.DataFrame(daily_closes).dropna(how="all").ffill().dropna()
    volumes = pd.DataFrame(daily_volumes).reindex(closes.index).ffill().dropna()
...
def compute_rankings(closes: pd.DataFrame, volumes: pd.DataFrame,
                     date_idx: int) -> dict:
    """
    Rank assets by turnover velocity (lagged by 1 day).
    Turnover velocity = mean(dollar_vol, short_window) / mean(dollar_vol, long_window).
    High ratio = volume surge = growing attention. Long top N, short bottom N.
    """
    short_w = CONFIG["short_vol_window"]
    long_w = CONFIG["long_vol_window"]
    warmup = long_w + 5

    if date_idx < warmup:
        return {}

    # Lagged: use data through (date_idx - 1)
    lagged_idx = date_idx - 1
    if lagged_idx < long_w:
        return {}

    signals = {}
    for sym in volumes.columns:
        vol_series = volumes[sym].iloc[:lagged_idx + 1]
        if len(vol_series) < long_w:
            continue

        short_avg = vol_series.iloc[-short_w:].mean()
        long_avg = vol_series.iloc[-long_w:].mean()

        if long_avg > 0:
            signals[sym] = short_avg / long_avg
...
# -- Main runner --------------------------------------------------------------
def run():
    print("=== H-085 Turnover Velocity Paper Trade Runner ===")
    ...
    if len(closes) < CONFIG["long_vol_window"] + 10:
    ...
    if days_since >= CONFIG["rebal_freq"]:
        ...
        if not new_weights:
            ...
            print(f"  LONG (high turnover):  {', '.join(s.replace('/USDT','') for s in longs)}")
            print(f"  SHORT (low turnover):  {', '.join(s.replace('/USDT','') for s in shorts)}")
