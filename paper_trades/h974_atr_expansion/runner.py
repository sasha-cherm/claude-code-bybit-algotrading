"""
H-974 ATR Expansion XS Paper Trade Runner

ATR(5d) / ATR(30d). Expanding range = breakout / volatility regime shift.
Backtest: IS Sharpe 1.763, WF 4/4, SH p=0.015, H-012 corr -0.006.
Best params: S=5, L=30, R=7, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(closes) < lb + 5:
        return {}
    highs_df = pd.DataFrame({s: d["high"] for s, d in daily_dict.items()}).dropna(how="all").ffill().dropna()
    lows_df = pd.DataFrame({s: d["low"] for s, d in daily_dict.items()}).dropna(how="all").ffill().dropna()
    signal = pd.Series(np.nan, index=closes.columns)
    for col in closes.columns:
        if col not in highs_df.columns or col not in lows_df.columns:
            continue
        h = highs_df[col].values
        l = lows_df[col].values
        c = closes[col].values
        tr = np.maximum(h[1:] - l[1:], np.maximum(np.abs(h[1:] - c[:-1]), np.abs(l[1:] - c[:-1])))
        if len(tr) < 35:
            continue
        atr_short = np.mean(tr[-5:])
        atr_long = np.mean(tr[-30:])
        if atr_long < 1e-10:
            continue
        signal[col] = atr_short / atr_long
...
def run():
    print("=== H-974 ATR Expansion XS Paper Trade Runner ===")
