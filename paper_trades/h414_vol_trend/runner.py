"""
H-414 Paper Trade Runner: Volume Trend Factor

Market-neutral strategy: rank 14 crypto assets by linear regression slope
of log-volume over rolling window. HIGH volume trend (rising volume) → long.
Signal uses LAGGED data (up to yesterday) — no look-ahead.

Backtest (lagged): IS 96.7% (29/30 high_voltrd_long). Best LB15_R3_N4
Sharpe 2.520, WF 5/6 mean 2.437. Split-half H1=2.369, H2=2.762.
Corr H-012 0.028 — near-zero, excellent diversifier.
"""
...
def compute_rankings(daily_dict):
    """Rank assets by volume trend slope. HIGH slope → long."""
    scores = {}
    for sym, daily in daily_dict.items():
        vols = daily["volume"].iloc[-(CONFIG["lookback"]+1):-1]  # Exclude today
        if len(vols) < CONFIG["lookback"] * 0.7:
            continue
...
def run():
    print("=== H-414 Volume Trend Paper Trade Runner ===")
