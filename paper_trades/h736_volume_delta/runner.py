"""
H-736 Paper Trade Runner: Cumulative Volume Delta XS

Approximate buy/sell pressure from OHLC: (close-open)/(high-low) × volume.
Cumulate over lookback, normalize by total volume.
Long highest buy pressure, short highest sell pressure.

Backtest: IS Sharpe 1.703, Ann +70.8%, WF 6/6 (perfect).
SH corrected PASS (p1=0.061, p2=0.026). 96% param robust.
H-012 corr 0.366.
"""
...
def compute_rankings(daily_dict):
    """Rank by cumulative volume delta normalized by total volume."""
    scores = {}
    lb = CONFIG["lookback"]
    for sym, daily in daily_dict.items():
        window = daily.iloc[-(lb + 1):-1]
        if len(window) < lb * 0.7:
            continue
        ...
        hl_range = h - l + 1e-10
        buy_pct = (c - o) / hl_range
        vol_delta = np.sum(buy_pct * v)
        total_vol = np.sum(v)
        if total_vol > 0 and np.isfinite(vol_delta):
            scores[sym] = vol_delta / total_vol
...
def run():
    print("=== H-736 Volume Delta Paper Trade Runner ===")
