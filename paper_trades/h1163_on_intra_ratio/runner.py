"""
H-1163 Overnight/Intraday Ratio XS Paper Trade Runner

Long high overnight/intraday move ratio (institutional accumulation signal).
...
def compute_signal(closes_df, volumes_df, opens_df=None):
    """Overnight/Intraday Ratio: |open_t - close_{t-1}| / |close_t - open_t|, 5d avg."""
    if opens_df is None:
        return closes_df * 0  # fallback
    result = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    for col in closes_df.columns:
        if col in opens_df.columns:
            overnight = (opens_df[col] - closes_df[col].shift(1)).abs()
            intraday = (closes_df[col] - opens_df[col]).abs().clip(lower=1e-10)
            ratio = overnight / intraday
            result[col] = ratio.rolling(5).mean()
    return result
...
def run():
    print("=== H-1163 Overnight/Intraday Ratio XS Paper Trade Runner ===")
    ...
    if latest_date == state.get("last_daily_date"):
        ...
        signal_df = compute_signal(closes, volumes, opens)
