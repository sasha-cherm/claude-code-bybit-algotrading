"""
H-1354 Expected Shortfall Trend XS Paper Trade Runner

Avg loss on worst 3 days in recent 20d minus same for prior 20d. High = tail improving (less extreme losses).
Backtest: IS Sharpe 1.556, WF 3/4, SH 1.56/1.58, p=0.031, H-012 corr 0.013.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """H-1354: ES trend — avg of worst 3 days in recent 20d minus prior 20d."""
    ...
    for col in returns.columns:
        ...
        for i in range(40, len(vals)):
            recent = vals[i-20:i]
            prior = vals[i-40:i-20]
            valid_r = recent[np.isfinite(recent)]
            valid_p = prior[np.isfinite(prior)]
            if len(valid_r) < 3 or len(valid_p) < 3:
                continue
            es_recent = np.mean(np.sort(valid_r)[:3])
            es_prior = np.mean(np.sort(valid_p)[:3])
            out[i] = es_recent - es_prior
        result[col] = out
    return result
...
def run():
    print("=== H-1354 Expected Shortfall Trend XS Paper Trade Runner ===")
