"""
H-219 Paper Trade Runner: Up-Volume Ratio Factor (14 Assets)

Market-neutral strategy: rank assets by ratio of volume on positive-return
days to total volume over a lookback window. Long assets with dominant
buying volume (high up-vol ratio), short those with low up-vol ratio.
Rebalance every 7 days.

Backtest: IS 80.0% upvol_long (24/30). Best LB10_R7_N3 Sharpe 1.177
(+53.1% ann, -43.5% DD). WF 4/6 positive, mean OOS 0.204.
Split-half H1=1.266, H2=2.097. Corr H-012 0.157.
"""
...
def compute_rankings(closes: pd.DataFrame, volumes: pd.DataFrame,
                     date_idx: int) -> dict:
    """
    Rank assets by up-volume ratio over lookback window.
    up_vol_ratio = sum(volume on days where return > 0) / sum(total volume).
    High up-vol ratio (dominant buying) -> long.
    Low up-vol ratio (dominant selling) -> short.
    """
    ...
    for sym in closes.columns:
        ...
        vol_window = volumes[sym].iloc[date_idx - lookback: date_idx].values
    ...
    if len(ratios) < n_long + n_short:
        return {}

    ranked = pd.Series(ratios).sort_values(ascending=False)
...
def run():
    print("=== H-219 Up-Volume Ratio Paper Trade Runner ===")
