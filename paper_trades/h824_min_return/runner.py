"""
H-824 Paper Trade Runner: Min Daily Return (Resilience) XS

Ranks assets by their worst single-day loss over lookback window.
High = least negative worst day = most resilient. Long resilient, short fragile.

Backtest: IS Sharpe 2.094, Ann +93.1%, WF 5/5 PERFECT. SH p=0.004.
H-012 corr -0.052. 100% param robust.
...
def compute_rankings(daily_dict):
    """Rank assets by minimum daily return (resilience). High = less negative = resilient."""
    lb = CONFIG["lookback"]
    ...
    min_ret = ret.rolling(lb).min()  # Most negative = worst day

    latest = min_ret.iloc[-1].dropna()
    n_required = CONFIG["n_long"] + CONFIG["n_short"]
    if len(latest) < n_required:
        return {}

    # high_long: long the ones with HIGHEST min return (least negative worst day = resilient)
...
def run():
    print("=== H-824 Min Daily Return (Resilience) Paper Trade Runner ===")
