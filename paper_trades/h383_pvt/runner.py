"""
H-383 Paper Trade Runner: Price-Volume Trend Factor

Market-neutral strategy: rank 14 crypto assets by normalized OBV-like signal.
Sum of volume × sign(return) over lookback, divided by total volume.
High PVT = buying pressure (volume on up-days dominates). Low PVT = selling pressure.

Backtest: IS 87.5% positive (21/24). Best LB30_R7_N4 Sharpe 1.347
(+52.7% ann, -31.4% DD). WF 4/6 mean 1.312. Split-half H1=1.441, H2=2.226.
Corr H-012 0.435 — moderate correlation with momentum.
"""
...
def compute_rankings(closes: pd.DataFrame, volumes: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by price-volume trend = sum(vol * sign(ret)) / sum(vol).
    High PVT = buying pressure → long.
    Low PVT = selling pressure → short.
    """
    ...
    for sym in closes.columns:
        r = rets[sym].iloc[max(0, date_idx - lookback + 1):date_idx + 1].values
        v = volumes[sym].iloc[max(0, date_idx - lookback + 1):date_idx + 1].values
        mask = ~np.isnan(r) & ~np.isnan(v) & (v > 0)
        if mask.sum() < 10:
            continue
        signed_vol = np.sum(v[mask] * np.sign(r[mask]))
        total_vol = np.sum(v[mask])
        if total_vol > 0:
            pvt = signed_vol / total_vol
            scores[sym] = pvt
...
def run():
    print("=== H-383 Price-Volume Trend Paper Trade Runner ===")
