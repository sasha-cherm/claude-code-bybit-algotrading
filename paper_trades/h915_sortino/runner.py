"""
H-915 Sortino-like XS Paper Trade Runner

Mean return / downside deviation.
Penalizes downside risk, not upside volatility.

Backtest: IS Sharpe 1.549, WF 3/5, SH p=0.032, H-012 corr 0.044.
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        ...
        mean_r = np.mean(r)
        neg = r[r < 0]
        if len(neg) < 3 or np.std(neg) == 0:
            continue
        signal[col] = mean_r / np.std(neg)
    sig_row = signal
...
def run():
    print("=== H-915 Sortino XS Paper Trade Runner ===")
