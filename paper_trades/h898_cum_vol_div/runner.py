"""
H-898 Cumulative Volume Divergence XS Paper Trade Runner

Cumulative sum of (vol - SMA(vol)) / SMA(vol) over short window.
Sustained above-average volume = accumulation signal.

Backtest: IS Sharpe 1.713, WF 4/5, SH p=0.018, H-012 corr -0.008.
Best params: Short=3, Long=20, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    if len(volumes) < long_w + short_w + 5:
        return {}
    sma = volumes.rolling(long_w).mean()
    deviation = (volumes - sma) / sma.replace(0, np.nan)
    cum_div = deviation.rolling(short_w).sum()
    sig_row = cum_div.iloc[-1].dropna()
...
def run():
    print("=== H-898 Cumulative Volume Divergence XS Paper Trade Runner ===")
