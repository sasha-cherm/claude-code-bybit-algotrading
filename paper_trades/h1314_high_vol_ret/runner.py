"""
H-1314 High-Volume Return Sign XS Paper Trade Runner

Average return on top-5-volume days in 20d. Directional bias of active trading days.
Backtest: IS Sharpe 1.545, WF 2/4, SH 1.69/1.39, p=0.033, H-012 corr -0.048.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """H-1314 High-Volume Return Sign — avg return on top-5-volume days in 20d."""
    ...
    for col in closes_df.columns:
        ret_s = returns[col]
        vol_s = volumes_df[col]
        for i in range(20, len(ret_s)):
            r = ret_s.iloc[i-20:i].values
            v = vol_s.iloc[i-20:i].values
            top_idx = np.argsort(v)[-5:]
            result.at[result.index[i], col] = np.mean(r[top_idx])
    return result
...
def run():
    print("=== H-1314 High-Volume Return Sign XS Paper Trade Runner ===")
