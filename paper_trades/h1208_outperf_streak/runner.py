"""
H-1208 Outperformance Streak (Contrarian) XS Paper Trade Runner

Contrarian: long assets that STOPPED outperforming (low streak), short those on winning streaks.
Backtest: IS Sharpe 1.513, WF 3/4, IS 83%, SH 2.20/0.63, p=0.036, H-012 corr -0.053.
Params: R=3, N=4, dir=low_long.
"""
...
CONFIG = {"rebal_freq": 3, "n_long": 4, "n_short": 4, "initial_capital": 10_000.0,
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    """Outperformance Streak: consecutive days outperforming XS median."""
    returns = closes_df.pct_change()
    xs_median = returns.median(axis=1)
    outperf = returns.sub(xs_median, axis=0) > 0
    result = pd.DataFrame(0, index=closes_df.index, columns=closes_df.columns, dtype=float)
...
def run():
    print("=== H-1208 Outperformance Streak (Contrarian) XS Paper Trade Runner ===")
