"""
H-817 Paper Trade Runner: Cross-Asset Vol Spillover XS

When BTC vol rises: long low-vol assets (shelter), short high-vol.
When BTC vol falls: long high-vol assets (risk-on), short low-vol.

Backtest: IS Sharpe 1.423, Ann +55.1%, WF 3/4. SH p=0.054.
H-012 corr 0.011. 83% param robust.
Best params: lb=60, R=7, N=5.
"""
...
def compute_rankings(daily_dict):
    """Rank by vol spillover signal: BTC vol direction × asset vol rank."""
    lb = CONFIG["lookback"]
...
def run():
    print("=== H-817 Vol Spillover Paper Trade Runner ===")
