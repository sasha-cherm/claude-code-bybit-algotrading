"""
H-599 Paper Trade Runner: RSI Cross-Sectional Factor

Market-neutral strategy: rank 14 crypto assets by 14-period RSI.
Long assets with highest RSI (strong momentum), short lowest RSI.
Signal uses LAGGED data (up to yesterday) — no look-ahead.

Backtest: IS Sharpe 1.148, Ann +47.7%, DD -29.4%.
WF 4/6 positive (mean 0.977). Split-half 1.621/0.544 PASS.
Param robust 60/60 (100%), best 2.306.
Corr H-012: 0.455 (borderline but acceptable).
"""
...
def compute_rsi(prices_series, period=14):
    """Compute RSI for a single asset's price series."""
...
def compute_rankings(daily_dict):
    """Rank assets by RSI. HIGH RSI -> long (momentum)."""
    scores = {}
    for sym, daily in daily_dict.items():
        ...
        rsi = compute_rsi(d["close"], CONFIG["rsi_period"])
        latest_rsi = rsi.iloc[-1]

        if np.isfinite(latest_rsi):
            scores[sym] = latest_rsi
...
def run():
    print("=== H-599 RSI Cross-Sectional Paper Trade Runner ===")
