"""
H-1193 Gap Fill Ratio XS Paper Trade Runner

Long high gap-fill ratio assets (frequent gap fills = strong market microstructure).
Backtest: IS Sharpe 1.905, WF 4/4 PERFECT, IS 100%, SH 2.65/1.11, p=0.008, H-012 corr -0.016. SESSION BEST.
Params: R=3, N=3, dir=high_long.
"""
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Gap Fill Ratio: % of overnight gaps filled same day over 20d."""
    if opens_df is None or highs_df is None or lows_df is None:
        return closes_df * 0
    ...
    for col in closes_df.columns:
        if col not in opens_df.columns:
            continue
        gap = opens_df[col] - closes_df[col].shift(1)
        gap_up = gap > 0
        gap_dn = gap < 0
        filled_up = gap_up & (lows_df[col] <= closes_df[col].shift(1))
        filled_dn = gap_dn & (highs_df[col] >= closes_df[col].shift(1))
        ...
        result[col] = fill_count / gap_count
    return result
...
def compute_rankings(signal_df):
    ...
    ranked = sig_row.sort_values(ascending=False)  # high_long
    weights = {}
...
def run():
    print("=== H-1193 Gap Fill Ratio XS Paper Trade Runner ===")
