"""
H-878 Stochastic %K XS Paper Trade Runner

Stochastic oscillator ranked cross-sectionally — long high %K (momentum).

Backtest: IS Sharpe 1.406, WF 3/5, SH p=0.050, H-012 corr 0.005.
Best params: W=14, R=7, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    lb = CONFIG["lookback"]
    low_n = lows.rolling(lb).min()
    high_n = highs.rolling(lb).max()
    denom = (high_n - low_n).replace(0, np.nan)
    signal = (closes - low_n) / denom * 100
...
def run():
    print("=== H-878 Stochastic %K XS Paper Trade Runner ===")
