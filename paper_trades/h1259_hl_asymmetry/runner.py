"""
H-1259 High-Low Asymmetry XS Paper Trade Runner

log((high-close)/(close-low)) averaged 20d. Lower upper wicks → outperforms.
Backtest: IS Sharpe 1.041, WF 3/4, SH 0.63/1.72, p=0.149, H-012 corr 0.007.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """High-Low Asymmetry XS."""
    upper_wick = highs_df - closes_df
    lower_wick = closes_df - lows_df
    wick_ratio = np.log((upper_wick + 1e-10) / (lower_wick + 1e-10))
    return wick_ratio.rolling(20).mean()
...
def run():
    print("=== H-1259 High-Low Asymmetry XS Paper Trade Runner ===")
