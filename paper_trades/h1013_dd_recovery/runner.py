"""
H-1013 DD Recovery XS Paper Trade Runner

Recovery speed from drawdown. Ratio of (close - period_low) / (period_high - period_low).
Fast recoverers = resilient assets.
Backtest: IS Sharpe 1.394, WF 4/4, SH p=0.055, H-012 corr -0.062.
...
def compute_rankings(daily_dict):
    ...
    if len(closes) < lb + 5:
        return {}
    period_high = closes.rolling(lb).max()
    period_low = closes.rolling(lb).min()
    rng = period_high - period_low
    rng = rng.replace(0, np.nan)
    recovery = (closes - period_low) / rng
    signal = recovery.iloc[-1].dropna()
...
def run():
    print("=== H-1013 DD Recovery XS Paper Trade Runner ===")
