"""
H-866 Volume-Weighted Return Divergence XS Paper Trade Runner

VWAP return vs simple return — smart money buying signal.

Backtest: IS Sharpe 1.591, WF 3/5, SH p=0.027, H-012 corr 0.005.
Best params: W=20, R=3, N=4.
"""
    ...
    for i in range(lb + 5, len(returns)):
        for col in returns.columns:
            if col not in volumes.columns:
                continue
            r = returns[col].iloc[i - lb:i].dropna()
            v = volumes[col].iloc[i - lb:i].dropna()
            idx = r.index.intersection(v.index)
            if len(idx) < 10:
                continue
            r_c, v_c = r[idx], v[idx]
            vw_ret = (r_c * v_c).sum() / v_c.sum() if v_c.sum() > 0 else 0
            ew_ret = r_c.mean()
...
def run():
    print("=== H-866 Volume-Weighted Return Divergence XS Paper Trade Runner ===")
