"""
H-1010 Weekly Volume Profile XS Paper Trade Runner

5-day avg volume vs 20-day avg volume.
Volume surge = attention/institutional inflow.
Backtest: IS Sharpe 1.939, WF 3/5, SH p=0.007, H-012 corr -0.033.
Best params: P=20, R=5, N=5, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    lb = CONFIG["lookback"]
    sp = CONFIG["short_vol_period"]
    if len(volumes) < lb + 5:
        return {}
    avg_short = volumes.rolling(sp).mean()
    avg_long = volumes.rolling(lb).mean().replace(0, np.nan)
    signal = (avg_short / avg_long).iloc[-1].dropna()
...
def run():
    print("=== H-1010 Weekly Volume Profile XS Paper Trade Runner ===")
