"""
H-333 Paper Trade Runner: Smart Volume Return (4h Microstructure)

Market-neutral strategy: rank 14 crypto assets by the return of the
highest-volume 4h bar (averaged over lookback). High smart-vol return
captures directional bias of informed high-volume activity → long.

Backtest: IS 100% high_long (24/24). Best LB10_R3_N3 Sharpe 2.447
(WF 6/6 mean 2.467). Split-half H1=2.145, H2=2.924.
Corr H-012 0.428 (borderline but good standalone).
"""
...
def compute_4h_features():
    """Compute smart volume return features from hourly data."""
    features = {}
    for sym in ASSETS:
        try:
            ...
            for date, group in df_4h.groupby("date"):
                if len(group) < 4:
                    continue
                # Smart Volume Return: return of the highest-volume 4h bar
                max_vol_idx = group["volume"].idxmax()
                smart_return = group.loc[max_vol_idx, "bar_return"]
                daily_smart_returns.append({
                    "date": pd.Timestamp(date),
                    "smart_return": float(smart_return),
                })

            if daily_smart_returns:
                feat_df = pd.DataFrame(daily_smart_returns).set_index("date")
...
def compute_rankings(closes: pd.DataFrame, features_4h: dict, date_idx: int) -> dict:
    """
    Rank assets by smart volume return (avg over lookback days).
    High smart return -> long (high_long direction).
    """
...
def run():
    print("=== H-333 Smart Volume Return Paper Trade Runner ===")
    ...
    # Compute 4h features
    print("Computing 4h smart volume return features...")
