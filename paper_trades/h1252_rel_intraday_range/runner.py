"""
H-1252 Relative Intraday Range XS Paper Trade Runner

(high-low)/close 10d avg vs 30d avg ratio. Range expansion signal.
Backtest: IS Sharpe 1.112, WF 3/4, SH 1.35/0.84, p=0.124, H-012 corr -0.028.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Relative Intraday Range XS."""
    hl_range = (highs_df - lows_df) / closes_df
    range_10 = hl_range.rolling(10).mean()
    range_30 = hl_range.rolling(30).mean()
    return range_10 / (range_30 + 1e-10)
...
def run():
    print("=== H-1252 Relative Intraday Range XS Paper Trade Runner ===")
