"""
H-1127 Beta Stability XS Paper Trade Runner

Long assets with stable BTC beta (low std of rolling beta), short unstable.
Beta Stability = std of rolling 20d beta estimates over 60d window.
Backtest: IS Sharpe 1.151, WF 3/3 PERFECT, SH 0.71/1.60, p=0.119, H-012 corr 0.089.
...
def compute_signal(closes_df, volumes_df):
    """Beta Stability: std of rolling 20d beta over 60d window."""
    returns = closes_df.pct_change()
    btc_col = "BTC/USDT" if "BTC/USDT" in returns.columns else returns.columns[0]
    btc_ret = returns[btc_col]
    btc_var_20 = btc_ret.rolling(20).var().clip(lower=1e-10)
    ...
    for col in closes_df.columns:
        ...
        rolling_beta = cov_20 / btc_var_20
        result[col] = rolling_beta.rolling(60).std()
    return result
...
def compute_rankings(signal_df):
    ...
    ranked = sig_row.sort_values(ascending=True)  # low_long: long most stable beta
    weights = {}
...
def run():
    print("=== H-1127 Beta Stability XS Paper Trade Runner ===")
