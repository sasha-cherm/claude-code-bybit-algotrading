"""
H-777 Paper Trade Runner: Price-Volume Trend XS

Long assets with high cumulative (return × volume), short low PVT.

Backtest: IS Sharpe 1.679, Ann +84.5%, WF 3/5. SH p=0.020.
H-012 corr -0.024. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    scores = {}
    window = CONFIG["pvt_window"]
    for sym, daily in daily_dict.items():
        if len(daily) < window + 10:
            continue
        c = daily["close"]
        v = daily["volume"] * daily["close"]
        returns = c.pct_change()
        pvt_daily = returns * v
        pvt = pvt_daily.iloc[-window:].sum()
        if np.isfinite(pvt):
            scores[sym] = pvt
...
def run():
    print("=== H-777 Price-Volume Trend Paper Trade Runner ===")
