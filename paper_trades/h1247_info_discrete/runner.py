"""
H-1247 Information Discreteness XS Paper Trade Runner

Fraction of days where daily return sign matches 60d cumulative return sign. Continuous info flow.
Backtest: IS Sharpe 1.668, WF 3/4, SH 2.25/0.92, p=0.021, H-012 corr -0.021.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Information Discreteness XS."""
    returns = closes_df.pct_change()
    ret60 = closes_df.pct_change(60)
    ...
    for col in closes_df.columns:
        ...
        for i in range(60, len(r)):
            ...
            if cum_sign == 0: continue
            rv = r[i-60:i]
            mask = np.isfinite(rv) & (rv != 0)
            if mask.sum() < 30: continue
            out[i] = np.sum(np.sign(rv[mask]) == cum_sign) / mask.sum()
        result[col] = out
    return result
...
def run():
    print("=== H-1247 Information Discreteness XS Paper Trade Runner ===")
