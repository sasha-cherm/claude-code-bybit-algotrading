"""
H-1334 Drawdown Recovery Speed XS Paper Trade Runner

Avg bars to recover from intraperiod dips in 30d. Fast recovery = strong buying pressure.
Backtest: IS Sharpe 1.335, WF 3/4, SH 1.75/0.82, p=0.065, H-012 corr -0.012.
Params: R=5, N=3, dir=high_long (long slow recovery = contrarian oversold).
"""
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """H-1334 Drawdown Recovery Speed — avg recovery bars in 30d."""
    ...
    for col in closes_df.columns:
        ...
        for i in range(window, len(s)):
            ...
            in_dd = dd < -0.001
            if not np.any(in_dd):
                result.at[result.index[i], col] = window
                continue
            recovery_times = []
            start = None
            ...
            result.at[result.index[i], col] = np.mean(recovery_times) if recovery_times else window
    return result
...
def run():
    print("=== H-1334 Drawdown Recovery Speed XS Paper Trade Runner ===")
