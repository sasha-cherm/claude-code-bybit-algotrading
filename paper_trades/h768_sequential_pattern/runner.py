"""
H-768 Paper Trade Runner: Sequential Pattern Score XS

Long assets with persistent positive 3-day patterns, short negative patterns.

Backtest: IS Sharpe 1.667, Ann +74.0%, WF 3/4. SH p=0.022.
H-012 corr -0.018. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    ...
    for sym, daily in daily_dict.items():
        ...
        pattern_scores = []
        for i in range(max(3, len(returns) - window), len(returns)):
            d1 = 1 if returns.iloc[i - 2] > 0 else -1
            d2 = 1 if returns.iloc[i - 1] > 0 else -1
            d3 = 1 if returns.iloc[i] > 0 else -1
            pattern_scores.append(d1 + d2 + d3)
        if pattern_scores:
            scores[sym] = np.mean(pattern_scores[-window:])
...
def run():
    print("=== H-768 Sequential Pattern Score Paper Trade Runner ===")
