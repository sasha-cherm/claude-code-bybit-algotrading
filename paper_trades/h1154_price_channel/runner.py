"""
H-1154 Price Channel Position XS Paper Trade Runner

Long assets near top of 20d Donchian channel, short those near bottom.
Backtest: IS Sharpe 1.324, WF 3/4, SH 1.26/1.43, p=0.067, H-012 corr 0.010.
...
def compute_signal(closes_df, volumes_df):
    """Price Channel Position: (close - 20d low) / (20d high - 20d low)."""
    highs_approx = closes_df.rolling(1).max()  # approximation from close
    lows_approx = closes_df.rolling(1).min()
    # Use closes as proxy for high/low in live runner
    high20 = closes_df.rolling(20).max()
    low20 = closes_df.rolling(20).min()
    return (closes_df - low20) / (high20 - low20).clip(lower=1e-10)
...
def run():
    print("=== H-1154 Price Channel Position XS Paper Trade Runner ===")
