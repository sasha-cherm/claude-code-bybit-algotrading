"""
H-1046 Relative Volume Duration XS Paper Trade Runner

Consecutive days with short-term avg volume > long-term. Sustained high RVOL = institutional interest.
Backtest: IS Sharpe 2.000, WF 5/5, SH p=0.006, H-012 corr 0.015.
Best params: P=25, R=5, N=4, dir=high_long.
"""
...
def compute_rankings(daily_dict):
    ...
    rvol = volumes.rolling(5).mean() / volumes.rolling(20).mean()
    rvol = rvol.replace([np.inf, -np.inf], np.nan)
    signal = pd.Series(0.0, index=closes.columns)
    for col in closes.columns:
        if col not in rvol.columns:
            continue
        streak = 0
        for j in range(min(30, len(rvol) - 1)):
            val = rvol[col].iloc[-(j+1)]
...
def run():
    print("=== H-1046 Relative Volume Duration XS Paper Trade Runner ===")
