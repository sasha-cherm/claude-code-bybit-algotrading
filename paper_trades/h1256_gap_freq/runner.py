"""
H-1256 Gap Frequency XS Paper Trade Runner

abs(open-prev_close)/prev_close mean 20d. Assets with more overnight gaps → outperform.
Backtest: IS Sharpe 1.905, IS 100%, WF 4/4 PERFECT, SH 2.65/1.11, p=0.008, H-012 corr -0.016. SESSION BEST.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Gap Frequency XS."""
    prev_close = closes_df.shift(1)
    gap = (opens_df - prev_close).abs() / (prev_close + 1e-10)
...
def run():
    print("=== H-1256 Gap Frequency XS Paper Trade Runner ===")
