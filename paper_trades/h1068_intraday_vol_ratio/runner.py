"""
H-1068 Intraday Volatility Ratio (Day/Night) XS Paper Trade Runner

Assets with higher day-session volatility relative to night → long (institutional activity).
Backtest: IS Sharpe 1.330, WF 4/5, SH p=0.063, H-012 corr 0.006.
Best params: R=7, N=4, dir=high_long.
"""
...
def compute_intraday_vol_ratio(hourly_dict):
    """Compute day/night vol ratio for each asset from hourly data."""
    signals = {}
    for sym, df in hourly_dict.items():
        ...
        for date, group in df.groupby("date"):
            ...
            if night_vol and night_vol > 0 and np.isfinite(day_vol):
                daily_ratios[pd.Timestamp(date)] = day_vol / night_vol
        if daily_ratios:
            signals[sym] = pd.Series(daily_ratios)
    if not signals:
        return pd.DataFrame()
    df = pd.DataFrame(signals).sort_index()
    return df.rolling(CONFIG["avg_period"]).mean()
...
def run():
    print("=== H-1068 Intraday Vol Ratio XS Paper Trade Runner ===")
    ...
    if len(hourly_dict) < 8:
        save_state(state)
        return state
    signal_df = compute_intraday_vol_ratio(hourly_dict)
