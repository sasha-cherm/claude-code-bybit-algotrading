"""
H-761 Paper Trade Runner: Gap Signal XS

Open-to-previous-close gap effect. 5-day average gap as ranking signal.
Long assets with persistent positive gaps, short negative gap assets.

Backtest: IS Sharpe 1.673, Ann +76.9%, WF 5/5. SH p=0.019.
H-012 corr 0.054. 100% param robust.
Note: rebal_freq=1 (daily) in backtest, using 3 here for practical turnover control.
"""
...
def compute_rankings(daily_dict):
    """Rank by 5-day average gap (open - prev close) / prev close."""
    scores = {}
    window = CONFIG["gap_window"]
    for sym, daily in daily_dict.items():
        if len(daily) < window + 5:
            continue
        gap = (daily["open"] - daily["close"].shift(1)) / daily["close"].shift(1)
        avg_gap = gap.rolling(window).mean()
        last_val = avg_gap.iloc[-2] if len(avg_gap) > 1 else np.nan
...
def run():
    print("=== H-761 Gap Signal Paper Trade Runner ===")
