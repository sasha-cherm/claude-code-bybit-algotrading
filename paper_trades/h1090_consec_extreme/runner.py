"""
H-1090 Consecutive Extreme Frequency XS Paper Trade Runner

Assets with frequent back-to-back extreme moves (|ret| > 1.5σ) → long.
Captures vol clustering that leads to trends.
Backtest: IS Sharpe 2.397, IS 100% (8/8), WF 3/4, SH p=0.001, H-012 corr -0.023.
Params: R=7, N=3, dir=high_long.
"""
...
def compute_signal(closes_df):
    """Frequency of consecutive extreme days (|ret| > 1.5σ) over 30d window."""
    returns = closes_df.pct_change()
    consec_extreme = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
...
def run():
    print("=== H-1090 Consecutive Extreme Frequency XS Paper Trade Runner ===")
