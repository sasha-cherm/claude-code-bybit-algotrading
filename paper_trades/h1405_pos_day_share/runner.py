"""
H-1405 Positive Day Share XS Paper Trade Runner

Sum of positive returns / sum of absolute returns over 15 days.
High = upside dominates downside = positive momentum quality. Long high.
Backtest: IS Sharpe 1.347, WF 4/4 PERFECT, SH 1.56/1.46, p=0.028, H-012 corr 0.463.
Params: R=5, N=3, lb=15, dir=high_long.
"""
...
CONFIG = {"rebal_freq": 5, "n_long": 3, "n_short": 3, "lb": 15, "initial_capital": 10_000.0,
...
def compute_signal(closes_df):
    """Positive Day Share: sum(max(r,0)) / sum(|r|) over lb days."""
    rets = closes_df.pct_change()
    pos_sum = rets.clip(lower=0).rolling(CONFIG["lb"]).sum()
    abs_sum = rets.abs().rolling(CONFIG["lb"]).sum()
    return pos_sum / (abs_sum + 1e-10)
...
def run():
    print("=== H-1405 Positive Day Share XS Paper Trade Runner ===")
