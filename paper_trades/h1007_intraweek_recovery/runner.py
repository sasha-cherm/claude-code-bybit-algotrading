"""
H-1007 Intraweek Recovery XS Paper Trade Runner

Average (close - low) / (high - low) over 20-day lookback.
Assets consistently closing near highs = strong demand.
Backtest: IS Sharpe 1.448, WF 5/5, SH p=0.045, H-012 corr 0.002.
...
def compute_rankings(daily_dict):
    ...
    recovery = (closes - lows) / daily_range
    signal = recovery.rolling(lb).mean().iloc[-1].dropna()
...
def run():
    print("=== H-1007 Intraweek Recovery XS Paper Trade Runner ===")
