"""
H-1102 Kyle Lambda (Market Depth) XS Paper Trade Runner

Long assets with lowest Kyle lambda (deepest markets), short highest.
Kyle lambda = regression slope of |return| on sqrt(dollar_volume).
Backtest: IS Sharpe 1.218, WF 3/3, SH 0.39/2.08, p=0.099, H-012 corr 0.295.
...
def compute_signal(closes_df, volumes_df):
    """Kyle lambda: rolling regression of |return| on sqrt(dollar_volume)."""
    returns = closes_df.pct_change()
    dollar_vol = volumes_df.clip(lower=1)
    kyle = pd.DataFrame(index=closes_df.index, columns=closes_df.columns, dtype=float)
    for col in closes_df.columns:
        for i in range(30, len(closes_df)):
            ...
            kyle.loc[closes_df.index[i], col] = slope
    return kyle
...
def compute_rankings(signal_df):
    ...
    ranked = sig_row.sort_values(ascending=True)  # low_long: long lowest lambda
    weights = {}
...
def run():
    print("=== H-1102 Kyle Lambda XS Paper Trade Runner ===")
