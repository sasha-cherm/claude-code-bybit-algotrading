"""
H-882 Ease of Movement XS Paper Trade Runner

EMV = midpoint change / box ratio — high EMV = price moves easily.

Backtest: IS Sharpe 1.383, WF 3/5, SH p=0.053, H-012 corr -0.009.
Best params: W=10, R=7, N=4.
"""
...
def compute_rankings(daily_dict):
    ...
    box_ratio = volumes / hl / 1e6
    emv = dm / box_ratio.replace(0, np.nan)
    lb = CONFIG["lookback"]
    signal = emv.rolling(lb).mean()
...
def run():
    print("=== H-882 Ease of Movement XS Paper Trade Runner ===")
