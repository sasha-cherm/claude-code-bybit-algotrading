"""
H-843 Paper Trade Runner: Intraday Range-Vol Ratio XS

Signal = (-range_body_zscore + volume_zscore).rolling(window).mean()
Low range/body at high volume = strong directional moves.

Backtest: IS Sharpe 2.038, WF 5/5 PERFECT, SH p=0.005.
H-012 corr -0.015. 100% param robust.
Best params: W=30, R=7, N=4.
"""
...
def compute_rankings(daily_dict):
    """Rank by intraday range-vol ratio."""
    lb = CONFIG["lookback"]
    ...
    # Long: low range-body ratio (strong directional) + high volume
    raw_signal = (-rb_z + vol_z).rolling(lb).mean()
    signal = raw_signal.iloc[-1].dropna()
...
def run():
    print("=== H-843 Range-Vol Ratio Paper Trade Runner ===")
