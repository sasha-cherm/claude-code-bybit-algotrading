"""
H-1379 Momentum Lifecycle XS Paper Trade Runner

Days since the 20d rolling-mean return last flipped sign (lifecycle age of current trend).
High = trend has been in force longer. Long these.
Backtest: IS Sharpe 1.651, WF 4/4 PERFECT, SH 1.84/1.45, p=0.022, H-012 corr -0.035.
Params: R=7, N=4, dir=high_long. SESSION STAR.
"""
...
CONFIG = {"rebal_freq": 7, "n_long": 4, "n_short": 4, "initial_capital": 10_000.0,
...
def days_since_flip(series, window=20, max_look=60):
    ...
    signs = np.sign(mean.values)
    n = len(signs)
    result = np.full(n, np.nan)
    # Run-length: count backward consecutive same-sign days from each position
    run = 0
    prev_sign = None
    # forward pass: for each i, days_since_flip = (#consecutive same sign ending at i) - 1
    for i in range(n):
        s = signs[i]
        if np.isnan(s):
            run = 0
            prev_sign = None
            continue
        if prev_sign is None or s != prev_sign:
            run = 1
        else:
            run += 1
        result[i] = min(run - 1, max_look)
        prev_sign = s
    return pd.Series(result, index=series.index)


def compute_signal(closes_df):
    """Momentum Lifecycle: days since last sign flip of 20d mean."""
    returns = closes_df.pct_change()
    return returns.apply(lambda col: days_since_flip(col, 20, 60))
...
def run():
    print("=== H-1379 Momentum Lifecycle XS Paper Trade Runner ===")
    ...
    if latest_date == state.get("last_daily_date"):
        ...
        signal_df = compute_signal(closes)
