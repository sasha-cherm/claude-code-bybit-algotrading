"""
H-759 Paper Trade Runner: ADX Trend Strength XS

Long assets with strong trends (high ADX), short weak-trending assets.

Backtest: IS Sharpe 1.723, Ann +75.6%, WF 5/5. SH p=0.016.
H-012 corr 0.064. 100% param robust.
"""
...
def compute_rankings(daily_dict):
    """Rank by ADX trend strength."""
    scores = {}
    window = CONFIG["adx_window"]
    for sym, daily in daily_dict.items():
        if len(daily) < window * 3 + 5:
            continue
        h = daily["high"]
        l = daily["low"]
        c = daily["close"]
        adx = compute_adx(h, l, c, window)
        last_adx = adx.iloc[-2] if len(adx) > 1 else np.nan
...
def run():
    print("=== H-759 ADX Trend Strength Paper Trade Runner ===")
