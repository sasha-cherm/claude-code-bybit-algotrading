"""
H-851 Paper Trade Runner: Drawdown Mean Reversion XS

Z-score of current drawdown vs historical DD distribution.
Extreme negative z-score = unusually deep drawdown → reversion expected.

Backtest: IS Sharpe 1.637, WF 3/4, SH p=0.027.
H-012 corr -0.012. 100% param robust.
Best params: W=60, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    """Rank by drawdown mean reversion z-score."""
    lb = CONFIG["lookback"]
    zw = CONFIG["zscore_window"]
    closes = pd.DataFrame({s: d["close"] for s, d in daily_dict.items()}).dropna(how="all").ffill().dropna()

    if len(closes) < lb + zw + 10:
        return {}

    rolling_high = closes.rolling(lb).max()
    dd = (closes - rolling_high) / rolling_high.replace(0, np.nan)
    dd_mean = dd.rolling(zw).mean()
    dd_std = dd.rolling(zw).std().replace(0, np.nan)
    z = (dd - dd_mean) / dd_std

    signal = z.iloc[-1].dropna()

    n_required = CONFIG["n_long"] + CONFIG["n_short"]
    if len(signal) < n_required:
        return {}

    # Most negative z-score = deepest relative DD → buy (contrarian)
    ranked = signal.sort_values(ascending=True)  # Most negative first = LONG
...
def run():
    print("=== H-851 DD Mean Reversion Paper Trade Runner ===")
    ...
    for sym in ASSETS:
        try:
            df = fetch_and_cache(sym, "1h", limit_days=180)
            if len(df) < 200:
                continue
