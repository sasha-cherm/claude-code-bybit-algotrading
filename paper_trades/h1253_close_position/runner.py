"""
H-1253 Close Position XS Paper Trade Runner

(close-low)/(high-low) averaged 20d. Where in the range does it close? High = bullish.
Backtest: IS Sharpe 1.362, WF 4/4 PERFECT, SH 0.91/1.93, p=0.059, H-012 corr -0.013.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    """Close Position XS."""
    denom = (highs_df - lows_df).replace(0, np.nan)
    close_pos = (closes_df - lows_df) / denom
    return close_pos.rolling(20).mean()
...
def run():
    print("=== H-1253 Close Position XS Paper Trade Runner ===")
