"""
H-382 Paper Trade Runner: Return Kurtosis Factor

Market-neutral strategy: rank 14 crypto assets by rolling excess kurtosis
of daily returns. Low kurtosis (thin tails, more predictable) → long.
High kurtosis (fat tails, more crash-prone) → short.

Backtest: IS 87.5% positive (21/24). Best LB30_R5_N4 Sharpe 1.124
(+40.1% ann, -27.0% DD). WF 6/6 mean 1.500. Split-half H1=0.551, H2=1.850.
Corr H-012 -0.152 — negatively correlated with momentum, excellent diversifier.
"""
...
def compute_rankings(closes: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by excess kurtosis of returns.
    Low kurtosis → long (thin tails, more predictable).
    High kurtosis → short (fat tails, crash-prone).
    """
    ...
    for sym in closes.columns:
        r = rets[sym].iloc[max(0, date_idx - lookback):date_idx + 1].dropna()
        if len(r) < 10:
            continue
        kurt = stats.kurtosis(r.values, fisher=True)
        if np.isfinite(kurt):
            scores[sym] = -kurt  # negate: low kurtosis = high signal
...
def run():
    print("=== H-382 Return Kurtosis Paper Trade Runner ===")
