"""
H-894 Volume-Price Correlation XS Paper Trade Runner

Rolling correlation between returns and volume.
Positive corr = volume confirms price moves.

Backtest: IS Sharpe 1.314, WF 4/5, SH p=0.067, H-012 corr 0.048.
...
def compute_rankings(daily_dict):
    ...
    for col in returns.columns:
        if col not in volumes.columns:
            continue
        r = returns[col].iloc[-lb:].values
        v = volumes[col].iloc[-lb:].values
        if np.all(np.isfinite(r)) and np.all(np.isfinite(v)) and np.std(r) > 0 and np.std(v) > 0:
            sig_row[col] = np.corrcoef(r, v)[0, 1]
    if len(sig_row) < CONFIG["n_long"] + CONFIG["n_short"]:
        return {}
    sig_series = pd.Series(sig_row).sort_values(ascending=False)
    longs = sig_series.index[:CONFIG["n_long"]]
    shorts = sig_series.index[-CONFIG["n_short"]:]
    weights = {}
    for sym in longs:
...
def run():
    print("=== H-894 Vol-Price Correlation XS Paper Trade Runner ===")
