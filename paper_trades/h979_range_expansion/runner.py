"""
H-979 Consecutive Range Expansion XS Paper Trade Runner

Count of days where range > prior day range. More expansion = increasing activity/interest.
Backtest: IS Sharpe 2.204, WF 5/5, SH p=0.002, H-012 corr 0.004.
Best params: P=14, R=5, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    for col in closes.columns:
        if col not in highs_df.columns or col not in lows_df.columns:
            continue
        daily_range = (highs_df[col] - lows_df[col]).values
        if len(daily_range) < lb:
            continue
        r = daily_range[-lb:]
...
def run():
    print("=== H-979 Consecutive Range Expansion XS Paper Trade Runner ===")
