"""
H-1275 Range Position XS Paper Trade Runner

(close - low_20d) / (high_20d - low_20d). Price position in recent range.
Backtest: IS Sharpe 1.122, WF 3/4, SH 0.62/1.78, p=0.120, H-012 corr -0.005.
...
def compute_signal(closes_df, volumes_df, opens_df=None, highs_df=None, lows_df=None):
    ...
    import numpy as np
    rolling_high = closes_df.rolling(20).max()
    rolling_low = closes_df.rolling(20).min()
    range_denom = (rolling_high - rolling_low).replace(0, np.nan)
    return (closes_df - rolling_low) / range_denom
...
def run():
    print("=== H-1275 Range Position XS Paper Trade Runner ===")
