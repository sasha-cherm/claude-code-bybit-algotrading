"""
H-861 Downside Protection XS Paper Trade Runner

Avg return on BTC-down days — long assets that lose least when BTC drops.

Backtest: IS Sharpe 1.255, WF 3/4, SH p=0.085, H-012 corr -0.021.
Best params: W=40, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    btc_col = [c for c in returns.columns if "BTC" in c]
    if not btc_col:
        return {}
    btc_ret = returns[btc_col[0]]
    btc_down = btc_ret < 0
    signal = pd.DataFrame(np.nan, index=returns.index, columns=returns.columns)
    lb = CONFIG["lookback"]
    for i in range(lb + 5, len(returns)):
        mask = btc_down.iloc[i - lb:i]
        for col in returns.columns:
            dn_rets = returns[col].iloc[i - lb:i][mask]
            if len(dn_rets) > 5:
...
def run():
    print("=== H-861 Downside Protection XS Paper Trade Runner ===")
