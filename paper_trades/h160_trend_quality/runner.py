"""
H-160 Paper Trade Runner: Trend-Quality Factor (Efficiency × Inverse Vol, 14 Crypto Assets)

Market-neutral strategy: rank 14 assets by trend quality signal =
  efficiency(eff_lb) × inverse_vol(vol_lb) × sign(momentum).
Efficiency = |net_return| / sum(|daily_returns|) over lookback.
Captures smooth, low-volatility directional moves.
Long top 3, short bottom 3. Rebalance every 3 days.

Backtest: IS 87% positive, WF 4/6 mean OOS 0.303, split-half H1=1.174/H2=1.764.
Correlation: H-012 0.355, H-076 0.117 — genuinely novel.
"""
...
CONFIG = {
    "eff_lookback": 20,      # 20-day efficiency window
    "vol_lookback": 20,      # 20-day volatility window
    "rebal_freq": 3,         # rebalance every 3 days
    "n_long": 3,             # long top 3 (highest trend quality)
    "n_short": 3,            # short bottom 3 (lowest trend quality)
...
def compute_rankings(closes: pd.DataFrame, date_idx: int) -> dict:
    """
    Rank assets by trend quality = efficiency × inverse_vol × sign(momentum).
    Uses data through day (date_idx - 1) for lagged signals.
    """
    eff_lb = CONFIG["eff_lookback"]
    ...
    eff_start = lagged_end - eff_lb
    vol_start = lagged_end - vol_lb

    signals = {}
    for sym in closes.columns:
        eff_window = closes[sym].iloc[eff_start:lagged_end]
        vol_window = closes[sym].iloc[vol_start:lagged_end]

        if len(eff_window) < eff_lb or len(vol_window) < vol_lb:
            continue

        # Efficiency = |net_return| / sum(|daily_returns|)
        daily_rets = eff_window.pct_change().dropna()
        if len(daily_rets) == 0:
            continue

        net_return = eff_window.iloc[-1] / eff_window.iloc[0] - 1
        sum_abs_rets = daily_rets.abs().sum()

        if sum_abs_rets < 1e-8:
            continue

        efficiency = abs(net_return) / sum_abs_rets

        # Inverse volatility
        vol_rets = vol_window.pct_change().dropna()
        realized_vol = vol_rets.std()
        if realized_vol < 1e-8:
            continue
        inv_vol = 1.0 / realized_vol

        # Direction from momentum sign
        direction = 1.0 if net_return > 0 else -1.0

        signals[sym] = efficiency * inv_vol * direction
...
# ── Main runner ──────────────────────────────────────────────────────
def run():
    print("=== H-160 Trend-Quality Paper Trade Runner ===")
    ...
    warmup = max(CONFIG["eff_lookback"], CONFIG["vol_lookback"]) + 10
