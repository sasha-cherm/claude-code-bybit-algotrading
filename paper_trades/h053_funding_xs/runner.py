"""
H-053 Paper Trade Runner: Funding Rate Cross-Sectional Factor (14 Crypto Assets)

Market-neutral strategy: rank 14 assets by rolling 3-day average funding rate.
Contrarian: long bottom 4 (lowest funding — shorts paying longs, weak sentiment),
short top 4 (highest funding — crowded longs).
Rebalance every 10 days.

Backtest: IS Sharpe 1.52, +32.9% ann, 22.2% DD. WF 6/6 positive (mean OOS 2.29).
Fee-robust (0.92 at 5x fees). Split-half: 1.31/1.91.
Correlation: 0.004 H-012, 0.109 H-046, 0.360 H-052, 0.480 H-049.
"""
...
ASSET_TICKERS = {a: a.replace("/", "_").replace("USDT", "USDT_USDT") for a in ASSETS}
# e.g. "BTC/USDT" -> "BTC_USDT_USDT"

CONFIG = {
    "funding_window": 3,     # rolling avg window in days
    "rebal_freq": 10,        # rebalance every 10 days
    "n_long": 4,             # long bottom 4 (lowest funding — contrarian)
    "n_short": 4,            # short top 4 (highest funding)
...
def load_funding_data() -> pd.DataFrame:
    """Load 8h funding rate data for all assets, aggregate to daily average."""
    data_dir = ROOT / "data"
    funding_closes = {}

    for sym in ASSETS:
        asset_name = sym.split("/")[0]  # "BTC/USDT" -> "BTC"
        ...
        try:
            ...
            funding_closes[sym] = daily
        except Exception as e:
            print(f"  {sym}: funding load failed: {e}")
        time.sleep(0.02)

    funding = pd.DataFrame(funding_closes)
    funding = funding.dropna(how="all").ffill().dropna()
    return funding


# ── Signal generation ────────────────────────────────────────────────
def compute_funding_signal(funding: pd.DataFrame) -> dict:
    """
    Rank assets by rolling average funding rate.
    Contrarian: long lowest funding (most shorts-paying), short highest (crowded longs).
    """
    ...
    # Rolling average funding rate (lagged — use latest completed bars)
    avg_funding = funding.iloc[-window:].mean()
    valid = avg_funding.dropna()
    ...
    # Contrarian: long lowest funding (bottom N), short highest (top N)
...
# ── Main runner ──────────────────────────────────────────────────────
def run():
    print("=== H-053 Funding Rate XS Paper Trade Runner ===")
    ...
    print(f"Funding data: {len(funding)} days, {len(funding.columns)} assets")

    if len(funding) < CONFIG["funding_window"] + 2:
    ...
    if days_since >= CONFIG["rebal_freq"]:
        ...
        new_weights = compute_funding_signal(funding)
        if not new_weights:
            print("  Could not compute funding signal. Skipping rebalance.")
        else:
