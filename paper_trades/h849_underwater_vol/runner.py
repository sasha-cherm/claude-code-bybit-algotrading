"""
H-849 Paper Trade Runner: Underwater Volatility XS

Volatility during drawdown periods vs overall vol.
High underwater vol = risky. Long low-underwater-vol assets.

Backtest: IS Sharpe 1.463, WF 4/5, SH p=0.043.
H-012 corr 0.001. 100% param robust.
Best params: W=30, R=5, N=4.
"""
...
def compute_rankings(daily_dict):
    """Rank by underwater volatility (inverse — long calm-DD assets)."""
    lb = CONFIG["lookback"]
    ...
    for col in closes.columns:
        r = ret[col].iloc[-lb:]
        dd_mask = in_dd[col].iloc[-lb:]
        r_dd = r[dd_mask]
        r_all = r
        if len(r_dd) > 5 and r_all.std() > 0:
            signal[col] = -(r_dd.std() / r_all.std())  # Negative = long low underwater vol
        else:
            signal[col] = 0.0  # Not in DD = safe
...
def run():
    print("=== H-849 Underwater Volatility Paper Trade Runner ===")
