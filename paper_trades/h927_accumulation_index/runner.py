"""
H-927 Accumulation Index XS Paper Trade Runner

Rolling sum of CLV * volume, normalized.
CLV = (close-open)/(high-low). Measures buying vs selling pressure.

Backtest: IS Sharpe 1.557, WF 4/5, SH p=0.031, H-012 corr 0.081.
Best params: P=20, R=3, N=3.
"""
...
def compute_rankings(daily_dict):
    ...
    ad = clv * volumes
    accum = ad.rolling(lb).sum() / volumes.rolling(lb).sum().replace(0, np.nan)
    sig_row = accum.iloc[-1]
...
def run():
    print("=== H-927 Accumulation Index XS Paper Trade Runner ===")
