#!/usr/bin/env python3
"""
H-187: Rolling Sharpe Ratio as Cross-Sectional Factor

Concept: For each asset, compute a rolling Sharpe ratio:
  mean(daily_return[-LB:]) / std(daily_return[-LB:])
This captures risk-adjusted momentum -- assets with high rolling Sharpe have
strong recent returns relative to their risk.

Different from:
  - H-012 (raw momentum = cumulative return only, no risk adjustment)
  - H-019 (volatility level only, no return component)
Because it combines both return AND risk into a ratio.

Logic:
  For each asset on each rebalance day:
    1. Compute rolling Sharpe: mean(ret[-LB:]) / std(ret[-LB:])
    2. If std < 1e-10, set rolling Sharpe to 0
    3. Rank assets cross-sectionally by rolling Sharpe
    4. Two directions:
       - high_sharpe_long: Long top-N (highest rolling Sharpe),
                           Short bottom-N (lowest rolling Sharpe)
       - low_sharpe_long: opposite
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [high_sharpe_long, low_sharpe_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["high_sharpe_long", "low_sharpe_long"]
...
def backtest_rolling_sharpe(closes, lookback, rebal_freq, n, direction):
    """
    Rolling Sharpe Ratio cross-sectional factor backtest.

    Args:
        closes:     DataFrame of daily closes
        lookback:   rolling window for Sharpe computation
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "high_sharpe_long" or "low_sharpe_long"
    Returns:
        ...
        if i - last_rebal >= rebal_freq:
            ...
            for sym in trade_cols:
                ...
                if np.any(np.isnan(ret_window)):
                    continue
                ret_std = np.std(ret_window, ddof=1)
                if ret_std < 1e-10:
                    sharpe_scores[sym] = 0.0
                else:
                    ret_mean = np.mean(ret_window)
                    sharpe_scores[sym] = ret_mean / ret_std

            if len(sharpe_scores) < 2 * n:
                continue

            ranked = pd.Series(sharpe_scores).sort_values(ascending=True)
            # ranked: ascending by rolling Sharpe
            # Top = highest rolling Sharpe (best risk-adjusted momentum)
            # Bottom = lowest rolling Sharpe (worst risk-adjusted momentum)

            if direction == "high_sharpe_long":
                # Long high Sharpe (top), Short low Sharpe (bottom)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
            else:
                # Long low Sharpe (bottom), Short high Sharpe (top)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
...
def run_walk_forward(closes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_rolling_sharpe(train_closes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_rolling_sharpe(oos_closes, lb, rf, nn, best_direction)
...
def run_split_half(closes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_rolling_sharpe(h1_closes, lb, rf, nn, best_direction)
                r2 = backtest_rolling_sharpe(h2_closes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-187: Rolling Sharpe Ratio as Cross-Sectional Factor")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_rolling_sharpe(closes, lb, rf, nn, d)
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-187",
            "name": "Rolling Sharpe Ratio as Cross-Sectional Factor",
            "status": "REJECTED",
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_rolling_sharpe(closes, best_lb, best_rf, best_n, best_dir)
