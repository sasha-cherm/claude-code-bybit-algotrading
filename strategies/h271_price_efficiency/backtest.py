...
Idea: Compute efficiency = |price(t) - price(t-LB)| / sum(|daily_returns_over_LB|).
Values near 1 mean the asset is trending (net ≈ gross movement).
Values near 0 mean the asset is choppy (lots of back-and-forth cancelling out).
Long trending assets, short choppy ones (or vice versa).

Different from:
- H-012 (Momentum): momentum doesn't normalize by gross movement
- H-093 (Trend Consistency): used different consistency metric
- H-157 (Range Ratio): used high-low range, not return-based efficiency
- H-248 (Intraday Efficiency): used hourly data within a day, rejected

Signal construction:
  1. For each asset over lookback window:
     net = |close(t) / close(t-LB) - 1|
     gross = sum(|daily_return(i)| for i in window)
     efficiency = net / gross
  2. Rank XS: long high efficiency (trending), short low (choppy)

Parameter grid:
  Lookback: [10, 15, 20, 30, 60]
  Rebal freq: [3, 5, 7]
  N: [3, 4]
  Direction: [high_eff_long, low_eff_long]
...
REBALS = [3, 5, 7]
NS = [3, 4]
DIRECTIONS = ["high_eff_long", "low_eff_long"]
...
def compute_price_efficiency(closes, lookback):
    """Compute price efficiency ratio: |net movement| / gross movement."""
    rets = closes.pct_change()
    abs_rets = rets.abs()

    # Net movement (absolute value of total return over window)
    net = (closes / closes.shift(lookback) - 1).abs()

    # Gross movement (sum of absolute daily returns over window)
    gross = abs_rets.rolling(lookback, min_periods=lookback).sum()

    # Efficiency ratio
    efficiency = net / gross.replace(0, np.nan)
    return efficiency
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            sig = compute_price_efficiency(tr_c, lb)
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
        ...
        full_sig = compute_price_efficiency(closes.iloc[:oos_end], lb)
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        h1_sig = compute_price_efficiency(h1c, lb)
        h2_sig = compute_price_efficiency(h2c, lb)
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
...
def main():
    print("=" * 60)
    print("  H-271: Price Efficiency Ratio Factor")
    print("=" * 60)
    ...
    # Pre-compute signals
    print("\nPre-computing efficiency signals...")
    sig_cache = {}
    for lb in LOOKBACKS:
        sig_cache[lb] = compute_price_efficiency(closes, lb)
        print(f"  LB={lb} done")
    ...
    # Dominant direction
    dir_counts = {"high_eff_long": 0, "low_eff_long": 0}
    for r in positive:
        for d in DIRECTIONS:
            if d in r["label"]:
                dir_counts[d] += 1
    ...
    rf = int(parts[1][1:])
    nn = int(parts[2][1:])
    d = "high_eff_long" if "high_eff_long" in best["label"] else "low_eff_long"
    sig = sig_cache[lb]
