#!/usr/bin/env python3
"""
H-220: Short-Term Reversal Factor

Idea: Assets with the most negative recent returns (1-5 day) tend to bounce back,
while recent winners mean-revert. Classic market microstructure effect: short-term
overreaction followed by correction.

Long recent losers, short recent winners (contrarian at short horizons).

Differs from H-012 (60-day momentum = intermediate-term trend continuation).
Reversal and momentum operate at different frequencies — reversal at 1-5 days,
momentum at 20-60 days. They're complementary, often negatively correlated.

Signal construction:
  1. Compute trailing return over a short lookback (1-5 days).
  2. Rank XS: long bottom-N (biggest losers), short top-N (biggest winners).
  3. Dollar-neutral, equal-weight.

Parameter grid:
  Lookback  : [1, 2, 3, 5]
  Rebal freq: [1, 2, 3, 5]
  N         : [3, 4]
  Total: 4 x 4 x 2 = 32 combos
"""
...
def backtest_reversal(closes, lookback, rebal_freq, n):
    """Short-term reversal: long biggest losers, short biggest winners."""
    ...
    for i in range(warmup, len(dates)):
        ...
        trail_ret = closes.iloc[i] / closes.iloc[i - lookback] - 1
        valid = trail_ret.dropna()
        if len(valid) < 2 * n:
            continue
        # Reversal: long losers (ascending sort, bottom first), short winners
        ranked = valid.sort_values(ascending=True)
        longs = ranked.index[:n]  # biggest losers
        shorts = ranked.index[-n:]  # biggest winners
...
def backtest_momentum_variant(closes, lookback, rebal_freq, n):
    """Pro-momentum at same short horizon (for comparison: long winners, short losers)."""
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    ...
    for i in range(warmup, len(dates)):
        ...
        if len(valid) < 2 * n:
            continue
        ranked = valid.sort_values(ascending=False)  # winners first
...
def backtest_h012_momentum(closes, lookback=60, rebal_freq=5, n=4):
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    # Test both reversal and short-term momentum
                    r_rev = backtest_reversal(tr_c, lb, rf, nn)
                    ev = evaluate(r_rev)
                    if ev and ev["sharpe"] > best_sharpe:
                        best_sharpe = ev["sharpe"]
                        best_params = (lb, rf, nn, "reversal")
                    r_mom = backtest_momentum_variant(tr_c, lb, rf, nn)
                    ev = evaluate(r_mom)
                    if ev and ev["sharpe"] > best_sharpe:
                        best_sharpe = ev["sharpe"]
                        best_params = (lb, rf, nn, "momentum")
        if best_params is None:
            fold_results.append({"fold": fold + 1, "oos_sharpe": None})
            continue
        lb, rf, nn, d = best_params
        if d == "reversal":
            oos_r = backtest_reversal(oo_c, lb, rf, nn)
        else:
            oos_r = backtest_momentum_variant(oo_c, lb, rf, nn)
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                # Both directions
                for func in [backtest_reversal, backtest_momentum_variant]:
                    e1 = evaluate(func(h1c, lb, rf, nn))
                    e2 = evaluate(func(h2c, lb, rf, nn))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-220: Short-Term Reversal Factor")
    print("=" * 60)
    ...
    total_combos = len(LOOKBACKS) * len(REBALS) * len(NS) * 2  # 2 directions
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for label, func in [("reversal", backtest_reversal),
                                    ("momentum", backtest_momentum_variant)]:
                    count += 1
                    r = func(closes, lb, rf, nn)
                    ev = evaluate(r, f"LB{lb}_R{rf}_N{nn}_{label}")
                    if ev:
                        ev["lb"], ev["rf"], ev["n"], ev["dir"] = lb, rf, nn, label
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-220", "name": "Short-Term Reversal",
    ...
    # Stage 4: Correlation with H-012 (60d momentum)
    print("\n--- Stage 4: Correlation with H-012 ---")
    best_r = None
    if best_row["dir"] == "reversal":
        best_r = backtest_reversal(closes, int(best_row["lb"]), int(best_row["rf"]), int(best_row["n"]))
    else:
        best_r = backtest_momentum_variant(closes, int(best_row["lb"]), int(best_row["rf"]), int(best_row["n"]))
    mom_r = backtest_h012_momentum(closes)
