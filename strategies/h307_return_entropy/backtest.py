...
Concept: Measure the entropy (randomness/unpredictability) of an asset's
daily return distribution. Low entropy means returns cluster around a few
values (predictable pattern). High entropy means returns are more uniformly
distributed (random walk).

Signal: Shannon entropy of binned daily returns over a lookback window.
Long low-entropy assets (they have a pattern → exploit it).
Short high-entropy assets (random → no edge, use as hedge).

This is genuinely novel — no previous hypothesis has used information-theoretic
measures. Entropy captures the information content of price moves, not just
their direction or magnitude.

Implementation: Bin returns into quantiles (5 bins), compute Shannon entropy
of the empirical distribution.

Parameter grid: lookback [10,20,30,40] × n_bins [3,5,7] × R [3,5,7] × N [3,4]
  = 4×3×3×2 = 72 combos
"""
...
def shannon_entropy(values, n_bins):
    """Compute Shannon entropy of a 1D array using histogram bins."""
...
def compute_signal(closes, lookback, n_bins):
    """
    Signal = rolling Shannon entropy of daily returns.
    LOW entropy = predictable → LONG (exploit the pattern).
    We negate so that high signal = low entropy.
    """
    ...
    for asset in ASSETS:
        ...
        for i in range(lookback, len(r)):
            ...
            if len(window) >= n_bins:
                ent[i] = shannon_entropy(window, n_bins)
        signal[asset] = -pd.Series(ent, index=closes.index)  # Negate: low entropy → high signal
...
def main():
    print("=" * 70)
    print("H-307: Return Distribution Entropy Factor")
    print("=" * 70)
    ...
    results = []
    total = len(LOOKBACKS) * len(N_BINS_LIST) * len(REBAL_FREQS) * len(N_POSITIONS)
    print(f"\nRunning {total} parameter combinations...")

    for lb, n_bins, rebal, n_pos in product(LOOKBACKS, N_BINS_LIST, REBAL_FREQS, N_POSITIONS):
        signal = compute_signal(closes, lb, n_bins)
        ...
        label = f"LB{lb}_B{n_bins}_R{rebal}_N{n_pos}"
        results.append({
            "label": label, "lookback": lb, "n_bins": n_bins,
    ...
    # Direction
    low_ent_pos = (df["sharpe"] > 0).sum()
    high_ent_pos = (df["sharpe"] < 0).sum()
    print(f"  Low-entropy-long (predictable): {low_ent_pos}/{len(df)} ({low_ent_pos/len(df)*100:.1f}%)")
    print(f"  High-entropy-long (random): {high_ent_pos}/{len(df)} ({high_ent_pos/len(df)*100:.1f}%)")
    ...
    for nb in N_BINS_LIST:
        ...
        mean_sr = sub["sharpe"].mean()
        pos = (sub["sharpe"] > 0).sum()
        print(f"  B{nb}: mean Sharpe {mean_sr:.3f}, {pos}/{len(sub)} positive")

    # Walk-forward
    signal = compute_signal(closes, int(best["lookback"]), int(best["n_bins"]))
    ...
    # Save
    out = {
        "hypothesis": "H-307", "name": "Return Entropy",
