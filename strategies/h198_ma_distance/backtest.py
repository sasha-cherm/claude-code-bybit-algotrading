#!/usr/bin/env python3
"""
H-198: Price-MA Distance (Mean Reversion) Factor

Concept: Compute the distance of each asset's price from its simple moving
average: factor = close / SMA(close, window) - 1. Cross-sectionally rank
14 crypto assets by this distance.

Hypothesis: Assets far above their MA are "overbought" and will underperform,
while assets far below are "oversold" and will outperform (mean reversion).
We also test the opposite direction (momentum continuation).

Different from:
  - H-012 (momentum -- uses raw returns over lookback, not deviation from MA)
  - H-104 (RSI mean reversion -- uses RSI oscillator, not price/MA distance)
  - H-109 (short-term reversal -- uses 1-5d raw returns, not MA distance)
  - H-176 (momentum/reversal hybrid -- composite signal, not pure MA distance)

Logic:
  For each asset on each rebalance day:
    1. Compute close_t / SMA(close, window)_t - 1
    2. Rank assets cross-sectionally by this distance
    3. Two directions:
       - below_ma_long: Long bottom-N (furthest below MA = oversold),
                        Short top-N (furthest above MA = overbought)
                        [mean reversion]
       - above_ma_long: Long top-N (furthest above MA = trending up),
                        Short bottom-N (furthest below MA = trending down)
...
Parameter grid:
  MA window (lookback): [5, 10, 20, 30, 60]
  Rebalance freq:       [3, 5, 7]
  N (top/bottom):       [3, 4]
  Direction:            [below_ma_long, above_ma_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["below_ma_long", "above_ma_long"]
...
def backtest_ma_distance(closes, ma_window, rebal_freq, n, direction):
    """
    Price-MA Distance Factor backtest.

    Args:
        closes:     DataFrame of daily closes
        ma_window:  SMA window for computing distance
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "below_ma_long" (mean reversion) or "above_ma_long" (momentum)
    Returns:
    ...
    # Pre-compute SMA for all assets
    sma = closes.rolling(window=ma_window, min_periods=ma_window).mean()
    # Factor: close / SMA - 1 (positive = above MA, negative = below MA)
    ma_distance = (closes / sma) - 1
    ma_distance = ma_distance.reindex(returns.index)
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Get current MA distance scores for each asset
            scores = {}
            for sym in trade_cols:
                if sym not in ma_distance.columns:
                    continue
                val = ma_distance[sym].iloc[i - 1]  # Use prior day's signal
                if np.isnan(val):
                    continue
                scores[sym] = val
            ...
            # ranked: ascending by MA distance
            # Bottom = most below MA (oversold)
            # Top = most above MA (overbought)

            if direction == "below_ma_long":
                # Mean reversion: Long oversold (below MA), Short overbought (above MA)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # Momentum: Long overbought (above MA), Short oversold (below MA)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def run_walk_forward(closes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for mw in MA_WINDOWS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_ma_distance(train_closes, mw, rf, nn, best_direction)
                    ev = evaluate(r)
                    if ev and ev["sharpe"] > best_sharpe:
                        best_sharpe = ev["sharpe"]
                        best_params = (mw, rf, nn)
        ...
        mw, rf, nn = best_params
        # OOS evaluation
        oos_r = backtest_ma_distance(oos_closes, mw, rf, nn, best_direction)
...
def run_split_half(closes, best_direction):
    ...
    for mw in MA_WINDOWS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_ma_distance(h1_closes, mw, rf, nn, best_direction)
                r2 = backtest_ma_distance(h2_closes, mw, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-198: Price-MA Distance (Mean Reversion) Factor")
    print("=" * 60)
    ...
    all_results = []
    count = 0
    for mw, rf, nn, d in valid_grid:
        count += 1
        r = backtest_ma_distance(closes, mw, rf, nn, d)
        ev = evaluate(r, f"MA{mw}_R{rf}_N{nn}_{d}")
        if ev:
            ev["mw"] = mw
            ev["rf"] = rf
            ev["n"] = nn
    ...
    for _, row in df_best.head(10).iterrows():
        print(f"    MA{int(row['mw'])}_R{int(row['rf'])}_N{int(row['n'])}: "
    ...
    print(f"\n  Best combo: MA{best_mw}_R{best_rf}_N{best_n} ({best_dir})")
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-198",
            "name": "Price-MA Distance (Mean Reversion) Factor",
            "status": "REJECTED",
            ...
            "best_params": {"ma_window": best_mw, "rebal": best_rf, "n": best_n, "direction": best_dir},
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_ma_distance(closes, best_mw, best_rf, best_n, best_dir)
