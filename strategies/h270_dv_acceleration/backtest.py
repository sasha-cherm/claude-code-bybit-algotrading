...
Idea: Compute the rate of change of dollar volume momentum (second derivative).
Not just volume level (H-031) or volume momentum (H-021), but whether volume growth
is ACCELERATING or DECELERATING. Assets with accelerating dollar volume are attracting
increasing attention and capital inflows.

Signal construction:
  1. DV = close * volume (dollar volume) per day
  2. DV_momentum = DV_MA(short) / DV_MA(long) - 1 (volume momentum)
  3. DV_acceleration = DV_momentum(t) - DV_momentum(t-lag) (change in momentum)
  4. Rank XS: long high acceleration (volume momentum increasing), short low

Parameter grid:
  Short MA: [3, 5]
  Long MA: [15, 20, 30]
  Acceleration lag: [5, 10, 15]
  Rebal freq: [3, 5, 7]
  N: [3, 4]
  Direction: [high_accel_long, low_accel_long]
  Total: 2 x 3 x 3 x 3 x 2 x 2 = 216 combos
  Note: Skip combos where short >= long. Effective ~216 combos.
"""
...
REBALS = [3, 5, 7]
NS = [3, 4]
DIRECTIONS = ["high_accel_long", "low_accel_long"]
...
def load_data():
    ...
    for sym in ASSETS_ALL:
        ...
        c = df["close"].copy()
        v = df["volume"].copy()
        if c.index.tzinfo is not None:
            c.index = c.index.tz_localize(None)
            v.index = v.index.tz_localize(None)
        closes[sym] = c
        volumes[sym] = v
    closes = pd.DataFrame(closes).dropna(how="all").ffill().dropna()
    volumes = pd.DataFrame(volumes).reindex(closes.index).ffill().dropna()
    return closes, volumes


def compute_dv_acceleration(closes, volumes, short_ma, long_ma, accel_lag):
    """Compute dollar volume acceleration (second derivative of DV)."""
    dv = closes * volumes
    dv_short = dv.rolling(short_ma, min_periods=short_ma).mean()
    dv_long = dv.rolling(long_ma, min_periods=long_ma).mean()
    dv_mom = dv_short / dv_long - 1
    dv_accel = dv_mom - dv_mom.shift(accel_lag)
    return dv_accel
    ...
    for fold in range(WF_FOLDS):
        ...
        for sm in SHORT_MAS:
            for lm in LONG_MAS:
                if sm >= lm:
                    continue
                for al in ACCEL_LAGS:
                    sig = compute_dv_acceleration(tr_c, tr_v, sm, lm, al)
                    for rf in REBALS:
                        for nn in NS:
                            for d in DIRECTIONS:
                                ...
                                if ev and ev["sharpe"] > best_sharpe:
                                    ...
                                    best_params = (sm, lm, al, rf, nn, d)
        if best_params is None:
            fold_results.append({"fold": fold + 1, "oos_sharpe": None})
            continue
        sm, lm, al, rf, nn, d = best_params
        full_sig = compute_dv_acceleration(closes.iloc[:oos_end], volumes.iloc[:oos_end], sm, lm, al)
        oos_sig = full_sig.iloc[oos_start:oos_end]
        oos_r = backtest_factor(oo_c, oos_sig, rf, nn, d)
        oos_ev = evaluate(oos_r)
        fold_results.append({
            "fold": fold + 1, "is_params": f"S{sm}_L{lm}_AL{al}_R{rf}_N{nn}_{d}",
    ...
    for sm in SHORT_MAS:
        for lm in LONG_MAS:
            if sm >= lm:
                continue
            for al in ACCEL_LAGS:
                h1_sig = compute_dv_acceleration(h1c, h1v, sm, lm, al)
                h2_sig = compute_dv_acceleration(h2c, h2v, sm, lm, al)
                for rf in REBALS:
                    for nn in NS:
                        for d in DIRECTIONS:
...
def main():
    print("=" * 60)
    print("  H-270: Dollar Volume Acceleration Factor")
    print("=" * 60)
    ...
    # Pre-compute signals
    print(f"\nPre-computing DV acceleration signals ({total_combos} combos)...")
    sig_cache = {}
    for sm in SHORT_MAS:
        for lm in LONG_MAS:
            if sm >= lm:
            ...
            for al in ACCEL_LAGS:
                key = (sm, lm, al)
                sig_cache[key] = compute_dv_acceleration(closes, volumes, sm, lm, al)
    print(f"  {len(sig_cache)} signal variants computed")

    # Stage 1: IS Parameter Scan
    print(f"\n--- Stage 1: IS Parameter Scan ({total_combos} combos) ---")
    all_results = []
    count = 0
    for (sm, lm, al), sig in sig_cache.items():
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_factor(closes, sig, rf, nn, d)
                    ev = evaluate(r, f"S{sm}_L{lm}_AL{al}_R{rf}_N{nn}_{d}")
                    if ev:
    ...
    # Dominant direction
    dir_counts = {"high_accel_long": 0, "low_accel_long": 0}
    for r in positive:
        for d in DIRECTIONS:
            if d in r["label"]:
                dir_counts[d] += 1
    ...
    # Parse best label to get params
    ...
    rf = int(parts[3][1:])
    nn = int(parts[4][1:])
    d = "high_accel_long" if "high_accel_long" in best["label"] else "low_accel_long"
    sig = sig_cache[(sm, lm, al)]
