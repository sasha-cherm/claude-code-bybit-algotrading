#!/usr/bin/env python3
"""
H-186: Close Location Value (CLV) Factor

Concept: Close Location Value measures where the close sits within the day's range:
  ...
  - CLV = 0 means close at the midpoint

Compute rolling mean CLV over lookback window. Assets with high CLV = persistent
buying pressure. Two directions:
  - high_clv_long: Long top-N (highest avg CLV = buying pressure),
                   Short bottom-N (selling pressure)
  - low_clv_long: opposite (contrarian -- mean-reversion after buying exhaustion)
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [high_clv_long, low_clv_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["high_clv_long", "low_clv_long"]
...
def load_daily_data():
    """Load 1d parquet files for all assets, return closes, highs, lows DataFrames."""
    data_dir = ROOT / "data"
    closes_dict = {}
    highs_dict = {}
    lows_dict = {}
    for sym in ASSETS_ALL:
        ...
        if path.exists():
            ...
            if all(c in df.columns for c in ["close", "high", "low"]) and len(df) >= 200:
                closes_dict[sym] = df["close"]
                highs_dict[sym] = df["high"]
                lows_dict[sym] = df["low"]
    closes = pd.DataFrame(closes_dict)
    highs = pd.DataFrame(highs_dict)
    lows = pd.DataFrame(lows_dict)
    # Align indices
    ...
    highs = highs.loc[common_idx].reindex(closes.index).ffill().dropna()
    lows = lows.loc[common_idx].reindex(closes.index).ffill().dropna()
...
def backtest_clv_factor(closes, highs, lows, lookback, rebal_freq, n, direction):
    """
    Close Location Value Factor backtest.

    Args:
        closes:     DataFrame of daily closes
        highs:      DataFrame of daily highs (aligned with closes)
        lows:       DataFrame of daily lows (aligned with closes)
        lookback:   rolling window for CLV mean calculation
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "high_clv_long" or "low_clv_long"
    Returns:
    ...
    # Compute daily CLV: (2*close - high - low) / (high - low)
    # Handle edge case where high == low (zero range): set CLV = 0
    hl_range = highs - lows
    raw_clv = (2 * closes - highs - lows) / hl_range
    raw_clv = raw_clv.where(hl_range > 1e-12, 0.0)  # CLV=0 when high==low
    raw_clv = raw_clv.reindex(returns.index)
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute rolling mean CLV for each asset over lookback window
            clv_scores = {}
            for sym in trade_cols:
                if sym not in raw_clv.columns:
                    continue
                clv_window = raw_clv[sym].iloc[i - lookback:i].values
                # Skip if we have NaN
                if np.any(np.isnan(clv_window)):
                    continue
                avg_clv = np.mean(clv_window)
                clv_scores[sym] = avg_clv

            if len(clv_scores) < 2 * n:
                continue

            ranked = pd.Series(clv_scores).sort_values(ascending=False)
            # ranked: descending by avg CLV
            # Top = highest CLV = persistent buying pressure (close near high)
            # Bottom = lowest CLV = persistent selling pressure (close near low)

            if direction == "high_clv_long":
                # Long high CLV (buying pressure), Short low CLV (selling pressure)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # Long low CLV (contrarian), Short high CLV
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def backtest_efficiency(closes, lookback=40, rebal_freq=5, n=4):
    """H-076 price efficiency factor: rank by net_move/sum_abs_moves over 40d, long top-4, short bottom-4, R5."""
    rets = closes.pct_change().dropna()
    dates = rets.index
    portfolio_rets = []
    last_rebal = -rebal_freq
...
def run_walk_forward(closes, highs, lows, best_direction):
    """
    ...
    for fold in range(WF_FOLDS):
        ...
        train_highs = highs.iloc[:oos_start]
        train_lows = lows.iloc[:oos_start]
        oos_closes = closes.iloc[oos_start:oos_end]
        oos_highs = highs.iloc[oos_start:oos_end]
        oos_lows = lows.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_clv_factor(train_closes, train_highs, train_lows, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_clv_factor(oos_closes, oos_highs, oos_lows, lb, rf, nn, best_direction)
...
def run_split_half(closes, highs, lows, best_direction):
    """
    ...
    h1_highs = highs.iloc[:half]
    h1_lows = lows.iloc[:half]
    h2_closes = closes.iloc[half:]
    h2_highs = highs.iloc[half:]
    h2_lows = lows.iloc[half:]
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_clv_factor(h1_closes, h1_highs, h1_lows, lb, rf, nn, best_direction)
                r2 = backtest_clv_factor(h2_closes, h2_highs, h2_lows, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-186: Close Location Value (CLV) Factor")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_clv_factor(closes, highs, lows, lb, rf, nn, d)
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-186",
            "name": "Close Location Value (CLV) Factor",
            "status": "REJECTED",
            ...
            "wf_folds": [],
            "wf_positive_folds": None,
            "wf_total_folds": None,
            "wf_mean_oos_sharpe": None,
            "split_half": None,
            "correlations": None,
    ...
    wf_results = run_walk_forward(closes, highs, lows, best_dir)
    ...
    sh_best_h1, sh_best_h2, sh_mean_h1, sh_mean_h2 = run_split_half(closes, highs, lows, best_dir)
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_clv_factor(closes, highs, lows, best_lb, best_rf, best_n, best_dir)
