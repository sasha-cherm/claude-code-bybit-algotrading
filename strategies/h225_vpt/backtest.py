...
Idea: VPT = cumulative(volume * pct_change). Unlike OBV (which adds all volume
on up days, subtracts all on down days), VPT weights volume by the magnitude
of the return. This captures flow-of-funds more precisely — a 5% up day with
heavy volume contributes 5x more than a 1% up day.

Cross-sectional: rank by rolling VPT slope (trend of VPT).
High VPT slope = sustained buying pressure -> long.
Low VPT slope = sustained selling pressure -> short.

Signal:
  1. VPT = cumsum(volume * daily_return)
  2. Compute rolling slope of VPT over lookback (linear regression).
  3. Normalize slope by mean(volume) to make cross-section comparable.
  4. Rank: high VPT slope -> long, low -> short.

Parameter grid:
  Lookback  : [10, 15, 20, 30, 40]
  Rebal freq: [3, 5, 7]
  N:          [3, 4]
  Direction:  [high_vpt_long, low_vpt_long]
...
REBALS     = [3, 5, 7]
NS         = [3, 4]
DIRECTIONS = ["high_vpt_long", "low_vpt_long"]
...
def compute_vpt_slope(closes, volumes, lookback):
    """
    Compute normalized VPT slope for each asset.
    VPT = cumsum(volume * pct_change).
    Slope = linear regression slope of VPT over lookback window.
    Normalize by mean volume to make cross-asset comparable.
    """
    rets = closes.pct_change()
    vpt = (volumes * rets).cumsum()
    ...
    for sym in closes.columns:
        if sym not in volumes.columns:
            continue
        vpt_vals = vpt[sym].values
        vol_vals = volumes[sym].values
        n = len(vpt_vals)
        slope_arr = np.full(n, np.nan)

        for i in range(lookback, n):
            y = vpt_vals[i - lookback: i]
            if np.any(np.isnan(y)):
                continue
            y_mean = y.mean()
            slope = ((x - x_mean) * (y - y_mean)).sum() / x_var if x_var > 0 else 0
            # Normalize by mean volume in window
            mean_vol = vol_vals[i - lookback: i].mean()
            if mean_vol > 0:
                slope_arr[i] = slope / mean_vol
            else:
                slope_arr[i] = 0

        slopes[sym] = pd.Series(slope_arr, index=closes.index)
...
def backtest_vpt(closes, volumes, lookback, rebal_freq, n, direction="high_vpt_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    vpt_slope = compute_vpt_slope(closes, volumes, lookback).reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal < rebal_freq:
            continue
        vals = vpt_slope.iloc[i]
        valid = vals.dropna()
        if len(valid) < 2 * n:
            continue
        if direction == "high_vpt_long":
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        r = backtest_vpt(tr_c, tr_v, lb, rf, nn, d)
                        ev = evaluate(r)
        ...
        oos_r = backtest_vpt(oo_c, oo_v, lb, rf, nn, d)
...
def run_split_half(closes, volumes):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_vpt(h1c, h1v, lb, rf, nn, d))
                    e2 = evaluate(backtest_vpt(h2c, h2v, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-225: Volume-Price Trend (VPT) Factor")
    print("=" * 60)
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_vpt(closes, volumes, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-225", "name": "Volume-Price Trend (VPT)",
    ...
    best_r = backtest_vpt(closes, volumes, int(best_row["lb"]), int(best_row["rf"]),
