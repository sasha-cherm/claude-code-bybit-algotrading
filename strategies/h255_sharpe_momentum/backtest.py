#!/usr/bin/env python3
"""
H-255: Risk-Adjusted Momentum (Rolling Sharpe Ranking) Factor

Idea: Rank assets by their rolling Sharpe ratio (return/vol) instead of raw return.
Long high-Sharpe (quality momentum), short low-Sharpe (bad risk-adjusted performers).

Different from:
- H-012 (Momentum): raw return ranking, ignores volatility
- H-223 (Win Rate): counts positive days, ignores magnitude
- H-076 (Efficiency): path linearity, not return/risk ratio
- H-160 (Trend Quality): composite of efficiency * vol, different construction

The thesis: risk-adjusted return ranks should be more persistent and less vulnerable
to volatility-driven noise than raw return ranks. A coin up 50% with 100% vol is
less attractive than a coin up 30% with 20% vol.

Parameter grid:
  Lookback: [14, 21, 30, 40, 60]
  Rebal freq: [3, 5, 7]
  N: [3, 4]
...
def compute_rolling_sharpe(closes, lookback):
    """Compute rolling annualized Sharpe ratio for each asset."""
    rets = closes.pct_change().dropna()
    cols = list(rets.columns)
    sharpe_arr = np.full((len(rets), len(cols)), np.nan)
    ...
    for j, col in enumerate(cols):
        ...
        for i in range(lookback, len(vals)):
            ...
            if sigma > 1e-12:
                sharpe_arr[i, j] = mu / sigma  # daily Sharpe (no annualization needed for ranking)

    return pd.DataFrame(sharpe_arr, index=rets.index, columns=cols)
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            sig = compute_rolling_sharpe(tr_c, lb)
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
        ...
        full_sig = compute_rolling_sharpe(closes.iloc[:oos_end], lb)
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        h1_sig = compute_rolling_sharpe(h1c, lb)
        h2_sig = compute_rolling_sharpe(h2c, lb)
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
...
def main():
    print("=" * 60)
    print("  H-255: Risk-Adjusted Momentum (Rolling Sharpe) Factor")
    print("=" * 60)
    ...
    # Pre-compute rolling Sharpe for all lookbacks
    print("\nPre-computing rolling Sharpe ratios...")
    sharpe_cache = {}
    for lb in LOOKBACKS:
        sharpe_cache[lb] = compute_rolling_sharpe(closes, lb)
        print(f"  LB={lb} done")

    # Show average rolling Sharpe per asset (LB=30)
    if 30 in sharpe_cache:
        print("\nAvg rolling Sharpe per asset (LB=30):")
        avg_s = sharpe_cache[30].mean().sort_values(ascending=False)
        for sym, val in avg_s.items():
            if not np.isnan(val):
                print(f"  {sym}: {val:+.4f}")
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-255", "name": "Risk-Adjusted Momentum (Rolling Sharpe) Factor",
    ...
    best_sig = sharpe_cache[int(best_row["lb"])]
