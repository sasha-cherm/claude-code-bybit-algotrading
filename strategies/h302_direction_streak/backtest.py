...
Concept: Count the current streak of consecutive positive or negative daily returns.
This is a non-linear encoding of momentum — a 5-day winning streak is qualitatively
different from 5% return over 5 days (which could be one big day + 4 flat ones).

Signal: Current streak length (positive for consecutive up days, negative for
consecutive down days). Long assets with longest winning streaks (strong trend
continuation), short assets with longest losing streaks.

Alternative: Reversal variant — long assets with longest losing streaks (streak
exhaustion / reversal imminent), short assets with longest winning streaks.

This is fundamentally different from return-based momentum because it ignores
magnitude and focuses on direction consistency. An asset up 0.1% for 7 straight
days gets a higher signal than one up 10% over 7 days but with mixed days.

Parameter grid: max_lookback [10,14,20,30] (streak cap) × R [3,5,7] × N [3,4]
  × direction [continuation, reversal] = 4×3×2×2 = 48 combos
"""
...
def compute_streak(returns_col, max_lb):
    """
    Compute consecutive direction streak for a single asset.
    Positive streak = consecutive up days. Negative = consecutive down days.
    Capped at max_lb in either direction.
    """
    streak = np.zeros(len(returns_col))
    for i in range(1, len(returns_col)):
        if returns_col.iloc[i] > 0:
            if streak[i-1] > 0:
                streak[i] = min(streak[i-1] + 1, max_lb)
            else:
                streak[i] = 1
        elif returns_col.iloc[i] < 0:
            if streak[i-1] < 0:
                streak[i] = max(streak[i-1] - 1, -max_lb)
            else:
                streak[i] = -1
        else:
            streak[i] = 0
    return pd.Series(streak, index=returns_col.index)


def compute_signal(closes, max_lb, direction):
    """
    Signal based on consecutive direction streak.
    continuation: high streak (long winning streaks) → LONG
    reversal: low streak (long losing streaks) → LONG
    """
    ...
    for asset in ASSETS:
        if asset not in rets.columns:
            continue
        streak = compute_streak(rets[asset], max_lb)
        if direction == "continuation":
            signal[asset] = streak  # Long winning streaks, short losing
        else:  # reversal
            signal[asset] = -streak  # Long losing streaks (reversal bet)
...
def main():
    print("=" * 70)
    print("H-302: Consecutive Direction Streak Factor")
    print("=" * 70)
    ...
    results = []
    total = len(MAX_LOOKBACKS) * len(REBAL_FREQS) * len(N_POSITIONS) * len(DIRECTIONS)
    print(f"\nRunning {total} parameter combinations...")

    for max_lb, rebal, n_pos, direction in product(
        MAX_LOOKBACKS, REBAL_FREQS, N_POSITIONS, DIRECTIONS
    ):
        signal = compute_signal(closes, max_lb, direction)
        ...
        label = f"ML{max_lb}_R{rebal}_N{n_pos}_{direction}"
        results.append({
            "label": label, "max_lb": max_lb, "rebal": rebal,
            "n_pos": n_pos, "direction": direction, "sharpe": sr,
    ...
    for d in DIRECTIONS:
        ...
        print(f"  {d}: {pos}/{len(sub)} positive ({pos/len(sub)*100:.1f}%), mean Sharpe {mean_sr:.3f}")
    ...
    for lb in MAX_LOOKBACKS:
        ...
        mean_sr = sub["sharpe"].mean()
        pos = (sub["sharpe"] > 0).sum()
        print(f"  ML{lb}: mean Sharpe {mean_sr:.3f}, {pos}/{len(sub)} positive")

    # Walk-forward
    signal = compute_signal(closes, int(best["max_lb"]), best["direction"])
    ...
    neighbors = df[df["direction"] == best_d]
    n_neigh_pos = (neighbors["sharpe"] > 0).sum()
    print(f"\nSame-direction robustness: {n_neigh_pos}/{len(neighbors)} positive ({n_neigh_pos/len(neighbors)*100:.1f}%)")
    ...
    # Save
    out = {
        "hypothesis": "H-302", "name": "Direction Streak",
