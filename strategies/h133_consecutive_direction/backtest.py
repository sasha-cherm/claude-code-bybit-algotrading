"""
H-133: Consecutive Direction Factor — Cross-Sectional Backtest
==============================================================

Idea: Count the net number of up days minus down days over a lookback window.
  net_up_days = sum(sign(daily_return)) over last N days
  - Positive = more up days (potential uptrend)
  - Negative = more down days (potential downtrend)

This captures directional consistency, not return magnitude.
Differs from H-126 (return consistency = return/volatility) — this is purely
about the count of positive vs negative daily moves.

Directions:
  - MOMENTUM: long assets with most up days (positive net_up), short assets with
    most down days (negative net_up)
  - CONTRARIAN: reverse of above

Parameter grid: lookback [7,14,21,30] x rebal [3,5,7] x N [3,4] x direction [mom, contra]
...
def compute_consecutive_direction(closes, lookback):
    """
    Compute net up-days over the lookback window:
      signal = sum(sign(daily_return)) over last `lookback` days

    Positive = more up days than down days
    Negative = more down days than up days
    Zero = balanced
    """
    daily_ret = closes.pct_change()
    # sign: +1 for up, -1 for down, 0 for flat
    direction = daily_ret.apply(np.sign)
    # Rolling sum = net up days
    net_up = direction.rolling(lookback, min_periods=max(1, lookback // 2)).sum()
    return net_up
...
def backtest_xs_factor(closes, signals, rebal_days, n_long_short,
    ...
    direction='momentum':   long HIGHEST signal (most up days), short LOWEST
    direction='contrarian': long LOWEST signal (most down days), short HIGHEST
    """
    ...
    for i, date in enumerate(common_idx):
        ...
        if days_since >= rebal_days:
            ...
            if direction == 'momentum':
                # Momentum: long MOST UP DAYS (highest net_up), short MOST DOWN DAYS
                new_longs = ranked.index[-n_long_short:].tolist()
                new_shorts = ranked.index[:n_long_short].tolist()
            else:
                # Contrarian: long MOST DOWN DAYS, short MOST UP DAYS
                new_longs = ranked.index[:n_long_short].tolist()
                new_shorts = ranked.index[-n_long_short:].tolist()
...
def walk_forward_test(closes, lookback, rebal_days, n_long_short,
                      direction='momentum', n_folds=6, train_days=180, test_days=90):
    signals = compute_consecutive_direction(closes, lookback)
    returns = closes.pct_change()
    common_idx = signals.dropna(how='all').index.intersection(
...
def split_half_test(closes, lookback, rebal_days, n_long_short,
                    direction='momentum'):
    signals = compute_consecutive_direction(closes, lookback)
    common_idx = signals.dropna(how='all').index.intersection(
...
def compute_h031_returns(closes, rebal=5, n_long=4):
    ...
    returns = closes.pct_change()
    common_idx = size_signal.dropna(how='all').index.intersection(
        returns.dropna(how='all').index
    )
    size_signal = size_signal.loc[common_idx]
    asset_returns = returns.loc[common_idx]
    ...
    for i, date in enumerate(common_idx):
        if i == 0:
            continue
        prev_date = common_idx[i - 1]
        sig = size_signal.loc[prev_date].dropna()
        if len(sig) < 2 * n_long:
            continue
        days_since = (date - last_rebal).days if last_rebal else rebal + 1
        if days_since >= rebal:
            ranked = sig.sort_values()
            # Contrarian: long SMALL (low price), short LARGE (high price)
            current_longs = ranked.index[:n_long].tolist()
            current_shorts = ranked.index[-n_long:].tolist()
            last_rebal = date
        if current_longs and current_shorts:
...
def main():
    print("=" * 70)
    print("H-133: Consecutive Direction Factor")
    ...
    print(f"\nNet up-days sample (last date, lookback=14):")
    sample_sig = compute_consecutive_direction(closes, 14)
    last_sig = sample_sig.iloc[-1].sort_values()
    for asset, val in last_sig.items():
        print(f"  {asset:>5}: {val:+.0f} net up-days (of 14)")

    # Distribution analysis
    daily_ret = closes.pct_change()
    direction_raw = daily_ret.apply(np.sign)
    up_frac = (direction_raw > 0).mean()
    print(f"\nUp-day fractions across history:")
    for asset in ASSETS:
        if asset in up_frac.index:
            print(f"  {asset:>5}: {up_frac[asset]*100:.1f}% up days")

    # -- Parameter Grid Search --
    lookback_list = [7, 14, 21, 30]
    rebal_list = [3, 5, 7]
    n_list = [3, 4]
    ...
    for lb, rd, n, d in product(lookback_list, rebal_list, n_list, directions):
        signal = compute_consecutive_direction(closes, lb)
    ...
    signal_best = compute_consecutive_direction(closes, best_lb)
    ...
    wf_results = walk_forward_test(closes, best_lb, best_rd, best_n,
                                   direction=best_dir,
    ...
    for idx, (_, r) in enumerate(top3.iterrows()):
        ...
        wf = walk_forward_test(closes, lb, rd, n, direction=d)
        if wf:
    ...
    split = split_half_test(closes, best_lb, best_rd, best_n, direction=best_dir)
    ...
    h133_rets = backtest_xs_factor(closes, signal_best, best_rd, best_n,
                                   direction=best_dir)
    h012_rets = compute_h012_returns(closes, lookback=60, rebal=5, n_long=4)
    h031_rets = compute_h031_returns(closes, rebal=5, n_long=4)

    corr_h012, n_h012 = compute_correlation(h133_rets, h012_rets)
    corr_h031, n_h031 = compute_correlation(h133_rets, h031_rets)
    ...
    print(f"{'='*70}")
    # Distribution of net_up_days
    signal_all = compute_consecutive_direction(closes, 14).dropna(how='all')
    flat_vals = signal_all.values.flatten()
    flat_vals = flat_vals[~np.isnan(flat_vals)]
    print(f"  Net-up-days distribution (LB=14):")
    print(f"  Mean: {flat_vals.mean():.2f}, Std: {flat_vals.std():.2f}")
    print(f"  Min: {flat_vals.min():.0f}, Max: {flat_vals.max():.0f}")
    hist_vals, hist_bins = np.histogram(flat_vals, bins=[-15,-10,-5,0,5,10,15])
    for i in range(len(hist_vals)):
        print(f"    [{hist_bins[i]:+.0f}, {hist_bins[i+1]:+.0f}): {hist_vals[i]}")

    # -- Save Results --
    output = {
        'hypothesis': 'H-133',
        'name': 'Consecutive Direction Factor',
