...
Concept: Instead of simple N-day return (equal weight to all days), use
exponentially-weighted momentum where recent days get more weight. This captures
the RECENCY of momentum — an asset that surged yesterday has higher signal than
one that surged 30 days ago, even if total returns are equal.

Signal: EWM (exponentially weighted moving average) of daily returns.
Span parameter controls half-life of the exponential decay.

This is different from standard momentum because:
- Standard: all days equal weight → 60-day return = sum of 60 daily returns
- EWM: recent days dominate → same total return but concentrated recently = higher

Could capture information faster than standard momentum while maintaining cross-
sectional ranking properties.

Parameter grid: span [5,10,20,30,40] × R [3,5,7] × N [3,4] = 5×3×2 = 30 combos
"""
...
def compute_signal(closes, span):
    """
    Signal = EWM of daily returns with given span.
    Higher = stronger recent momentum → LONG.
    """
    rets = closes.pct_change()
    signal = rets.ewm(span=span, min_periods=span).mean()
...
def main():
    print("=" * 70)
    print("H-304: Exponentially-Weighted Momentum Factor")
    print("=" * 70)
    ...
    results = []
    total = len(SPANS) * len(REBAL_FREQS) * len(N_POSITIONS)
    print(f"\nRunning {total} parameter combinations...")

    for span, rebal, n_pos in product(SPANS, REBAL_FREQS, N_POSITIONS):
        signal = compute_signal(closes, span)
        ...
        label = f"SPAN{span}_R{rebal}_N{n_pos}"
        results.append({
            "label": label, "span": span, "rebal": rebal,
    ...
    # By span
    print("\nBy EWM span (half-life):")
    for s in SPANS:
        sub = df[df["span"] == s]
        mean_sr = sub["sharpe"].mean()
        mean_corr = sub["corr_h012"].mean()
        pos = (sub["sharpe"] > 0).sum()
        print(f"  SPAN{s}: mean Sharpe {mean_sr:.3f}, mean corr H-012 {mean_corr:.3f}, {pos}/{len(sub)} positive")

    # Walk-forward
    signal = compute_signal(closes, int(best["span"]))
    ...
    neighbors = df[df["span"].isin([max(5, best_span-5), best_span, best_span+5, best_span+10])]
    ...
    # Save
    out = {
        "hypothesis": "H-304", "name": "EWM Momentum",
