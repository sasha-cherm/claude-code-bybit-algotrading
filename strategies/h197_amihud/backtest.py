#!/usr/bin/env python3
"""
H-197: Amihud Illiquidity Factor (Cross-Sectional)

Concept: Amihud (2002) illiquidity measure = mean(|daily_return| / dollar_volume)
over a rolling lookback window. This measures price impact -- how much price moves
per unit of dollar volume traded.

Different from:
  - H-119 (Amihud -- different parameter grid, different validation framework)
  - H-155 (Amihud -- fetched live data, different IS criteria, different framework)
  This version uses the standardized 4-stage validation (IS >= 80%, WF, split-half,
  correlation) with cached parquet data and the exact parameter grid specified.

Logic:
  For each asset on each rebalance day:
    1. Compute Amihud = rolling mean of |return| / dollar_volume over lookback
       where dollar_volume = close * volume
    2. Rank assets cross-sectionally by Amihud illiquidity
    3. Two directions:
       - low_amihud_long: Long most liquid (low Amihud), Short most illiquid
         (flight to quality / liquidity preference)
       - high_amihud_long: Long most illiquid (high Amihud), Short most liquid
         (illiquidity premium)
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [low_amihud_long, high_amihud_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["low_amihud_long", "high_amihud_long"]
...
def backtest_amihud(closes, volumes, lookback, rebal_freq, n, direction):
    """
    Amihud Illiquidity Factor backtest.

    Args:
        closes:     DataFrame of daily closes
        volumes:    DataFrame of daily volumes (aligned with closes)
        lookback:   rolling window for Amihud calculation
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "low_amihud_long" or "high_amihud_long"
    Returns:
    ...
    # Compute |return| / dollar_volume for each day
    abs_returns = returns.abs()
    # Replace zero dollar volume with NaN to avoid division by zero
    dv_safe = dollar_volume.replace(0, np.nan)
    illiq_ratio = abs_returns / dv_safe
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute rolling mean Amihud for each asset
            amihud_scores = {}
            for sym in trade_cols:
                if sym not in illiq_ratio.columns:
                    continue
                window = illiq_ratio[sym].iloc[i - lookback:i].values
                # Skip if we have too many NaN
                valid_vals = window[~np.isnan(window)]
                if len(valid_vals) < lookback // 2:
                    continue
                avg_amihud = np.mean(valid_vals)
                if not np.isfinite(avg_amihud) or avg_amihud < 0:
                    continue
                amihud_scores[sym] = avg_amihud

            if len(amihud_scores) < 2 * n:
                continue

            ranked = pd.Series(amihud_scores).sort_values(ascending=True)
            # ranked: ascending by Amihud illiquidity
            # Bottom = lowest Amihud = most liquid
            # Top = highest Amihud = most illiquid

            if direction == "low_amihud_long":
                # Long liquid (low Amihud), Short illiquid (high Amihud)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # Long illiquid (high Amihud), Short liquid (low Amihud)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def run_walk_forward(closes, volumes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_amihud(train_closes, train_volumes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_amihud(oos_closes, oos_volumes, lb, rf, nn, best_direction)
...
def run_split_half(closes, volumes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_amihud(h1_closes, h1_volumes, lb, rf, nn, best_direction)
                r2 = backtest_amihud(h2_closes, h2_volumes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-197: Amihud Illiquidity Factor")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_amihud(closes, volumes, lb, rf, nn, d)
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_amihud(closes, volumes, best_lb, best_rf, best_n, best_dir)
