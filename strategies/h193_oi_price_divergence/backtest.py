...
Concept: Compare the momentum of open interest (OI) with price momentum.
When OI and price diverge, it signals a potential regime shift:
  - Price rising + OI falling: short squeeze / weak rally (bearish divergence)
  - Price falling + OI rising: new shorts entering / accumulation (bullish divergence)
  - Price rising + OI rising: healthy trend (confirmation)
  - Price falling + OI falling: capitulation / de-leveraging

The factor is the difference between OI momentum rank and price momentum rank
across assets. High divergence (OI rank >> price rank) means OI is strong
relative to price performance.

Direction options:
  - high_div_long: Long assets where OI momentum outpaces price momentum
                   (accumulation despite weak price = contrarian bullish),
                   Short assets where price outpaces OI (unsustained rally)
  - low_div_long:  Long assets where price outpaces OI (strong trend with
                   de-leveraging = healthy), Short where OI outpaces price
                   (crowded positioning despite weak price)

Different from:
  - H-044 (OI level -- uses raw OI rank, not momentum divergence)
  - H-053 (funding rate -- uses funding level, not OI-price divergence)
  - H-189 (funding dispersion -- cross-sectional funding spread, not OI-price)
  - H-012 (momentum -- uses price only, not OI)

Logic:
  For each asset on each rebalance day:
    1. Compute price_mom = close / close_lb_days_ago - 1
    2. Compute oi_mom = OI / OI_lb_days_ago - 1
    3. Rank price_mom and oi_mom cross-sectionally (1 = lowest)
    4. divergence = oi_rank - price_rank (positive = OI stronger than price)
    5. Rank divergence cross-sectionally
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [high_div_long, low_div_long]
...
# OI file names use ticker without /USDT
ASSET_TICKERS = {
    "BTC/USDT": "BTC", "ETH/USDT": "ETH", "SOL/USDT": "SOL",
    "XRP/USDT": "XRP", "DOGE/USDT": "DOGE", "ADA/USDT": "ADA",
    "AVAX/USDT": "AVAX", "DOT/USDT": "DOT", "LINK/USDT": "LINK",
    "NEAR/USDT": "NEAR", "OP/USDT": "OP", "ARB/USDT": "ARB",
    "ATOM/USDT": "ATOM", "SUI/USDT": "SUI",
}
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["high_div_long", "low_div_long"]
...
def load_daily_data():
    ...
    # Strip timezone to enable alignment with OI data (which is tz-naive)
...
def load_oi_data():
    """Load daily OI parquet files for all assets, return OI DataFrame."""
    data_dir = ROOT / "data" / "oi"
    oi_dict = {}
    for sym, ticker in ASSET_TICKERS.items():
        path = data_dir / f"{ticker}_oi_1d.parquet"
        if path.exists():
            df = pd.read_parquet(path)
            if "openInterest" in df.columns and len(df) >= 100:
                oi_dict[sym] = df["openInterest"]
    oi = pd.DataFrame(oi_dict)
    oi = oi.dropna(how="all").ffill().dropna()
...
def backtest_oi_price_divergence(closes, oi, lookback, rebal_freq, n, direction):
    """
    OI-Price Momentum Divergence Factor backtest.
    """
    # Find common assets
    trade_cols = list(set(closes.columns) & set(oi.columns))
    if len(trade_cols) < 2 * n:
        return None

    closes_aligned = closes[trade_cols]
    oi_aligned = oi[trade_cols]

    # Align indices
    common_idx = closes_aligned.index.intersection(oi_aligned.index)
    if len(common_idx) < lookback + 50:
        return None

    closes_aligned = closes_aligned.loc[common_idx]
    oi_aligned = oi_aligned.loc[common_idx]

    returns = closes_aligned.pct_change().dropna()
    # Re-align after pct_change
    common_idx2 = returns.index.intersection(oi_aligned.index)
    returns = returns.loc[common_idx2]
    closes_aligned = closes_aligned.loc[common_idx2]
    oi_aligned = oi_aligned.loc[common_idx2]
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            ...
            for sym in trade_cols:
                # Price momentum
                current_close = closes_aligned[sym].iloc[i]
                past_close = closes_aligned[sym].iloc[i - lookback]
                if past_close < 1e-10 or np.isnan(current_close) or np.isnan(past_close):
                    continue

                p_mom = current_close / past_close - 1
                price_mom[sym] = p_mom

                # OI momentum
                current_oi = oi_aligned[sym].iloc[i]
                past_oi = oi_aligned[sym].iloc[i - lookback]
                if past_oi < 1e-10 or np.isnan(current_oi) or np.isnan(past_oi):
                    continue

                o_mom = current_oi / past_oi - 1
                oi_mom[sym] = o_mom

            # Need enough assets with both signals
            common_syms = list(set(price_mom.keys()) & set(oi_mom.keys()))
            if len(common_syms) < 2 * n:
                continue

            # Rank cross-sectionally
            price_ranks = pd.Series({s: price_mom[s] for s in common_syms}).rank()
            oi_ranks = pd.Series({s: oi_mom[s] for s in common_syms}).rank()

            # Divergence = OI rank - Price rank
            # Positive: OI is relatively stronger than price (accumulation despite weak price)
            # Negative: Price is relatively stronger than OI (rally without OI confirmation)
            divergence = oi_ranks - price_ranks

            ranked = divergence.sort_values(ascending=True)

            if direction == "high_div_long":
                # Long where OI outpaces price (accumulation)
                # Short where price outpaces OI (unsustained)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
            else:
                # low_div_long: Long where price outpaces OI (healthy de-leverage)
                # Short where OI outpaces price (crowded)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
...
def run_walk_forward(closes, oi, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        train_oi = oi_wf.iloc[:oos_start]
        oos_closes = closes_wf.iloc[oos_start:oos_end]
        oos_oi = oi_wf.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_oi_price_divergence(train_closes, train_oi, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        oos_r = backtest_oi_price_divergence(oos_closes, oos_oi, lb, rf, nn, best_direction)
...
def run_split_half(closes, oi, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_oi_price_divergence(h1_closes, h1_oi, lb, rf, nn, best_direction)
                r2 = backtest_oi_price_divergence(h2_closes, h2_oi, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-193: OI-Price Momentum Divergence Factor")
    print("=" * 60)
    ...
    n_oi_days = len(oi)
    print(f"Loaded {n_oi_assets} assets, {n_oi_days} daily OI bars")
    print(f"OI date range: {oi.index[0]} -> {oi.index[-1]}")
    print(f"OI assets: {list(oi.columns)}")

    # Common assets
    common_assets = list(set(closes.columns) & set(oi.columns))
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_oi_price_divergence(closes, oi, lb, rf, nn, d)
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-193",
            "name": "OI-Price Momentum Divergence Factor",
            "status": "REJECTED",
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_oi_price_divergence(closes, oi, best_lb, best_rf, best_n, best_dir)
