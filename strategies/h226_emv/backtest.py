...
Idea: EMV = distance_moved / box_ratio, where:
  distance_moved = (high + low)/2 - (prev_high + prev_low)/2
  box_ratio = (volume / scale_factor) / (high - low)

High EMV = price moves easily with little volume -> trending smoothly.
Low EMV = heavy volume needed for small moves -> sticky/illiquid.

Cross-sectional: rank by rolling average EMV.
High EMV -> strong positive trend momentum with ease -> long.
Low EMV -> struggling with negative momentum -> short.

Unlike H-076 (efficiency ratio, price-only), H-191 (vol-price elasticity,
regression-based), or H-012 (momentum, return-only), EMV combines price
midpoint displacement with volume intensity in a single signal.

Parameter grid:
  Lookback  : [10, 15, 20, 30, 40]
  Rebal freq: [3, 5, 7]
  N         : [3, 4]
  Direction : [high_emv_long, low_emv_long]
...
REBALS     = [3, 5, 7]
NS         = [3, 4]
DIRECTIONS = ["high_emv_long", "low_emv_long"]
...
def load_daily_data():
    data_dir = ROOT / "data"
    highs_d, lows_d, closes_d, volumes_d = {}, {}, {}, {}
    ...
    volumes = pd.DataFrame(volumes_d).reindex(closes.index).ffill().dropna()
    for df in [closes, highs, lows, volumes]:
        df.index = df.index.tz_localize(None) if df.index.tzinfo else df.index
    return highs, lows, closes, volumes


def compute_emv(highs, lows, volumes, lookback):
    """
    Compute rolling average Ease of Movement for each asset.
    EMV = distance_moved / box_ratio
    distance_moved = midpoint - prev_midpoint
    box_ratio = (volume / 1e6) / (high - low)  (scale factor for volume)
    """
    midpoint = (highs + lows) / 2
    distance = midpoint.diff()
    hl_range = (highs - lows).replace(0, np.nan)

    # Box ratio: volume normalized by range. Use 1e6 scale to keep numbers manageable.
    # For cross-sectional ranking, the scale factor cancels out, so we just use raw.
    box_ratio = volumes / hl_range

    # EMV = distance / box_ratio = distance * hl_range / volume
    emv = distance * hl_range / volumes.replace(0, np.nan)

    # Rolling mean EMV
    avg_emv = emv.rolling(lookback).mean()
    # Normalize cross-sectionally by dividing by rolling std to get z-like score
    return avg_emv
...
def backtest_emv(highs, lows, closes, volumes, lookback, rebal_freq, n,
                 direction="high_emv_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    emv_df = compute_emv(highs, lows, volumes, lookback).reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
...
def run_walk_forward(highs, lows, closes, volumes):
    n_total = len(closes)
    fold_results = []
    for fold in range(WF_FOLDS):
        ...
        if oos_start < WF_TRAIN_MIN + 20:
            break
        tr_h, tr_l, tr_c, tr_v = highs.iloc[:oos_start], lows.iloc[:oos_start], closes.iloc[:oos_start], volumes.iloc[:oos_start]
        oo_h, oo_l, oo_c, oo_v = highs.iloc[oos_start:oos_end], lows.iloc[oos_start:oos_end], closes.iloc[oos_start:oos_end], volumes.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        r = backtest_emv(tr_h, tr_l, tr_c, tr_v, lb, rf, nn, d)
                        ev = evaluate(r)
        ...
        oos_r = backtest_emv(oo_h, oo_l, oo_c, oo_v, lb, rf, nn, d)
...
def run_split_half(highs, lows, closes, volumes):
    half = len(closes) // 2
    h1h, h1l, h1c, h1v = highs.iloc[:half], lows.iloc[:half], closes.iloc[:half], volumes.iloc[:half]
    h2h, h2l, h2c, h2v = highs.iloc[half:], lows.iloc[half:], closes.iloc[half:], volumes.iloc[half:]
    h1_sharpes, h2_sharpes = [], []
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_emv(h1h, h1l, h1c, h1v, lb, rf, nn, d))
                    e2 = evaluate(backtest_emv(h2h, h2l, h2c, h2v, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-226: Ease of Movement (EMV) Factor")
    print("=" * 60)

    print("\nLoading daily OHLCV data...")
    highs, lows, closes, volumes = load_daily_data()
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_emv(highs, lows, closes, volumes, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-226", "name": "Ease of Movement (EMV)",
    ...
    wf = run_walk_forward(highs, lows, closes, volumes)
    ...
    print("\n--- Stage 3: Split-Half ---")
    h1_best, h2_best, h1_mean, h2_mean = run_split_half(highs, lows, closes, volumes)
    ...
    best_r = backtest_emv(highs, lows, closes, volumes, int(best_row["lb"]),
