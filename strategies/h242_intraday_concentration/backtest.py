#!/usr/bin/env python3
"""
H-242: Intraday Momentum Concentration Factor

Idea: Using hourly data, measure what fraction of a day's absolute return comes
from the single highest-absolute-return hour. Assets with low concentration
(distributed moves) may have more sustainable trends vs spike-driven assets.

Signal construction:
  1. For each asset on each day, using hourly bars, find the max |hourly return|.
  2. Compute concentration = max(|h_ret|) / sum(|h_ret|) over that day's hours.
  3. Average over a lookback window.
  4. Rank XS: long low-concentration (steady), short high-concentration (spiky).

This is genuinely novel — no existing hypothesis tests intraday return distribution
patterns as a cross-sectional factor. Different from range (H-182), efficiency
(H-076), or volatility (H-019) because it captures the *temporal distribution*
of returns within each day.

Parameter grid:
  Lookback  : [10, 15, 20, 30]
  Rebal freq: [3, 5, 7]
  N         : [3, 4]
  Direction : [low_conc_long, high_conc_long]
...
def load_data():
    """Load hourly and daily data for all assets."""
    data_dir = ROOT / "data"
    closes_daily = {}
    concentration_daily = {}
    ...
    for sym in ASSETS_ALL:
        ...
        # Group by date, compute concentration per day
        h_rets_abs = h_rets.abs()
        grouped = h_rets_abs.groupby(h_rets_abs.index.date)
        daily_conc = {}
        for date, group in grouped:
            total = group.sum()
            if total > 1e-10 and len(group) >= 12:  # need at least half a day of data
                daily_conc[pd.Timestamp(date)] = group.max() / total
        conc_series = pd.Series(daily_conc).sort_index()
        ...
        closes_daily[sym] = dc
        concentration_daily[sym] = conc_series

    closes = pd.DataFrame(closes_daily).dropna(how="all").ffill().dropna()
    conc = pd.DataFrame(concentration_daily).reindex(closes.index).ffill()
    return closes, conc
...
def backtest_concentration(closes, conc, lookback, rebal_freq, n, direction="low_conc_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    # Rolling average concentration
    avg_conc = conc.rolling(lookback, min_periods=max(lookback // 2, 5)).mean()
    avg_conc = avg_conc.reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
    ...
    for i in range(warmup, len(dates)):
        ...
        if direction == "low_conc_long":
            ranked = valid.sort_values(ascending=True)  # lowest concentration first
        else:
            ranked = valid.sort_values(ascending=False)  # highest concentration first
...
def run_walk_forward(closes, conc):
    n_total = len(closes)
    fold_results = []
    for fold in range(WF_FOLDS):
        ...
        if oos_start < WF_TRAIN_MIN + 20:
            break
        tr_c, tr_conc = closes.iloc[:oos_start], conc.iloc[:oos_start]
        oo_c, oo_conc = closes.iloc[oos_start:oos_end], conc.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        r = backtest_concentration(tr_c, tr_conc, lb, rf, nn, d)
                        ev = evaluate(r)
        ...
        oos_r = backtest_concentration(oo_c, oo_conc, lb, rf, nn, d)
...
def run_split_half(closes, conc):
    half = len(closes) // 2
    h1c, h1conc = closes.iloc[:half], conc.iloc[:half]
    h2c, h2conc = closes.iloc[half:], conc.iloc[half:]
    h1_sharpes, h2_sharpes = [], []
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_concentration(h1c, h1conc, lb, rf, nn, d))
                    e2 = evaluate(backtest_concentration(h2c, h2conc, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-242: Intraday Momentum Concentration Factor")
    print("=" * 60)

    print("\nLoading hourly + daily data...")
    closes, conc = load_data()
    ...
    # Quick sanity check: show average concentration per asset
    print("\nAvg intraday concentration per asset:")
    avg_c = conc.mean().sort_values()
    for sym, val in avg_c.items():
        print(f"  {sym}: {val:.4f}")
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_concentration(closes, conc, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-242", "name": "Intraday Momentum Concentration",
    ...
    wf = run_walk_forward(closes, conc)
    ...
    # Stage 3: Split-Half
    print("\n--- Stage 3: Split-Half ---")
    h1_best, h2_best, h1_mean, h2_mean = run_split_half(closes, conc)
    ...
    best_r = backtest_concentration(closes, conc, int(best_row["lb"]), int(best_row["rf"]),
