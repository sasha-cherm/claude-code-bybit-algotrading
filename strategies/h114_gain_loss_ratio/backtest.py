"""
H-114: Gain/Loss Ratio Factor (Cross-Sectional)

Rank by avg(positive daily returns) / avg(|negative daily returns|) over N days.
Assets with high G/L ratio have asymmetric upside — their good days are bigger than
their bad days. This is different from skewness (third moment) and different from
simple momentum (total return).

Strategy: Long top N (high G/L ratio), Short bottom N (low G/L ratio).
Market-neutral (dollar-neutral), equal-weighted.

Differs from H-110 (skewness) which used return^3 distributional shape.
This uses the simple mean-gain / mean-loss ratio.
...
def compute_gain_loss_ratio_matrix(returns: pd.DataFrame, lookback: int) -> pd.DataFrame:
    """
    For each day t, compute rolling gain/loss ratio for each asset.
    G/L ratio = mean(positive returns in window) / mean(|negative returns| in window)
    Requires at least 3 positive and 3 negative days. Else NaN.
    """
    n_days = len(returns)
    n_assets = len(returns.columns)
    gl_vals = np.full((n_days, n_assets), np.nan)
    ...
    for i in range(lookback, n_days):
        ...
        for j in range(n_assets):
            ...
            if len(gains) < 3 or len(losses) < 3:
                continue
            avg_gain = np.mean(gains)
            avg_loss = np.mean(np.abs(losses))
            if avg_loss < 1e-10:
                continue
            gl_vals[i, j] = avg_gain / avg_loss

    return pd.DataFrame(gl_vals, index=returns.index, columns=returns.columns)


def run_gain_loss_factor(
    closes: pd.DataFrame,
    lookback: int,
    rebal_freq: int,
    n_long: int,
    ...
    if pre_computed is not None:
        gl_matrix = pre_computed
    else:
        gl_matrix = compute_gain_loss_ratio_matrix(returns, lookback)
    ...
    for i in range(1, n):
        ...
        if i >= warmup and (i - warmup) % rebal_freq == 0:
            ...
            longs  = ranked.index[:n_long]    # highest G/L ratio
            shorts = ranked.index[-n_short:]  # lowest G/L ratio
...
def run_full_scan(closes: pd.DataFrame):
    print("\n" + "=" * 72)
    print("H-114: GAIN/LOSS RATIO FACTOR -- Full Param Scan")
    print("=" * 72)
    ...
    print(f"  Strategy : LONG high G/L ratio, SHORT low G/L ratio")

    returns = closes.pct_change()

    # Pre-compute G/L matrices
    print("\n  Pre-computing gain/loss ratio matrices...")
    gl_cache = {}
    for lb in LOOKBACKS:
        print(f"    Lookback {lb}d ...", end=" ", flush=True)
        gl_cache[lb] = compute_gain_loss_ratio_matrix(returns, lb)
        print("done")

    results = []
    print(f"\n  Running {len(LOOKBACKS) * len(REBAL_FREQS) * len(N_SIZES)} param combos...")
    for lb, rebal, n in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
        res = run_gain_loss_factor(closes, lb, rebal, n, pre_computed=gl_cache[lb])
    ...
    for fold in range(n_folds):
        ...
        for lb, rebal, nn in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
            if len(train_closes) < lb + 30:
                continue
            res = run_gain_loss_factor(train_closes, lb, rebal, nn)
        ...
        lb, rebal, nn = best_p
        test_res = run_gain_loss_factor(test_closes, lb, rebal, nn)
        wf_results.append({
...
def split_half_test(closes):
    ...
    r1, r2 = [], []
    for lb, rebal, n in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
        res1 = run_gain_loss_factor(h1, lb, rebal, n)
        res2 = run_gain_loss_factor(h2, lb, rebal, n)
...
def correlation_with_h012(closes, best_lb=40, best_rebal=5, best_n=4):
    """Compute correlation with H-012 (raw momentum)."""
    res = run_gain_loss_factor(closes, best_lb, best_rebal, best_n)
