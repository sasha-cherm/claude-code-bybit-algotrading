"""
H-210: RSI Dispersion Factor
Cross-sectional ranking on standard deviation of rolling RSI(14) over a
lookback window. Low RSI dispersion = consistent trend; high = choppy.
"""
...
def load_data():
    """Load close prices, compute RSI(14) series for each asset."""
    closes = {}
    for asset in ASSETS:
    ...
    # Compute RSI(14) for each asset
    rsi_df = pd.DataFrame(index=close_df.index, columns=ASSETS, dtype=float)
    for asset in ASSETS:
        delta = close_df[asset].diff()
        gain = delta.clip(lower=0)
        loss = (-delta).clip(lower=0)
        avg_gain = gain.ewm(alpha=1/14, min_periods=14).mean()
        avg_loss = loss.ewm(alpha=1/14, min_periods=14).mean()
        rs = avg_gain / avg_loss
        rsi_df[asset] = 100 - (100 / (1 + rs))

    return log_ret, rsi_df


def compute_signal(rsi_df, lookback):
    """Rolling std of RSI(14) over lookback window = RSI dispersion."""
    n_dates = len(rsi_df)
    ...
    for i in range(lookback, n_dates):
        for j in range(len(ASSETS)):
            window = rsi_vals[i - lookback:i, j]
            valid = window[~np.isnan(window)]
            if len(valid) >= lookback // 2:
                signal[i, j] = np.std(valid, ddof=1)

    return pd.DataFrame(signal, index=rsi_df.index, columns=ASSETS)
...
def walk_forward(log_ret, rsi_df, lookback, rebal_period, n_top, direction, n_folds=6, fold_days=90):
    ...
    for fold in range(n_folds):
        ...
        if test_start <= lookback:
            break
        sig = compute_signal(rsi_df.iloc[:test_end], lookback)
...
def correlation_with_h012(log_ret, signal_df, lookback, rebal_period, n_top, direction):
    ...
    for i in range(max(lookback, mom_lb) + 1, len(dates)):
        ...
        if mom_weights and sig_weights:
            ...
            h210_rets.append(r_sig)

    if len(h012_rets) > 30:
        return float(np.corrcoef(h012_rets, h210_rets)[0, 1])
    return None
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("H-210: RSI Dispersion Factor Backtest")
    print("=" * 60)

    log_ret, rsi_df = load_data()
    ...
    for lb in LOOKBACKS:
        t1 = time.time()
        signals[lb] = compute_signal(rsi_df, lb)
    ...
    dir_lo = [r["sharpe"] for r in results if r["direction"] == "low_disp_long"]
    dir_hi = [r["sharpe"] for r in results if r["direction"] == "high_disp_long"]
    mean_lo, mean_hi = float(np.mean(dir_lo)), float(np.mean(dir_hi))
    dom_direction = "low_disp_long" if mean_lo >= mean_hi else "high_disp_long"
    ...
    print(f"  Mean low_disp_long: {mean_lo:.4f}, high_disp_long: {mean_hi:.4f}")

    top10 = sorted(results, key=lambda x: x["sharpe"], reverse=True)[:10]
    print("\n  Top-10:")
    for r in top10:
        print(f"    lb={r['lookback']} rp={r['rebal_period']} N={r['n_top']} {r['direction'][:8]:>8} S={r['sharpe']:.4f} AR={r['annual_return']:.2%}")

    wf_result = None
    if hit_rate >= 0.80:
        print(f"\n  IS {hit_rate:.1%} >= 80% — running walk-forward...")
        fold_sharpes = walk_forward(log_ret, rsi_df, best["lookback"], best["rebal_period"],
    ...
    if wf_result and wf_result.get("wf_pass"):
        ...
        mid = len(log_ret) // 2
        sig_h1 = compute_signal(rsi_df.iloc[:mid], best["lookback"])
        sig_h2 = compute_signal(rsi_df.iloc[mid:], best["lookback"])
    ...
    output = {
        "hypothesis": "H-210", "title": "RSI Dispersion Factor",
        "in_sample": {"hit_rate": round(hit_rate,4), "mean_sharpe": round(mean_sharpe,4),
                      ...
                      "mean_low_disp": round(mean_lo,4), "mean_high_disp": round(mean_hi,4), "top10": top10},
