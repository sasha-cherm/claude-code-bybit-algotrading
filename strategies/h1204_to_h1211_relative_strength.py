"""
Batch backtest: H-1204 to H-1211 — Relative Strength Dynamics.
Not momentum itself, but how relative strength evolves over time.

H-1204: Rank Velocity — change in XS rank over 10d. Fast rank movers.
H-1205: Rank Mean Reversion — distance from 20d avg rank. XS contrarian.
H-1206: Relative Strength Index (XS) — RSI(14) ranked cross-sectionally.
H-1207: Momentum Smoothness — R² of cumulative return curve over 20d.
H-1208: Outperformance Streak — consecutive days outperforming XS median.
H-1209: Relative Drawdown — current DD from asset's own 30d high vs XS median DD.
H-1210: Win Rate Rank — % of positive return days over 20d, ranked XS.
H-1211: Return Dispersion Contribution — asset's contribution to XS return dispersion.
"""
...
def compute_signals(closes, volumes):
    ...
    # H-1204: Rank Velocity — change in XS return rank over 10d
    ret20 = closes.pct_change(20)
    rank_now = ret20.rank(axis=1, pct=True)
    rank_10ago = rank_now.shift(10)
    signals["rank_velocity"] = rank_now - rank_10ago

    # H-1205: Rank Mean Reversion — distance from 20d average rank
    rank_avg = rank_now.rolling(20).mean()
    signals["rank_mr"] = rank_avg - rank_now

    # H-1206: RSI(14) XS — standard RSI ranked cross-sectionally
    rsi_df = pd.DataFrame(index=closes.index, columns=closes.columns, dtype=float)
    for col in closes.columns:
        delta = returns[col]
        gain = delta.clip(lower=0)
        loss = (-delta).clip(lower=0)
        avg_gain = gain.rolling(14).mean()
        avg_loss = loss.rolling(14).mean().clip(lower=1e-15)
        rs = avg_gain / avg_loss
        rsi_df[col] = 100 - (100 / (1 + rs))
    signals["rsi_xs"] = rsi_df

    # H-1207: Momentum Smoothness — R² of cumulative return curve over 20d
    smooth = pd.DataFrame(index=closes.index, columns=closes.columns, dtype=float)
    x = np.arange(20)
    for col in closes.columns:
        ...
        for i in range(20, len(r)):
            ...
            slope, _, r_value, _, _ = stats.linregress(x, cum_ret)
            out[i] = r_value ** 2 * np.sign(slope)
            # positive R² = smooth uptrend, negative R² * sign = smooth downtrend
        smooth[col] = out
    signals["mom_smoothness"] = smooth

    # H-1208: Outperformance Streak — consecutive days outperforming XS median
    xs_median = returns.median(axis=1)
    outperf = returns.sub(xs_median, axis=0) > 0
    streak = pd.DataFrame(0, index=closes.index, columns=closes.columns, dtype=float)
    for col in closes.columns:
        s = 0
        vals = outperf[col].values
        ...
        for i in range(len(vals)):
            ...
            out[i] = s
        streak[col] = out
    signals["outperf_streak"] = streak

    # H-1209: Relative Drawdown — own DD from 30d high vs cross-sectional median DD
    high30 = closes.rolling(30).max()
    dd = (closes / high30 - 1)
    dd_median = dd.median(axis=1)
    signals["relative_dd"] = dd.sub(dd_median, axis=0)

    # H-1210: Win Rate Rank — % of positive return days over 20d
    pos_days = (returns > 0).astype(float)
    signals["win_rate"] = pos_days.rolling(20).mean()

    # H-1211: Return Dispersion Contribution — how much each asset contributes to XS dispersion
    xs_mean = returns.mean(axis=1)
    sq_dev = returns.sub(xs_mean, axis=0) ** 2
    signals["disp_contribution"] = sq_dev.rolling(20).mean()
...
def main():
    ...
    feature_map = {
        "H-1204": ("rank_velocity", "Rank Velocity (10d rank change)"),
        "H-1205": ("rank_mr", "Rank Mean Reversion (20d avg rank - current)"),
        "H-1206": ("rsi_xs", "RSI(14) XS Ranking"),
        "H-1207": ("mom_smoothness", "Momentum Smoothness (R² of cum return)"),
        "H-1208": ("outperf_streak", "Outperformance Streak (consec days > median)"),
        "H-1209": ("relative_dd", "Relative Drawdown (own DD vs XS median)"),
        "H-1210": ("win_rate", "Win Rate (20d % positive days)"),
        "H-1211": ("disp_contribution", "Return Dispersion Contribution (20d)"),
    }
