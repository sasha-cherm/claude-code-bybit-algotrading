...
Concept: Use BTC volatility regime to condition cross-sectional factor choice.
In high-volatility BTC regimes (trending), use momentum (winners keep winning).
In low-volatility BTC regimes (range-bound), use mean-reversion (contrarian).

Signal construction:
1. Compute BTC realized vol (rolling std of returns)
2. Compare to long-term median: if BTC_vol > median → "trending" regime
3. In trending: rank by N-day momentum, long top / short bottom
4. In range-bound: rank by N-day momentum, long BOTTOM / short TOP (reversal)

Key insight: Momentum works in trends, mean-reversion works in ranges.
BTC vol is the regime indicator that determines which altcoin factor to use.

Parameter grid: mom_LB [20,40,60] × vol_LB [10,20,30] × vol_long [60,90,120] × R [3,5,7] × N [3,4]
...
VOL_SHORT_WINDOWS = [10, 20, 30]
VOL_LONG_WINDOWS = [60, 90, 120]
REBAL_FREQS = [3, 5, 7]
N_POSITIONS = [3, 4]
...
def compute_signal(
    ...
    """
    Returns cross-sectional signal that flips based on BTC regime.
    In high-vol (trending): positive momentum → positive signal (momentum)
    In low-vol (range-bound): positive momentum → negative signal (reversal)
    """
    btc_rets = closes["BTC"].pct_change()
    btc_vol_short = btc_rets.rolling(vol_short, min_periods=vol_short).std()
    btc_vol_long = btc_rets.rolling(vol_long, min_periods=vol_long).std()

    # Regime: 1 = trending (high vol), -1 = range-bound (low vol)
    regime = pd.Series(1.0, index=closes.index)
    regime[btc_vol_short < btc_vol_long] = -1.0

    # Momentum for all assets
    momentum = closes.pct_change(mom_lb)

    # Regime-conditional signal: multiply momentum rank by regime
    # In trending: high momentum → high signal (long)
    # In range: high momentum → low signal (short — contrarian)
    signal = momentum.multiply(regime, axis=0)
...
def main():
    print("=" * 70)
    print("H-293: BTC-Regime Conditional Factor Switching")
    print("=" * 70)
    ...
    results = []
    total = len(MOM_LOOKBACKS) * len(VOL_SHORT_WINDOWS) * len(VOL_LONG_WINDOWS) * len(REBAL_FREQS) * len(N_POSITIONS)
    print(f"\nRunning {total} parameter combinations...")

    for mom_lb, vol_short, vol_long, rebal, n_pos in product(
        MOM_LOOKBACKS, VOL_SHORT_WINDOWS, VOL_LONG_WINDOWS, REBAL_FREQS, N_POSITIONS
    ):
        if vol_short >= vol_long:
            continue

        signal = compute_signal(closes, mom_lb, vol_short, vol_long)
        ...
        label = f"MOM{mom_lb}_VS{vol_short}_VL{vol_long}_R{rebal}_N{n_pos}"
        results.append({
            "label": label,
            "mom_lb": mom_lb,
            "vol_short": vol_short,
    ...
    # Walk-forward best
    print(f"\n--- Walk-Forward ---")
    signal = compute_signal(closes, int(best["mom_lb"]), int(best["vol_short"]), int(best["vol_long"]))
    ...
    # Neighboring params
    bm, bvs, bvl, br, bn = int(best["mom_lb"]), int(best["vol_short"]), int(best["vol_long"]), int(best["rebal"]), int(best["n_pos"])
    neighbors = df[
        (df["mom_lb"].isin([max(20, bm-20), bm, min(60, bm+20)])) &
        (df["vol_short"].isin([max(10, bvs-10), bvs, min(30, bvs+10)])) &
    ...
    out = {
        "hypothesis": "H-293",
        "name": "BTC-Regime Conditional Factor Switching",
        "total_params": len(df),
