...
Concept: Measure the CHANGE in an asset's beta to BTC over time. Assets whose
beta is increasing are becoming more correlated with BTC (market sentiment
absorbing them). Assets whose beta is decreasing are decoupling.

Signal: short_window_beta - long_window_beta.
High = beta increasing (asset becoming more BTC-correlated)
Low = beta decreasing (asset decoupling from BTC)

This is different from H-295 (BTC beta timing) which used BTC direction to time
beta exposure. This ranks assets by beta CHANGE regardless of BTC direction.

Intuition: Assets with increasing beta may be entering "crowd" mode (institutional
herding), while those with decreasing beta may be driven by idiosyncratic factors.

Parameter grid: short_beta [10,20] × long_beta [30,40,60] × R [3,5,7] × N [3,4]
  × direction [increasing_long, decreasing_long] = 2×3×3×2×2 = 72 combos
"""
...
REBAL_FREQS = [3, 5, 7]
N_POSITIONS = [3, 4]
DIRECTIONS = ["increasing_long", "decreasing_long"]
...
def compute_rolling_beta(asset_rets, btc_rets, window):
    """Compute rolling beta of asset returns to BTC returns."""
    cov = asset_rets.rolling(window, min_periods=window).cov(btc_rets)
    var = btc_rets.rolling(window, min_periods=window).var()
    return cov / var.replace(0, np.nan)


def compute_signal(closes, short_win, long_win, direction):
    """
    Signal = short_beta - long_beta (beta change/acceleration).
    increasing_long: high beta change → LONG (herding)
    decreasing_long: low beta change → LONG (decoupling)
    """
    rets = closes.pct_change()
    btc_rets = rets["BTC"]

    signal = pd.DataFrame(index=closes.index, columns=ALT_ASSETS, dtype=float)

    for asset in ALT_ASSETS:
        if asset not in rets.columns:
            continue
        short_beta = compute_rolling_beta(rets[asset], btc_rets, short_win)
        long_beta = compute_rolling_beta(rets[asset], btc_rets, long_win)
        beta_change = short_beta - long_beta

        if direction == "increasing_long":
            signal[asset] = beta_change  # High = increasing beta → LONG
        else:
            signal[asset] = -beta_change  # Low = decreasing beta → LONG
...
def simulate(closes, signal, rebal, n_pos, start_idx=0, end_idx=None):
    # Use only ALT_ASSETS for trading
    avail_assets = [a for a in ALT_ASSETS if a in closes.columns]
    dates = signal.index
    if end_idx is None:
        end_idx = len(dates)
    dates = dates[start_idx:end_idx]
    sig = signal.iloc[start_idx:end_idx]
    cls = closes[avail_assets].reindex(dates)
...
def main():
    print("=" * 70)
    print("H-305: Beta Change Factor")
    print("=" * 70)
    ...
    results = []
    total = len(SHORT_BETAS) * len(LONG_BETAS) * len(REBAL_FREQS) * len(N_POSITIONS) * len(DIRECTIONS)
    print(f"\nRunning {total} parameter combinations...")

    for sb, lb, rebal, n_pos, direction in product(
        SHORT_BETAS, LONG_BETAS, REBAL_FREQS, N_POSITIONS, DIRECTIONS
    ):
        if sb >= lb:
            continue
        signal = compute_signal(closes, sb, lb, direction)
        ...
        label = f"SB{sb}_LB{lb}_R{rebal}_N{n_pos}_{direction}"
        results.append({
            "label": label, "short_beta": sb, "long_beta": lb,
            "rebal": rebal, "n_pos": n_pos, "direction": direction,
            "sharpe": sr, "annual_return": ar, "max_dd": dd, "corr_h012": corr_012,
        })
    ...
    # Walk-forward
    signal = compute_signal(closes, int(best["short_beta"]), int(best["long_beta"]), best["direction"])
    ...
    # Save
    out = {
        "hypothesis": "H-305", "name": "Beta Change",
