#!/usr/bin/env python3
"""
H-162: MAX Effect (Maximum Daily Return Factor, 14 Crypto Assets)

Based on Bali, Cakici, Whitelaw (2011) — stocks with highest maximum
single-day return in past month underperform (overpriced lottery tickets).

Cross-section: short high-MAX assets (lottery/speculation premium),
long low-MAX assets (overlooked steady performers).
"""
...
def backtest_factor(closes, lookback, rebal_freq, n_long, n_short, direction="short_max"):
    """
    direction: 'short_max' = short highest MAX, long lowest MAX (classic MAX effect).
               'long_max' = reverse (momentum-like).
    """
        ...
        if i - last_rebal >= rebal_freq:
            ...
            for sym in closes.columns:
                ...
                if len(rets_window) < lookback:
                    continue
                max_ret = rets_window.max()
                signals[sym] = max_ret

            if len(signals) >= n_long + n_short:
                ranked = pd.Series(signals).sort_values(ascending=False)
                if direction == "short_max":
                    # Short highest MAX (lottery), long lowest MAX (steady)
                    longs = ranked.index[-n_long:]
                    shorts = ranked.index[:n_short]
                else:
                    # Long highest MAX (momentum), short lowest MAX
                    longs = ranked.index[:n_long]
                    shorts = ranked.index[-n_short:]
...
def main():
    ...
    for d in directions:
        ...
        if len(sub) > 0:
            ...
            print(f"\nBest {d}: {best['label']} Sharpe {best['sharpe']:.3f}, Ann {best['annual']:.1f}%, DD {best['dd']:.1f}%")

    # Check if best direction passes 80% threshold
    best_dir = None
    for d in directions:
        sub = df[df["label"].str.contains(d)]
    ...
    if mom_rets is not None and best_rets is not None:
        ...
        if len(common) > 30:
            ...
            print(f"Corr H-012 (momentum): {corr_mom:.3f}")
