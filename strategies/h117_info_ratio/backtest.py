"""
H-117: Information Ratio Factor (Cross-Sectional)

Rank assets by rolling information ratio (return / volatility) over a lookback
window. LONG assets with highest IR (consistent outperformers), SHORT lowest IR.

This differs from pure momentum (H-012, ranks by return level only) and
low-volatility (H-019, ranks by vol only). The information ratio is a composite:
it rewards assets that deliver returns *efficiently* — high return with low noise.
An asset with +50% return and 80% vol ranks lower than one with +30% return and
20% vol. This should capture a different cross-section than either signal alone.
...
def compute_info_ratio_matrix(returns: pd.DataFrame, lookback: int) -> pd.DataFrame:
    """
    Rolling information ratio: mean(return) / std(return) over lookback.
    Effectively a rolling Sharpe (without annualization — just for ranking).
    """
    n_days = len(returns)
    n_assets = len(returns.columns)
    ir_vals = np.full((n_days, n_assets), np.nan)
    ...
    for i in range(lookback, n_days):
        for j in range(n_assets):
            ...
            if std_r > 1e-12:
                ir_vals[i, j] = mean_r / std_r
            else:
                ir_vals[i, j] = 0.0

    return pd.DataFrame(ir_vals, index=returns.index, columns=returns.columns)


def run_info_ratio_factor(
    closes: pd.DataFrame,
    lookback: int,
    rebal_freq: int,
    n_long: int,
    ...
    if pre_computed is not None:
        ir_matrix = pre_computed
    else:
        ir_matrix = compute_info_ratio_matrix(returns, lookback)
    ...
    for i in range(1, n):
        ...
        if i >= warmup and (i - warmup) % rebal_freq == 0:
            ...
            # Long highest IR (best risk-adjusted performers), short lowest
...
def run_full_scan(closes: pd.DataFrame):
    print("\n" + "=" * 72)
    print("H-117: INFORMATION RATIO FACTOR -- Full Param Scan")
    print("=" * 72)
    ...
    print(f"  Strategy : LONG high info ratio, SHORT low info ratio")

    returns = closes.pct_change()

    print("\n  Pre-computing info ratio matrices...")
    ir_cache = {}
    for lb in LOOKBACKS:
        print(f"    Lookback {lb}d ...", end=" ", flush=True)
        ir_cache[lb] = compute_info_ratio_matrix(returns, lb)
        print("done")

    results = []
    print(f"\n  Running {len(LOOKBACKS) * len(REBAL_FREQS) * len(N_SIZES)} param combos...")
    for lb, rebal, n in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
        res = run_info_ratio_factor(closes, lb, rebal, n, pre_computed=ir_cache[lb])
    ...
    for fold in range(n_folds):
        ...
        for lb, rebal, nn in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
            if len(train_closes) < lb + 30:
                continue
            res = run_info_ratio_factor(train_closes, lb, rebal, nn)
        ...
        lb, rebal, nn = best_p
        test_res = run_info_ratio_factor(test_closes, lb, rebal, nn)
        wf_results.append({
...
def split_half_test(closes):
    ...
    r1, r2 = [], []
    for lb, rebal, n in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
        res1 = run_info_ratio_factor(h1, lb, rebal, n)
        res2 = run_info_ratio_factor(h2, lb, rebal, n)
...
def correlation_with_h012(closes, best_lb=30, best_rebal=5, best_n=4):
    res = run_info_ratio_factor(closes, best_lb, best_rebal, best_n)
