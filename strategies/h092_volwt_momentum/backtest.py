"""
H-092: Volume-Weighted Momentum Factor (Cross-Sectional)

Instead of raw 60d return (like H-012 momentum), compute volume-weighted return:
each day's return is weighted by that day's relative volume (volume / avg volume
over the lookback). This captures whether big price moves happened on high or low
volume days -- moves on high volume are more meaningful.

- For each asset, compute daily log return and daily relative volume (vol / rolling_mean_vol)
- Volume-weighted momentum = sum(daily_log_return * relative_volume) over lookback period
- Rank assets by this metric. Long top N (highest vol-weighted momentum), short bottom N
- Dollar-neutral, equal weight per side

Validation: walk-forward (6 folds, 300d train, 90d test), split-half, 70/30 train/test,
parameter robustness, correlation with H-012 momentum.
"""
...
# Parameter grid
LOOKBACKS = [20, 40, 60, 90]          # momentum lookback (days)
...
def volume_weighted_momentum(closes, volumes, lookback):
    """
    Compute volume-weighted momentum for each asset/day.

    For each day t:
      daily_log_return[t] = log(close[t] / close[t-1])
      relative_volume[t] = volume[t] / rolling_mean(volume, lookback)[t]
      vol_wt_mom[t] = sum over [t-lookback+1 .. t] of (daily_log_return * relative_volume)

    Higher value = strong momentum confirmed by high volume.
    """
    # Daily log returns
    log_rets = np.log(closes / closes.shift(1))

    # Relative volume: volume / rolling mean volume over lookback
    avg_vol = volumes.rolling(lookback, min_periods=lookback).mean()
    rel_vol = volumes / avg_vol
    rel_vol = rel_vol.replace([np.inf, -np.inf], np.nan)

    # Volume-weighted momentum: rolling sum of (log_return * relative_volume)
    weighted_rets = log_rets * rel_vol
    vw_mom = weighted_rets.rolling(lookback, min_periods=lookback).sum()
...
def run_full_scan(closes, volumes):
    ...
    print("\n" + "=" * 70)
    print("H-092: VOLUME-WEIGHTED MOMENTUM FACTOR -- Full Parameter Scan")
    print("=" * 70)
    ...
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking = volume_weighted_momentum(closes, volumes, lookback)
        warmup = lookback + 5  # need at least lookback days for factor computation
...
def run_train_test_split(closes, volumes, split_ratio=0.7):
    ...
    train_results = []
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking = volume_weighted_momentum(train_closes, train_volumes, lookback)
        warmup = lookback + 5
    ...
    test_ranking = volume_weighted_momentum(test_closes, test_volumes, lookback)
    warmup = lookback + 5
...
def run_split_half(closes, volumes):
    ...
    results_h2 = []
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking1 = volume_weighted_momentum(half1_closes, half1_volumes, lookback)
        warmup = lookback + 5
        res1 = run_xs_factor(half1_closes, ranking1, rebal, n_long, warmup=warmup)

        ranking2 = volume_weighted_momentum(half2_closes, half2_volumes, lookback)
...
def run_walk_forward(closes, volumes, lookback, rebal, n_long):
    """
...
def compute_h012_correlation(closes, volumes, lookback, rebal, n_long):
    """Compute daily return correlation between vol-weighted momentum and H-012 momentum."""
    print(f"\n  Correlation with H-012 (60d Momentum)")

    # Run vol-weighted momentum factor with best params
    vw_ranking = volume_weighted_momentum(closes, volumes, lookback)
    warmup_vw = lookback + 5
    res_vw = run_xs_factor(closes, vw_ranking, rebal, n_long, warmup=warmup_vw)

    # Run H-012 momentum (60d lookback, rebal 5d, N=4) -- same as H-012 deployed params
    ...
    rets_vw = eq_vw.pct_change().dropna()
    rets_mom = eq_mom.pct_change().dropna()

    common = rets_vw.index.intersection(rets_mom.index)
    if len(common) < 50:
        print("    Insufficient overlap for correlation")
        return 0.0

    corr = rets_vw.loc[common].corr(rets_mom.loc[common])
    print(f"    Daily return correlation: {corr:.3f}")
    print(f"    H-012 Momentum:       Sharpe {res_mom['sharpe']:.3f}, Ann {res_mom['annual_ret']:.1%}")
    print(f"    H-092 Vol-Wt Momentum: Sharpe {res_vw['sharpe']:.3f}, Ann {res_vw['annual_ret']:.1%}")
    return round(corr, 3)
...
if __name__ == "__main__":
    print("H-092: Volume-Weighted Momentum Factor")
    print("=" * 70)
    ...
    wf_fixed = run_walk_forward(closes, volumes, best_lookback, best_rebal, best_n_long)
    ...
    h012_corr = compute_h012_correlation(closes, volumes, best_lookback, best_rebal, best_n_long)
    ...
    # ===== 9. Summary =====
    print("\n" + "=" * 70)
    print("SUMMARY: H-092 Volume-Weighted Momentum Factor")
    print("=" * 70)
    ...
    results_data = {
        "hypothesis": "H-092",
        "name": "Volume-Weighted Momentum Factor",
        "description": "Cross-sectional factor: long highest vol-weighted momentum, short lowest",
