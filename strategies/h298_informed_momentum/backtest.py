...
Concept: Standard momentum uses ALL daily returns. But high-volume days
contain more "informed" trading while low-volume days are noise.

Signal: Compute cumulative return using ONLY high-volume days (above
rolling median volume). This "informed momentum" should be more predictive
than raw momentum because it filters out noise days.

This is genuinely novel — no prior hypothesis has filtered by volume
before computing momentum signals.

Parameter grid: mom_LB [20,40,60] × vol_threshold [0.5,0.7] × R [3,5,7] × N [3,4]
  = 3 × 2 × 3 × 2 = 36 combos
"""
...
VOL_THRESHOLDS = [0.5, 0.7]  # percentile of volume
REBAL_FREQS = [3, 5, 7]
N_POSITIONS = [3, 4]
...
def load_data():
    ...
    volumes = pd.DataFrame(volumes).sort_index().reindex(closes.index).fillna(0)
    return closes, volumes


def compute_signal(closes, volumes, mom_lb, vol_threshold):
    """
    Compute informed momentum: cumulative return using only high-volume days.

    For each asset, each day:
    1. Look back mom_lb days
    2. Identify days where volume > rolling median × vol_threshold percentile
    3. Sum returns only on those high-volume days
    """
    rets = closes.pct_change()

    # Rolling volume percentile threshold (per asset)
    vol_median = volumes.rolling(mom_lb * 2, min_periods=mom_lb).quantile(vol_threshold)

    # Mask: 1 if high-volume day, 0 otherwise
    high_vol_mask = (volumes > vol_median).astype(float)

    # Informed returns: return × mask (zero out low-volume days)
    informed_rets = rets * high_vol_mask

    # Rolling sum of informed returns = informed momentum
    signal = informed_rets.rolling(mom_lb, min_periods=int(mom_lb * 0.5)).sum()
...
def main():
    print("=" * 70)
    print("H-298: Informed Momentum Factor (High-Volume Filtered)")
    print("=" * 70)
    ...
    results = []
    total = len(MOM_LOOKBACKS) * len(VOL_THRESHOLDS) * len(REBAL_FREQS) * len(N_POSITIONS)
    print(f"\nRunning {total} parameter combinations...")

    for mom_lb, vol_thresh, rebal, n_pos in product(
        MOM_LOOKBACKS, VOL_THRESHOLDS, REBAL_FREQS, N_POSITIONS
    ):
        signal = compute_signal(closes, volumes, mom_lb, vol_thresh)
        ...
        label = f"MOM{mom_lb}_VT{vol_thresh}_R{rebal}_N{n_pos}"
        results.append({
            "label": label, "mom_lb": mom_lb, "vol_thresh": vol_thresh,
    ...
    # Walk-forward
    signal = compute_signal(closes, volumes, int(best["mom_lb"]), float(best["vol_thresh"]))
    ...
    # Save
    out = {
        "hypothesis": "H-298", "name": "Informed Momentum",
