...
Idea: The Close Location Value = (close - low) / (high - low) measures where
the close sits within the day's range. A CLV near 1.0 means closing near the high
(bullish), near 0.0 means closing near the low (bearish).

We compute the ROLLING AVERAGE CLV over a lookback window. Assets that consistently
close near their highs have sustained buying pressure. Unlike the earlier H-124 CLV
attempt which used raw CLV, we use a SMOOTHED, NORMALIZED version with a wider
parameter grid and both directions.

Signal:
  1. CLV = (close - low) / (high - low) for each day (0-1 scale).
  2. Rolling mean CLV over lookback window.
  3. Rank: high rolling CLV -> long (buying pressure), low -> short.
...
def load_daily_ohlc():
    ...
    common = closes.index.intersection(highs.index).intersection(lows.index)
    ...
    highs.index = highs.index.tz_localize(None) if highs.index.tzinfo is not None else highs.index
    lows.index = lows.index.tz_localize(None) if lows.index.tzinfo is not None else lows.index
    return closes, highs, lows


def compute_rolling_clv(closes, highs, lows, lookback):
    """Compute rolling average CLV for each asset."""
    hl_range = highs - lows
    hl_range = hl_range.replace(0, np.nan)
    clv = (closes - lows) / hl_range
    rolling_clv = clv.rolling(lookback, min_periods=lookback).mean()
    return rolling_clv
...
def backtest_clv(closes, highs, lows, lookback, rebal_freq, n, direction="high_clv_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    rolling_clv = compute_rolling_clv(closes, highs, lows, lookback).reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
...
def run_walk_forward(closes, highs, lows):
    ...
    for fold in range(WF_FOLDS):
        ...
        if oos_start < WF_TRAIN_MIN + 20:
            break
        tr_c, tr_h, tr_l = closes.iloc[:oos_start], highs.iloc[:oos_start], lows.iloc[:oos_start]
        oo_c, oo_h, oo_l = closes.iloc[oos_start:oos_end], highs.iloc[oos_start:oos_end], lows.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        r = backtest_clv(tr_c, tr_h, tr_l, lb, rf, nn, d)
                        ev = evaluate(r)
        ...
        oos_r = backtest_clv(oo_c, oo_h, oo_l, lb, rf, nn, d)
...
def run_split_half(closes, highs, lows):
    half = len(closes) // 2
    h1c, h1h, h1l = closes.iloc[:half], highs.iloc[:half], lows.iloc[:half]
    h2c, h2h, h2l = closes.iloc[half:], highs.iloc[half:], lows.iloc[half:]
    h1_sharpes, h2_sharpes = [], []
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_clv(h1c, h1h, h1l, lb, rf, nn, d))
                    e2 = evaluate(backtest_clv(h2c, h2h, h2l, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-228: Close Location Value (CLV) Persistence Factor")
    print("=" * 60)
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_clv(closes, highs, lows, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-228", "name": "CLV Persistence Factor",
    ...
    print("\n--- Stage 3: Split-Half ---")
    h1_best, h2_best, h1_mean, h2_mean = run_split_half(closes, highs, lows)
    ...
    best_r = backtest_clv(closes, highs, lows, int(best_row["lb"]), int(best_row["rf"]),
