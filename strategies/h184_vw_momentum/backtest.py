#!/usr/bin/env python3
"""
H-184: Volume-Weighted Return Momentum

Concept: Standard momentum weights all days equally. Volume-weighted momentum
weights each day's return by its relative volume (vol_t / avg_vol_20). This
emphasizes "high conviction" moves -- big up-moves on heavy volume count more.
The idea is that volume confirms price moves.

Different from:
  - H-012 (standard cross-sectional momentum -- equal-weight returns)
  - H-092 (earlier vol-weighted momentum attempt -- different implementation)
  - H-175 (net money flow -- directional volume flow, not return weighting)
  - H-177 (volume trend -- OBV slope, not return weighting)

Logic:
  For each asset on each rebalance day:
    1. Compute daily return = (close_t - close_{t-1}) / close_{t-1}
    2. Compute vol_ratio = volume_t / rolling_mean(volume, 20)
    3. Compute vw_momentum = rolling_sum(return * vol_ratio, lookback)
    4. Rank assets cross-sectionally by vw_momentum
    5. Two directions:
       - vwmom_long: Long highest vw-momentum (trend followers confirmed by volume),
                     Short lowest (weakest conviction moves)
       - vwmom_short: Opposite (contrarian -- overextended high-vol moves revert)
    6. Dollar-neutral, equal weight within legs

Transaction costs: 0.05% per side (0.10% round trip) on each rebalance.

Parameter grid:
  LB (lookback for vw_momentum sum): [10, 20, 30, 40, 60]
  Rebalance                        : [3, 5, 7]
  N (top/bottom)                   : [3, 4]
  Direction                        : [vwmom_long, vwmom_short]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["vwmom_long", "vwmom_short"]

# Transaction costs: 10bps round-trip = 5bps per side
COST_PER_SIDE = 0.0005

# Volume ratio rolling window (fixed at 20 for relative volume)
VOL_RATIO_WINDOW = 20
...
def backtest_vw_momentum(closes, volumes, lookback, rebal_freq, n, direction):
    """
    Volume-Weighted Return Momentum backtest.

    Factor:
      daily_return = pct_change(close)
      vol_ratio = volume / rolling_mean(volume, 20)
      vw_momentum = rolling_sum(daily_return * vol_ratio, lookback)

    Args:
        closes:     DataFrame of daily closes
        volumes:    DataFrame of daily volumes (aligned with closes)
        lookback:   rolling window for vw_momentum sum
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "vwmom_long" or "vwmom_short"
    Returns:
    ...
    # Pre-compute vol_ratio for all assets: vol_t / rolling_mean(vol, 20)
    vol_rolling_mean = vol_data.rolling(window=VOL_RATIO_WINDOW, min_periods=VOL_RATIO_WINDOW).mean()
    vol_ratio = vol_data / vol_rolling_mean

    # Pre-compute vw_return = daily_return * vol_ratio
    vw_return = returns * vol_ratio

    # Pre-compute vw_momentum = rolling_sum(vw_return, lookback)
    vw_momentum = vw_return.rolling(window=lookback, min_periods=lookback).sum()

    # Clean returns for portfolio simulation
    returns = returns.dropna(how="all")
    dates = returns.index

    warmup = max(lookback, VOL_RATIO_WINDOW) + 5
    ...
    for i in range(warmup, len(dates)):
        ...
        if weights.abs().sum() > 0:
            ...
            portfolio_rets.append({"date": date, "return": day_ret})

        # Rebalance check
        if i - last_rebal >= rebal_freq:
            # Get vw_momentum scores for this date
            if date not in vw_momentum.index:
                continue

            scores = vw_momentum.loc[date]
            valid_scores = scores.dropna()

            if len(valid_scores) < 2 * n:
                continue

            # Rank by vw_momentum
            ranked = valid_scores.sort_values(ascending=False)

            if direction == "vwmom_long":
                # Long highest vw-momentum, Short lowest
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # Contrarian: Long lowest, Short highest
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def run_walk_forward(closes, volumes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_vw_momentum(train_closes, train_volumes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_vw_momentum(oos_closes, oos_volumes, lb, rf, nn, best_direction)
...
def run_split_half(closes, volumes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_vw_momentum(h1_closes, h1_volumes, lb, rf, nn, best_direction)
                r2 = backtest_vw_momentum(h2_closes, h2_volumes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-184: Volume-Weighted Return Momentum")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_vw_momentum(closes, volumes, lb, rf, nn, d)
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-184",
            "name": "Volume-Weighted Return Momentum",
            "status": "REJECTED",
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_vw_momentum(closes, volumes, best_lb, best_rf, best_n, best_dir)
