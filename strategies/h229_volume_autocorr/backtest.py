...
Idea: Measure the first-order autocorrelation of daily volume over a rolling
window. Assets with high volume autocorrelation have persistent, predictable
flow patterns — potentially indicating institutional participation or systematic
trading. Low autocorrelation means erratic, news-driven volume.

We hypothesize that high volume autocorrelation (persistent flows) is bullish:
institutional buying/selling tends to persist and compress volatility.

Signal:
  1. Compute rolling first-order autocorrelation of daily volume over lookback.
  2. Rank cross-sectionally: high autocorr -> long, low -> short.
...
def compute_volume_autocorr(volumes, lookback):
    """Compute rolling first-order autocorrelation of volume for each asset."""
    autocorrs = {}
    for sym in volumes.columns:
        vol_series = volumes[sym].values
        n = len(vol_series)
        ac = np.full(n, np.nan)
        for i in range(lookback, n):
            window = vol_series[i - lookback: i]
            ...
            ac[i] = corr
        autocorrs[sym] = pd.Series(ac, index=volumes.index)
...
def backtest_vol_ac(closes, volumes, lookback, rebal_freq, n, direction="high_autocorr_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    vol_ac = compute_volume_autocorr(volumes, lookback).reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        r = backtest_vol_ac(tr_c, tr_v, lb, rf, nn, d)
                        ev = evaluate(r)
        ...
        oos_r = backtest_vol_ac(oo_c, oo_v, lb, rf, nn, d)
...
def run_split_half(closes, volumes):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_vol_ac(h1c, h1v, lb, rf, nn, d))
                    e2 = evaluate(backtest_vol_ac(h2c, h2v, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-229: Volume Autocorrelation Factor")
    print("=" * 60)
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_vol_ac(closes, volumes, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-229", "name": "Volume Autocorrelation Factor",
    ...
    best_r = backtest_vol_ac(closes, volumes, int(best_row["lb"]), int(best_row["rf"]),
