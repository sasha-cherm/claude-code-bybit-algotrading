...
Idea: Rank assets by the rate of change of open interest over a lookback window.
High OI growth = increasing speculative interest / new money flowing in.
Low OI growth = declining interest / position closing.

Different from:
- H-044 (OI-Price Divergence): measures MISMATCH between OI change and price change
- H-193 (OI-Price Momentum Divergence): measures alignment of OI and price momentum
- H-217 (Volume/OI Ratio): measures speculative activity as volume relative to OI level
This is pure OI momentum — does absolute OI growth predict cross-sectional returns?

Signal construction:
  1. Compute OI_growth = OI(t) / OI(t-lookback) - 1 for each asset
  2. Rank XS: long high OI growth (new interest), short low OI growth (declining)

Parameter grid:
  Lookback: [5, 10, 14, 20, 30]
  Rebal freq: [3, 5, 7]
  N: [3, 4]
  Direction: [high_oi_long, low_oi_long]
...
TICKERS = ["BTC", "ETH", "SOL", "SUI", "XRP", "DOGE", "AVAX", "LINK", "ADA", "DOT", "NEAR", "OP", "ARB", "ATOM"]

LOOKBACKS = [5, 10, 14, 20, 30]
REBALS = [3, 5, 7]
NS = [3, 4]
DIRECTIONS = ["high_oi_long", "low_oi_long"]
...
def load_data():
    ...
    # Load OI data
    oi_data = {}
    for sym, ticker in zip(ASSETS_ALL, TICKERS):
        path = oi_dir / f"{ticker}_oi_1d.parquet"
        if not path.exists():
            continue
        df = pd.read_parquet(path)
        if "openInterest" not in df.columns:
            continue
        oi = df["openInterest"].copy()
        if oi.index.tzinfo is not None:
            oi.index = oi.index.tz_localize(None)
        oi_data[sym] = oi
    oi_df = pd.DataFrame(oi_data).dropna(how="all").ffill().dropna()

    # Align
    common_idx = closes.index.intersection(oi_df.index)
    closes = closes.loc[common_idx]
    oi_df = oi_df.loc[common_idx]

    return closes, oi_df


def compute_oi_growth(oi_df, lookback):
    """Compute OI growth rate over lookback window."""
    growth = oi_df / oi_df.shift(lookback) - 1
    return growth
...
def run_walk_forward(closes, oi_df):
    n_total = len(closes)
    fold_results = []
    for fold in range(WF_FOLDS):
        ...
        full_oi = oi_df.iloc[:oos_end]
        full_sig = compute_oi_growth(full_oi, lb)
...
def run_split_half(closes, oi_df):
    half = len(closes) // 2
    h1c, h2c = closes.iloc[:half], closes.iloc[half:]
    h1oi, h2oi = oi_df.iloc[:half], oi_df.iloc[half:]
    h1_sharpes, h2_sharpes = [], []
    for lb in LOOKBACKS:
        h1_sig = compute_oi_growth(h1oi, lb)
        h2_sig = compute_oi_growth(h2oi, lb)
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
...
def main():
    print("=" * 60)
    print("  H-268: OI Growth Rate Factor")
    print("=" * 60)

    print("\nLoading daily data + OI data...")
    closes, oi_df = load_data()
    n_assets = len(closes.columns)
    n_oi_assets = len(oi_df.columns)
    n_days = len(closes)
    print(f"Loaded {n_assets} price assets, {n_oi_assets} OI assets, {n_days} aligned days")
    print(f"Date range: {closes.index[0]} -> {closes.index[-1]}")

    total_combos = len(LOOKBACKS) * len(REBALS) * len(NS) * len(DIRECTIONS)

    # Pre-compute signals
    print("\nPre-computing OI growth signals...")
    sig_cache = {}
    for lb in LOOKBACKS:
        sig_cache[lb] = compute_oi_growth(oi_df, lb)
        print(f"  LB={lb} done")
    ...
    # Dominant direction
    dir_counts = {"high_oi_long": 0, "low_oi_long": 0}
    for r in positive:
        for d in DIRECTIONS:
            if d in r["label"]:
                dir_counts[d] += 1
    ...
    wf = run_walk_forward(closes, oi_df)
    for f in wf:
    ...
    # Stage 3: Split-half
    print("\n--- Stage 3: Split-Half ---")
    h1_best, h2_best, h1_mean, h2_mean = run_split_half(closes, oi_df)
    ...
    rf = int(parts[1][1:])
    nn = int(parts[2][1:])
    d = "high_oi_long" if "high_oi_long" in best["label"] else "low_oi_long"
    sig = sig_cache[lb]
