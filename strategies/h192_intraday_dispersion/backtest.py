...
Concept: Using hourly data, compute the standard deviation of hourly returns
within each day, then average this "intraday dispersion" over a lookback window.
High intraday dispersion means the asset is choppy/volatile intraday even if
daily returns are moderate. Low intraday dispersion means smooth directional moves.

The factor is the ratio of intraday volatility (from hourly data) to daily
volatility, which captures the "choppiness" or "microstructure noise" level.
Assets with high intraday-to-daily vol ratio have noisy price action (mean-reverting
intraday); assets with low ratio have clean directional moves (trending intraday).

Direction options:
  - low_disp_long: Long low-dispersion assets (clean trends, institutional flow),
                   Short high-dispersion (noisy, choppy, retail-driven)
  - high_disp_long: Long high-dispersion assets (volatile = opportunity),
                    Short low-dispersion (quiet = dead money)

Different from:
  - H-019 (realized volatility -- uses daily returns only, not hourly)
  - H-059 (GK volatility -- uses OHLC range, not actual intraday returns)
  - H-182 (range factor -- uses high-low range width, not hourly return dispersion)
  - H-188 (vol asymmetry -- up vs down vol, not intraday microstructure)

Logic:
  For each asset on each rebalance day:
    1. Over lookback window of days, for each day compute std(hourly returns)
    2. Also compute std(daily returns) over same window
    3. intraday_dispersion_ratio = mean(daily_intraday_std) / daily_return_std
...
Parameter grid:
  LB (lookback days): [10, 20, 30, 40, 60]
  Rebalance          : [3, 5, 7]
  N (top/bottom)     : [3, 4]
  Direction           : [low_disp_long, high_disp_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["low_disp_long", "high_disp_long"]
...
def load_daily_data():
    ...
    # Strip timezone for alignment with intraday dispersion (groupby date is tz-naive)
...
def compute_intraday_dispersion(hourly_closes):
    """
    For each asset and each day, compute std of hourly returns within that day.
    ...
    # Group by date
    hourly_rets.index = pd.to_datetime(hourly_rets.index)
    daily_intraday_std = hourly_rets.groupby(hourly_rets.index.date).std()
    daily_intraday_std.index = pd.to_datetime(daily_intraday_std.index)

    return daily_intraday_std
...
def backtest_intraday_dispersion(closes, intraday_std, lookback, rebal_freq, n, direction):
    """
    Intraday Return Dispersion Factor backtest.
    """
    trade_cols = list(set(closes.columns) & set(intraday_std.columns))
    if len(trade_cols) < 2 * n:
        return None

    closes = closes[trade_cols]
    intraday_std = intraday_std[trade_cols]

    returns = closes.pct_change().dropna()

    # Align intraday_std to returns index (both should be daily)
    common_idx = returns.index.intersection(intraday_std.index)
    if len(common_idx) < lookback + 50:
        return None

    returns = returns.loc[common_idx]
    intraday_std_aligned = intraday_std.loc[common_idx]
    dates = returns.index
    warmup = lookback + 5
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            ...
            for sym in trade_cols:
                # Get lookback window of intraday stds
                intra_window = intraday_std_aligned[sym].iloc[i - lookback:i].values
                daily_rets_window = returns[sym].iloc[i - lookback:i].values

                if np.any(np.isnan(intra_window)) or np.any(np.isnan(daily_rets_window)):
                    continue

                # Mean intraday std over lookback
                mean_intra_std = np.mean(intra_window)

                # Daily return std over lookback
                daily_std = np.std(daily_rets_window)

                if daily_std < 1e-10:
                    continue

                # Dispersion ratio: intraday noise relative to daily moves
                disp_ratio = mean_intra_std / daily_std
                disp_scores[sym] = disp_ratio

            if len(disp_scores) < 2 * n:
                continue

            ranked = pd.Series(disp_scores).sort_values(ascending=True)
            # Low ratio = clean directional moves (smooth trends)
            # High ratio = choppy/noisy intraday action

            if direction == "low_disp_long":
                # Long clean-trending assets, Short noisy ones
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # high_disp_long: Long volatile/noisy, Short quiet
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def run_walk_forward(closes, intraday_std, best_direction):
    """6-fold walk-forward."""
    # Use common index
    common_idx = closes.index.intersection(intraday_std.index)
    closes_wf = closes.loc[common_idx]
    intraday_std_wf = intraday_std.loc[common_idx]
    ...
    for fold in range(WF_FOLDS):
        ...
        train_intra = intraday_std_wf.iloc[:oos_start]
        oos_closes = closes_wf.iloc[oos_start:oos_end]
        oos_intra = intraday_std_wf.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_intraday_dispersion(train_closes, train_intra, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        oos_r = backtest_intraday_dispersion(oos_closes, oos_intra, lb, rf, nn, best_direction)
...
def run_split_half(closes, intraday_std, best_direction):
    """Run full grid on each half."""
    common_idx = closes.index.intersection(intraday_std.index)
    closes_sh = closes.loc[common_idx]
    intraday_std_sh = intraday_std.loc[common_idx]

    half = len(closes_sh) // 2
    h1_closes = closes_sh.iloc[:half]
    h1_intra = intraday_std_sh.iloc[:half]
    h2_closes = closes_sh.iloc[half:]
    h2_intra = intraday_std_sh.iloc[half:]
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_intraday_dispersion(h1_closes, h1_intra, lb, rf, nn, best_direction)
                r2 = backtest_intraday_dispersion(h2_closes, h2_intra, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-192: Intraday Return Dispersion Factor")
    print("=" * 60)
    ...
    print("\nComputing intraday dispersion (hourly return std per day)...")
    intraday_std = compute_intraday_dispersion(hourly)
    print(f"Intraday std shape: {intraday_std.shape}")
    print(f"Intraday std date range: {intraday_std.index[0]} -> {intraday_std.index[-1]}")

    # Common columns between daily closes and intraday data
    common_assets = list(set(closes.columns) & set(intraday_std.columns))
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_intraday_dispersion(closes, intraday_std, lb, rf, nn, d)
    ...
    wf_results = run_walk_forward(closes, intraday_std, best_dir)
    ...
    sh_best_h1, sh_best_h2, sh_mean_h1, sh_mean_h2 = run_split_half(closes, intraday_std, best_dir)
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_intraday_dispersion(closes, intraday_std, best_lb, best_rf, best_n, best_dir)
