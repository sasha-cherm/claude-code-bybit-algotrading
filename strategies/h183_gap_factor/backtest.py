#!/usr/bin/env python3
"""
H-183: Gap Factor (Overnight Sentiment)

Concept: Compute the gap between each day's open and the previous day's close:
  gap = (open_t - close_{t-1}) / close_{t-1}
Average this gap over a rolling window. Assets with consistently positive
overnight gaps have bullish overnight sentiment. This is a known anomaly in
equities -- test if it transfers to crypto.

Different from:
  - H-108 (overnight gap reversal -- that trades the gap itself for reversal)
  - H-134 (gap reversal -- similar single-day gap reversal)
  - H-012 (momentum -- based on close-to-close returns, not open-close gaps)
The key insight is this captures persistent OVERNIGHT sentiment (averaged over
many days), not single-day gap events or close-to-close momentum.

Logic:
  For each asset on each rebalance day:
    1. Compute daily gap = (open_t - close_{t-1}) / close_{t-1}
    2. Compute rolling mean of gap over lookback window
    3. Rank assets cross-sectionally by mean gap
    4. Two directions:
       - pos_gap_long: Long top-N (most positive avg gap = bullish overnight),
                       Short bottom-N (most negative avg gap)
       - neg_gap_long: opposite (contrarian -- fading the overnight sentiment)
    5. Dollar-neutral, equal weight within legs

Transaction costs: 0.05% per side (0.10% round trip) on each rebalance.

Parameter grid:
  LB (lookback)   : [5, 10, 20, 30, 60]
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [pos_gap_long, neg_gap_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["pos_gap_long", "neg_gap_long"]
...
def load_daily_data():
    """Load 1d parquet files for all assets, return opens and closes DataFrames."""
    data_dir = ROOT / "data"
    opens_dict = {}
    closes_dict = {}
    for sym in ASSETS_ALL:
        ...
        if path.exists():
            ...
            if "open" in df.columns and "close" in df.columns and len(df) >= 200:
                opens_dict[sym] = df["open"]
                closes_dict[sym] = df["close"]
    opens = pd.DataFrame(opens_dict)
    closes = pd.DataFrame(closes_dict)
    # Align indices
    common_idx = opens.index.intersection(closes.index)
    opens = opens.loc[common_idx].dropna(how="all").ffill().dropna()
    closes = closes.loc[common_idx].reindex(opens.index).ffill().dropna()
    return opens, closes
...
def backtest_gap_factor(opens, closes, lookback, rebal_freq, n, direction):
    """
    Gap Factor backtest.

    Args:
        opens:      DataFrame of daily opens
        closes:     DataFrame of daily closes (aligned with opens)
        lookback:   rolling window for mean gap calculation
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "pos_gap_long" or "neg_gap_long"
    Returns:
    ...
    # Compute daily returns (close-to-close) for portfolio PnL
    returns = closes.pct_change().dropna()

    # Compute daily gap: (open_t - close_{t-1}) / close_{t-1}
    prev_close = closes.shift(1)
    daily_gap = (opens - prev_close) / prev_close
    daily_gap = daily_gap.reindex(returns.index)
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute rolling mean gap for each asset over lookback window
            gap_scores = {}
            for sym in trade_cols:
                if sym not in daily_gap.columns:
                    continue
                gap_window = daily_gap[sym].iloc[i - lookback:i].values
                # Skip if we have NaN
                if np.any(np.isnan(gap_window)):
                    continue
                mean_gap = np.mean(gap_window)
                gap_scores[sym] = mean_gap

            if len(gap_scores) < 2 * n:
                continue

            ranked = pd.Series(gap_scores).sort_values(ascending=True)
            # ranked: ascending by mean gap
            # Bottom = most negative mean gap (bearish overnight sentiment)
            # Top = most positive mean gap (bullish overnight sentiment)

            if direction == "pos_gap_long":
                # Long positive gap (top), Short negative gap (bottom)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
            else:
                # Contrarian: Long negative gap (bottom), Short positive gap (top)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
...
def run_walk_forward(opens, closes, best_direction):
    """
    ...
    for fold in range(WF_FOLDS):
        ...
        train_opens = opens.iloc[:oos_start]
        train_closes = closes.iloc[:oos_start]
        oos_opens = opens.iloc[oos_start:oos_end]
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_gap_factor(train_opens, train_closes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_gap_factor(oos_opens, oos_closes, lb, rf, nn, best_direction)
...
def run_split_half(opens, closes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_gap_factor(h1_opens, h1_closes, lb, rf, nn, best_direction)
                r2 = backtest_gap_factor(h2_opens, h2_closes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-183: Gap Factor (Overnight Sentiment)")
    print("=" * 60)

    print("\nLoading daily open + close data...")
    opens, closes = load_daily_data()
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_gap_factor(opens, closes, lb, rf, nn, d)
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-183",
            "name": "Gap Factor (Overnight Sentiment)",
            "status": "REJECTED",
    ...
    wf_results = run_walk_forward(opens, closes, best_dir)
    ...
    sh_best_h1, sh_best_h2, sh_mean_h1, sh_mean_h2 = run_split_half(opens, closes, best_dir)
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_gap_factor(opens, closes, best_lb, best_rf, best_n, best_dir)
    ...
    result_json = {
        ...
        "correlations": {
            "H-012_momentum": float(corr_h012) if not np.isnan(corr_h012) else None,
            "H-076_efficiency": float(corr_h076) if not np.isnan(corr_h076) else None,
            "H-160_trend_quality": float(corr_h160) if not np.isnan(corr_h160) else None,
        },
