#!/usr/bin/env python3
"""
H-182: High-Low Range Factor

Concept: Use normalized intraday range (High - Low) / Close as a cross-sectional
signal. Assets with narrow ranges may be in quiet accumulation, while wide ranges
indicate panic/volatility.

Different from:
  - H-019 (close-close volatility -- uses closing prices only)
  - H-107/H-142 (range compression -- ratio of recent vs past ranges, not level)
  - H-129 (intraday vol -- uses intraday returns, not HL range)
The key insight is this captures the RAW intraday spread as a fraction of price,
directly measuring how much the market explored in a day.

Logic:
  For each asset on each rebalance day:
    1. Compute rolling mean of (high - low) / close over lookback window
    2. Rank assets cross-sectionally by normalized range
    3. Two directions:
       - narrow_long: Long bottom-N (narrowest range = quiet accumulation),
                      Short top-N (widest range = panic/volatility)
       - wide_long: opposite
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [narrow_long, wide_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["narrow_long", "wide_long"]
...
def backtest_range_factor(closes, highs, lows, lookback, rebal_freq, n, direction):
    """
    High-Low Range Factor backtest.
    ...
    Args:
        ...
        lookback:   rolling window for normalized range calculation
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "narrow_long" or "wide_long"
    Returns:
    ...
    # Compute daily normalized range: (high - low) / close
    norm_range = (highs - lows) / closes
    norm_range = norm_range.reindex(returns.index)
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute rolling mean of normalized range for each asset
            range_scores = {}
            for sym in trade_cols:
                if sym not in norm_range.columns:
                    continue
                range_window = norm_range[sym].iloc[i - lookback:i].values
                # Skip if we have NaN
                if np.any(np.isnan(range_window)):
                    continue
                avg_range = np.mean(range_window)
                if avg_range < 1e-10:
                    continue
                range_scores[sym] = avg_range

            if len(range_scores) < 2 * n:
                continue

            ranked = pd.Series(range_scores).sort_values(ascending=True)
            # ranked: ascending by normalized range
            # Bottom = lowest range = narrowest (quiet accumulation)
            # Top = highest range = widest (panic/volatility)

            if direction == "narrow_long":
                # Long narrow (low range), Short wide (high range)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # Long wide (high range), Short narrow (low range)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def run_walk_forward(closes, highs, lows, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_range_factor(train_closes, train_highs, train_lows, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_range_factor(oos_closes, oos_highs, oos_lows, lb, rf, nn, best_direction)
...
def run_split_half(closes, highs, lows, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_range_factor(h1_closes, h1_highs, h1_lows, lb, rf, nn, best_direction)
                r2 = backtest_range_factor(h2_closes, h2_highs, h2_lows, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-182: High-Low Range Factor")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_range_factor(closes, highs, lows, lb, rf, nn, d)
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_range_factor(closes, highs, lows, best_lb, best_rf, best_n, best_dir)
