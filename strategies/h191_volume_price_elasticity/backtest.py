...
Concept: Measure how much price moves per unit of volume (dollar-normalized).
For each asset, compute: elasticity = abs(return) / (volume * close / avg_notional)
averaged over a lookback window. This normalizes for absolute price/volume differences.

Assets with LOW elasticity (price barely moves despite high volume) suggest
institutional absorption, strong support/resistance, or accumulation.
Assets with HIGH elasticity (price moves a lot on little volume) suggest
thin liquidity, retail-driven, or fragile positioning.

Direction options:
  - low_elast_long: Long low-elasticity assets (institutional, absorbed demand),
                    Short high-elasticity assets (fragile, retail)
  - high_elast_long: Long high-elasticity assets (momentum amplification),
                     Short low-elasticity assets (stuck/heavy)

Different from:
  - H-021 (volume momentum -- uses raw volume trend, not price-per-volume)
  - H-012 (momentum -- uses returns only, not volume-adjusted)
  - H-169 (volume-weighted momentum -- weights returns BY volume, not ratio of move/volume)
  - H-181 (volume CV -- measures volume variability, not price sensitivity to volume)

Logic:
  For each asset on each rebalance day:
    1. Over lookback window, for each day compute abs_ret / normalized_volume
       where normalized_volume = volume * close / median(volume * close)
    2. Take median elasticity over the window (robust to outliers)
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [low_elast_long, high_elast_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["low_elast_long", "high_elast_long"]
...
def backtest_vol_price_elasticity(closes, volumes, lookback, rebal_freq, n, direction):
    """
    Volume-Price Elasticity Factor backtest.
    """
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            ...
            for sym in trade_cols:
                # Get lookback window of returns and notional volumes
                ret_window = returns[sym].iloc[i - lookback:i].values
                close_window = closes.loc[returns.index, sym].iloc[i - lookback:i].values
                vol_window = volumes_aligned[sym].iloc[i - lookback:i].values

                if np.any(np.isnan(ret_window)) or np.any(np.isnan(vol_window)):
                    continue
                if np.any(np.isnan(close_window)):
                    continue

                # Compute notional volume for each day
                notional_vol = vol_window * close_window
                median_notional = np.median(notional_vol)
                if median_notional < 1e-10:
                    continue

                # Normalized volume = notional / median (so it's ~1 on average)
                norm_vol = notional_vol / median_notional

                # Elasticity = abs(return) / normalized_volume for each day
                # Avoid division by zero
                valid_mask = norm_vol > 0.01
                if valid_mask.sum() < lookback * 0.5:
                    continue

                daily_elast = np.abs(ret_window[valid_mask]) / norm_vol[valid_mask]
                # Use median for robustness against outlier days
                elast_scores[sym] = float(np.median(daily_elast))

            if len(elast_scores) < 2 * n:
                continue

            ranked = pd.Series(elast_scores).sort_values(ascending=True)
            # ranked ascending: bottom = lowest elasticity (institutional/absorbed)
            #                   top = highest elasticity (fragile/retail)

            if direction == "low_elast_long":
                # Long low-elasticity (institutional), Short high-elasticity
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # high_elast_long: Long high-elasticity, Short low-elasticity
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
def run_walk_forward(closes, volumes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_vol_price_elasticity(train_closes, train_volumes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        oos_r = backtest_vol_price_elasticity(oos_closes, oos_volumes, lb, rf, nn, best_direction)
...
def run_split_half(closes, volumes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_vol_price_elasticity(h1_closes, h1_volumes, lb, rf, nn, best_direction)
                r2 = backtest_vol_price_elasticity(h2_closes, h2_volumes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-191: Volume-Price Elasticity Factor")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_vol_price_elasticity(closes, volumes, lb, rf, nn, d)
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_vol_price_elasticity(closes, volumes, best_lb, best_rf, best_n, best_dir)
