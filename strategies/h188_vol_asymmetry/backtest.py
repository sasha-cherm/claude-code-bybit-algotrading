#!/usr/bin/env python3
"""
H-188: Return-Volume Asymmetry Factor

Concept: For each asset, compute the ratio of average volume on UP days vs
average volume on DOWN days over a rolling window. High ratio = volume confirms
bullish moves more strongly. Low ratio = volume concentrated on down moves.

Different from:
  - H-021 (volume momentum -- total volume change, not directional)
  - H-167 (volume-price correlation -- continuous correlation, not discrete up/down days)
  - H-074 (volume divergence -- price vs volume trend divergence)

Logic:
  For each asset on each rebalance day:
    1. Over a lookback window, compute avg volume on days with return > 0 (up days)
       and avg volume on days with return <= 0 (down days).
    2. Signal = avg_up_vol / avg_down_vol
       (skip asset if fewer than 3 up or down days in the window)
    3. Rank assets cross-sectionally by this ratio
    4. Two directions:
       - up_vol_long: Long top-N (highest ratio = bullish conviction),
                      Short bottom-N (lowest ratio = bearish conviction)
       - down_vol_long: opposite
...
Parameter grid:
  ...
  Rebalance        : [3, 5, 7]
  N (top/bottom)   : [3, 4]
  Direction         : [up_vol_long, down_vol_long]
...
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["up_vol_long", "down_vol_long"]
...
def backtest_vol_asymmetry(closes, volumes, lookback, rebal_freq, n, direction):
    """
    Return-Volume Asymmetry Factor backtest.
    ...
    Args:
        ...
        direction:  "up_vol_long" or "down_vol_long"
    Returns:
    ...
    volumes = volumes.reindex(returns.index)
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute volume asymmetry ratio for each asset
            asymmetry_scores = {}
            for sym in trade_cols:
                if sym not in returns.columns or sym not in volumes.columns:
                    continue
                ret_window = returns[sym].iloc[i - lookback:i].values
                vol_window = volumes[sym].iloc[i - lookback:i].values

                # Skip if we have NaN
                if np.any(np.isnan(ret_window)) or np.any(np.isnan(vol_window)):
                    continue

                up_mask = ret_window > 0
                down_mask = ret_window <= 0

                n_up = up_mask.sum()
                n_down = down_mask.sum()

                # Skip if fewer than 3 up or down days
                if n_up < 3 or n_down < 3:
                    continue

                avg_up_vol = vol_window[up_mask].mean()
                avg_down_vol = vol_window[down_mask].mean()

                if avg_down_vol < 1e-10:
                    continue

                asymmetry_scores[sym] = avg_up_vol / avg_down_vol

            if len(asymmetry_scores) < 2 * n:
                continue

            ranked = pd.Series(asymmetry_scores).sort_values(ascending=True)
            # ranked: ascending by up/down volume ratio
            # Bottom = lowest ratio = down-day volume dominates
            # Top = highest ratio = up-day volume dominates

            if direction == "up_vol_long":
                # Long high ratio (bullish conviction), Short low ratio
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
            else:
                # Long low ratio, Short high ratio
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
...
def run_walk_forward(closes, volumes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_vol_asymmetry(train_closes, train_volumes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_vol_asymmetry(oos_closes, oos_volumes, lb, rf, nn, best_direction)
...
def run_split_half(closes, volumes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_vol_asymmetry(h1_closes, h1_volumes, lb, rf, nn, best_direction)
                r2 = backtest_vol_asymmetry(h2_closes, h2_volumes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-188: Return-Volume Asymmetry Factor")
    print("=" * 60)
    ...
    for lb, rf, nn, d in valid_grid:
        count += 1
        r = backtest_vol_asymmetry(closes, volumes, lb, rf, nn, d)
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-188",
            "name": "Return-Volume Asymmetry Factor",
            "status": "REJECTED",
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_vol_asymmetry(closes, volumes, best_lb, best_rf, best_n, best_dir)
