#!/usr/bin/env python3
"""
H-253: Return Entropy Factor

Idea: Compute the Shannon entropy of each asset's return distribution over
a rolling window. Discretize returns into bins and measure information content.
Low entropy = concentrated/predictable returns, high entropy = dispersed/random.

This is different from:
- Volatility (H-019): std dev measures spread, entropy measures distributional shape
- Efficiency (H-076): path linearity, not distribution randomness
- Range (H-182): high-low range, not return distribution character
- Hurst (H-251): fractal scaling, not distributional entropy

Low-entropy assets have concentrated return distributions — their returns cluster
in fewer bins, suggesting more predictable behavior. High-entropy assets are
more random/uniformly distributed.

Signal construction:
  1. For each asset, take N-day rolling returns.
  2. Discretize into K bins (e.g., K=10 equal-width bins based on min/max).
  3. Compute Shannon entropy: H = -sum(p_i * log(p_i)) for non-zero bins.
  4. Normalize by log(K) to get entropy in [0, 1].
  5. Rank XS: long low-entropy (predictable), short high-entropy (random).
...
Parameter grid:
  ...
  Rebal freq: [3, 5, 7]
  N         : [3, 4]
  Direction : [low_entropy_long, high_entropy_long]
  Total: 4 x 2 x 3 x 2 x 2 = 96 combos
"""
...
REBALS     = [3, 5, 7]
NS         = [3, 4]
DIRECTIONS = ["low_entropy_long", "high_entropy_long"]
...
def compute_entropy(closes, lookback, n_bins):
    """Compute rolling normalized Shannon entropy for each asset."""
    rets = closes.pct_change().dropna()
    cols = list(rets.columns)
    col_idx = {c: j for j, c in enumerate(cols)}
    ent_arr = np.full((len(rets), len(cols)), np.nan)
    max_entropy = np.log(n_bins)  # for normalization
    ...
    for col in cols:
        ...
        for i in range(lookback, len(vals)):
            ...
            if mx - mn < 1e-12:
                ent_arr[i, j] = 0.0
                continue
            counts, _ = np.histogram(window, bins=n_bins, range=(mn, mx))
            probs = counts / counts.sum()
            probs = probs[probs > 0]
            h = -np.sum(probs * np.log(probs))
            ent_arr[i, j] = h / max_entropy if max_entropy > 0 else 0.0

    return pd.DataFrame(ent_arr, index=rets.index, columns=cols)
...
def backtest_entropy(closes, entropy_vals, rebal_freq, n, direction="low_entropy_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    entropy_vals = entropy_vals.reindex(returns.index)
    dates = returns.index
    ...
    for i in range(1, len(dates)):
        ...
        if i - last_rebal < rebal_freq:
            continue
        cv = entropy_vals.iloc[i]
        valid = cv.dropna()
        if len(valid) < 2 * n:
            continue
        if direction == "low_entropy_long":
            ranked = valid.sort_values(ascending=True)  # lowest entropy first
        else:
            ranked = valid.sort_values(ascending=False)  # highest entropy first
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for nb in N_BINS:
                ent = compute_entropy(tr_c, lb, nb)
                for rf in REBALS:
                    for nn in NS:
                        for d in DIRECTIONS:
                            r = backtest_entropy(tr_c, ent, rf, nn, d)
                            ev = evaluate(r)
                            if ev and ev["sharpe"] > best_sharpe:
                                best_sharpe = ev["sharpe"]
                                best_params = (lb, nb, rf, nn, d)
        if best_params is None:
            fold_results.append({"fold": fold + 1, "oos_sharpe": None})
            continue
        lb, nb, rf, nn, d = best_params
        oos_ent = compute_entropy(closes.iloc[:oos_end], lb, nb)
        oos_ent = oos_ent.iloc[oos_start:oos_end]
        oos_r = backtest_entropy(oo_c, oos_ent, rf, nn, d)
        oos_ev = evaluate(oos_r)
        fold_results.append({
            "fold": fold + 1, "is_params": f"LB{lb}_B{nb}_R{rf}_N{nn}_{d}",
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        for nb in N_BINS:
            h1_ent = compute_entropy(h1c, lb, nb)
            h2_ent = compute_entropy(h2c, lb, nb)
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        e1 = evaluate(backtest_entropy(h1c, h1_ent, rf, nn, d))
                        e2 = evaluate(backtest_entropy(h2c, h2_ent, rf, nn, d))
                        if e1:
...
def main():
    print("=" * 60)
    print("  H-253: Return Entropy Factor")
    print("=" * 60)
    ...
    total_combos = len(LOOKBACKS) * len(N_BINS) * len(REBALS) * len(NS) * len(DIRECTIONS)

    # Pre-compute entropy for all lookback/bin combos
    print("\nPre-computing entropy values...")
    ent_cache = {}
    for lb in LOOKBACKS:
        for nb in N_BINS:
            print(f"  Computing LB={lb}, bins={nb}...")
            ent_cache[(lb, nb)] = compute_entropy(closes, lb, nb)

    # Show average entropy per asset (LB=30, bins=12)
    if (30, 12) in ent_cache:
        print("\nAvg normalized entropy per asset (LB=30, bins=12):")
        avg_e = ent_cache[(30, 12)].mean().sort_values()
        for sym, val in avg_e.items():
            if not np.isnan(val):
                print(f"  {sym}: {val:.4f}")
        ...
        for nb in N_BINS:
            ...
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        count += 1
                        r = backtest_entropy(closes, ent, rf, nn, d)
                        ev = evaluate(r, f"LB{lb}_B{nb}_R{rf}_N{nn}_{d}")
                        if ev:
                            ev["lb"], ev["nb"], ev["rf"], ev["n"], ev["dir"] = lb, nb, rf, nn, d
    ...
    for _, row in df_sorted.head(5).iterrows():
        print(f"    LB{int(row['lb'])}_B{int(row['nb'])}_R{int(row['rf'])}_N{int(row['n'])}_{row['dir']}: "
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-253", "name": "Return Entropy Factor",
    ...
    best_ent = ent_cache[(int(best_row["lb"]), int(best_row["nb"]))]
    best_r = backtest_entropy(closes, best_ent, int(best_row["rf"]),
    ...
    result = {
        ...
        "is_positive_rate": round(pct_pos, 1), "is_mean_sharpe": round(mean_shp, 3),
        "best_dir": best_dir, "best_dir_pct": round(best_dir_pct, 1),
        "best_params": {"lookback": int(best_row["lb"]), "n_bins": int(best_row["nb"]),
                        "rebal": int(best_row["rf"]), "n": int(best_row["n"]),
                        "dir": best_row["dir"]},
        "best_sharpe": round(float(best_row["sharpe"]), 3),
