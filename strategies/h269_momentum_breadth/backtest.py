#!/usr/bin/env python3
"""
H-269: Momentum Breadth Factor (% Positive Days)

Idea: Instead of total return over lookback (standard momentum), count the fraction
of positive-return days. This is a "breadth" measure of momentum quality. An asset
with 70% positive days has more consistent uptrend than one with 1 big up day.

Different from:
- H-012 (Momentum): uses total return, not hit rate
- H-052 (Return Persistence): uses autocorrelation of returns
- H-133 (Consecutive Direction): uses max consecutive same-direction days
- H-166 (Return Persistence): uses different persistence metric, was rejected

Signal construction:
  1. For each asset, compute fraction of positive return days over lookback window
  2. Rank XS: long high breadth (consistent uptrend), short low breadth

Parameter grid:
  Lookback: [10, 15, 20, 30, 60]
  Rebal freq: [3, 5, 7]
  N: [3, 4]
  Direction: [high_breadth_long, low_breadth_long]
...
REBALS = [3, 5, 7]
NS = [3, 4]
DIRECTIONS = ["high_breadth_long", "low_breadth_long"]
...
def compute_momentum_breadth(closes, lookback):
    """Fraction of positive return days over lookback window."""
    rets = closes.pct_change()
    positive = (rets > 0).astype(float)
    breadth = positive.rolling(lookback, min_periods=lookback).mean()
    return breadth
...
def backtest_factor(closes, signal_df, rebal_freq, n, direction):
    ...
    for i in range(1, len(dates)):
        ...
        if len(valid) < 2 * n:
            continue
        if direction == "high_breadth_long":
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            sig = compute_momentum_breadth(tr_c, lb)
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
        ...
        full_sig = compute_momentum_breadth(closes.iloc[:oos_end], lb)
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        h1_sig = compute_momentum_breadth(h1c, lb)
        h2_sig = compute_momentum_breadth(h2c, lb)
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
...
def main():
    print("=" * 60)
    print("  H-269: Momentum Breadth Factor (% Positive Days)")
    print("=" * 60)
    ...
    # Pre-compute signals
    print("\nPre-computing breadth signals...")
    sig_cache = {}
    for lb in LOOKBACKS:
        sig_cache[lb] = compute_momentum_breadth(closes, lb)
        print(f"  LB={lb} done")
    ...
    # Dominant direction
    dir_counts = {"high_breadth_long": 0, "low_breadth_long": 0}
    for r in positive:
        for d in DIRECTIONS:
            if d in r["label"]:
                dir_counts[d] += 1
    ...
    rf = int(parts[1][1:])
    nn = int(parts[2][1:])
    d = "high_breadth_long" if "high_breadth_long" in best["label"] else "low_breadth_long"
    sig = sig_cache[lb]
...
def save_results(all_results, is_pct, dom_dir, dom_pct, best, wf, split_half, corr_h012):
    out = {
        "hypothesis": "H-269",
        "name": "Momentum Breadth Factor (% Positive Days)",
