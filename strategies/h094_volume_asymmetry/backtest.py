"""
H-094: Volume Asymmetry Factor (Buy vs Sell Volume)

Compare volume on up-days vs down-days over a lookback period.
- up_volume = sum of volume on days where close > prev_close over lookback
- down_volume = sum of volume on days where close < prev_close over lookback
- volume_asymmetry = (up_volume - down_volume) / (up_volume + down_volume)
- Value between -1 and +1. Positive = more volume on up days (accumulation).
- Long top N (strongest buy pressure), short bottom N (strongest sell pressure)
...
# Parameter grid
LOOKBACKS = [10, 20, 30, 40, 60]       # volume asymmetry window
...
def volume_asymmetry(closes, volumes, lookback):
    """
    Compute volume asymmetry factor for each asset/day.
    volume_asymmetry = (up_volume - down_volume) / (up_volume + down_volume)
    where up_volume = sum of volume on days where close > prev_close,
    and down_volume = sum of volume on days where close < prev_close.
    Returns a value between -1 and +1. Positive = accumulation (buy pressure).
    """
    # Determine up/down days: close > prev_close
    daily_change = closes.diff()
    up_mask = (daily_change > 0).astype(float)
    down_mask = (daily_change < 0).astype(float)

    # Volume on up days / down days
    up_vol = (volumes * up_mask).rolling(lookback, min_periods=lookback).sum()
    down_vol = (volumes * down_mask).rolling(lookback, min_periods=lookback).sum()

    total = up_vol + down_vol
    asymmetry = (up_vol - down_vol) / total

    # Replace inf/nan with NaN
    asymmetry = asymmetry.replace([np.inf, -np.inf], np.nan)
    return asymmetry
...
def run_full_scan(closes, volumes):
    ...
    print("\n" + "=" * 70)
    print("H-094: VOLUME ASYMMETRY FACTOR -- Full Parameter Scan")
    print("=" * 70)
    ...
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking = volume_asymmetry(closes, volumes, lookback)
...
def run_train_test_split(closes, volumes, split_ratio=0.7):
    ...
    train_results = []
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking = volume_asymmetry(train_closes, train_volumes, lookback)
        warmup = lookback + 5
    ...
    test_ranking = volume_asymmetry(test_closes, test_volumes, lookback)
    warmup = lookback + 5
...
def run_split_half(closes, volumes):
    ...
    results_h2 = []
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking1 = volume_asymmetry(half1_closes, half1_volumes, lookback)
        warmup = lookback + 5
        res1 = run_xs_factor(half1_closes, ranking1, rebal, n_long, warmup=warmup)

        ranking2 = volume_asymmetry(half2_closes, half2_volumes, lookback)
...
def compute_h012_correlation(closes, volumes, lookback, rebal, n_long):
    """Compute daily return correlation between volume asymmetry factor and H-012 momentum."""
    print(f"\n  Correlation with H-012 (60d Momentum)")

    # Run volume asymmetry factor
    va_ranking = volume_asymmetry(closes, volumes, lookback)
    warmup_va = lookback + 5
    res_va = run_xs_factor(closes, va_ranking, rebal, n_long, warmup=warmup_va)
    ...
    rets_va = eq_va.pct_change().dropna()
    rets_mom = eq_mom.pct_change().dropna()

    common = rets_va.index.intersection(rets_mom.index)
    if len(common) < 50:
        print("    Insufficient overlap for correlation")
        return 0.0

    corr = rets_va.loc[common].corr(rets_mom.loc[common])
    print(f"    Daily return correlation: {corr:.3f}")
    print(f"    H-012 Momentum:       Sharpe {res_mom['sharpe']:.3f}, Ann {res_mom['annual_ret']:.1%}")
    print(f"    H-094 Vol Asymmetry:  Sharpe {res_va['sharpe']:.3f}, Ann {res_va['annual_ret']:.1%}")
    return round(corr, 3)
...
if __name__ == "__main__":
    print("H-094: Volume Asymmetry Factor (Buy vs Sell Volume)")
    print("=" * 70)
    ...
    # ===== 9. Summary =====
    print("\n" + "=" * 70)
    print("SUMMARY: H-094 Volume Asymmetry Factor (Buy vs Sell Volume)")
    print("=" * 70)
    ...
    results_data = {
        "hypothesis": "H-094",
        "name": "Volume Asymmetry Factor (Buy vs Sell Volume)",
        "description": "Cross-sectional factor: long assets with buy-volume dominance, short assets with sell-volume dominance",
