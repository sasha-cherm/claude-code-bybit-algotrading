...
Idea: Measure the rolling standard deviation of each asset's beta to BTC.
Assets with unstable beta are undergoing regime transitions — their
relationship with BTC is changing, which creates pricing uncertainty.

Hypothesis: Assets with LOW beta instability (stable relationship with BTC)
are "resolved" and attract systematic allocators. HIGH instability assets
are in transition and may misprice.

This is distinct from:
- H-024/H-238 (level of beta, not stability of beta)
- H-019 (total vol, not beta stability)
- H-138 (correlation fragility — similar concept but measures correlation
  instability, not beta instability; also H-138 was rejected)

Signal:
  1. Compute rolling 20d beta of each asset vs BTC
  2. Compute rolling std of that beta over outer window
  3. Rank cross-sectionally: low instability -> long, high -> short

Parameter grid:
  Beta window   : [10, 20, 30]
  Outer window  : [10, 20, 30, 40]
  Rebal freq    : [3, 5, 7]
  N             : [3, 4]
  Direction     : [low_instab_long, high_instab_long]
...
BETA_WINDOWS = [10, 20, 30]
OUTER_WINDOWS = [10, 20, 30, 40]
REBALS     = [3, 5, 7]
NS         = [3, 4]
DIRECTIONS = ["low_instab_long", "high_instab_long"]
...
def compute_beta_instability(closes, beta_window, outer_window):
    """Compute rolling std of rolling beta vs BTC for each non-BTC asset."""
    ...
    for sym in returns.columns:
        if sym == "BTC/USDT":
            # BTC beta to itself is always 1, instability = 0
            # Assign NaN so it's excluded from ranking
            result[sym] = pd.Series(np.nan, index=returns.index)
            continue
        ...
        for i in range(beta_window, n):
            window_btc = btc_ret.iloc[i - beta_window:i].values
            window_asset = asset_ret.iloc[i - beta_window:i].values

            if np.any(np.isnan(window_btc)) or np.any(np.isnan(window_asset)):
                continue
            var_btc = np.var(window_btc)
            if var_btc < 1e-16:
                continue
            cov = np.cov(window_asset, window_btc)[0, 1]
            betas[i] = cov / var_btc

        beta_series = pd.Series(betas, index=returns.index)
        # Rolling std of beta
        instab = beta_series.rolling(outer_window, min_periods=max(outer_window // 2, 5)).std()
        result[sym] = instab
...
def backtest_factor(closes, beta_window, outer_window, rebal_freq, n, direction="low_instab_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    signal = compute_beta_instability(closes, beta_window, outer_window).reindex(returns.index)
    dates = returns.index
    warmup = beta_window + outer_window + 5
    ...
    for i in range(warmup, len(dates)):
        ...
        if len(valid) < 2 * n:
            continue
        if direction == "low_instab_long":
            ranked = valid.sort_values(ascending=True)  # low instab first
        else:
            ranked = valid.sort_values(ascending=False)  # high instab first
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        for bw in BETA_WINDOWS:
            for ow in OUTER_WINDOWS:
                for rf in REBALS:
                    for nn in NS:
                        for d in DIRECTIONS:
                            r = backtest_factor(tr_c, bw, ow, rf, nn, d)
                            ev = evaluate(r)
                            if ev and ev["sharpe"] > best_sharpe:
                                best_sharpe = ev["sharpe"]
                                best_params = (bw, ow, rf, nn, d)
        if best_params is None:
            fold_results.append({"fold": fold + 1, "oos_sharpe": None})
            continue
        bw, ow, rf, nn, d = best_params
        oos_r = backtest_factor(oo_c, bw, ow, rf, nn, d)
        oos_ev = evaluate(oos_r)
        fold_results.append({
            "fold": fold + 1, "is_params": f"BW{bw}_OW{ow}_R{rf}_N{nn}_{d}",
    ...
    for bw in BETA_WINDOWS:
        for ow in OUTER_WINDOWS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        e1 = evaluate(backtest_factor(h1c, bw, ow, rf, nn, d))
                        e2 = evaluate(backtest_factor(h2c, bw, ow, rf, nn, d))
                        if e1:
...
def main():
    print("=" * 60)
    print("  H-240: Beta Instability Factor")
    print("=" * 60)
    ...
    total_combos = len(BETA_WINDOWS) * len(OUTER_WINDOWS) * len(REBALS) * len(NS) * len(DIRECTIONS)
        ...
        for ow in OUTER_WINDOWS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        count += 1
                        r = backtest_factor(closes, bw, ow, rf, nn, d)
                        ev = evaluate(r, f"BW{bw}_OW{ow}_R{rf}_N{nn}_{d}")
                        if ev:
                            ev["bw"], ev["ow"], ev["rf"], ev["n"], ev["dir"] = bw, ow, rf, nn, d
    ...
    for _, row in df_sorted.head(5).iterrows():
        print(f"    BW{int(row['bw'])}_OW{int(row['ow'])}_R{int(row['rf'])}_N{int(row['n'])}_{row['dir']}: "
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-240", "name": "Beta Instability Factor",
    ...
    best_r = backtest_factor(closes, int(best_row["bw"]), int(best_row["ow"]),
    ...
    result = {
        ...
        "is_positive_rate": round(pct_pos, 1), "is_mean_sharpe": round(mean_shp, 3),
        "best_dir": best_dir, "best_dir_pct": round(best_dir_pct, 1),
        "best_params": {"beta_window": int(best_row["bw"]), "outer_window": int(best_row["ow"]),
