"""
H-116: Hurst Exponent Factor (Cross-Sectional)

Rank assets by estimated Hurst exponent over a rolling window.
Assets with H > 0.5 tend to trend (LONG), H < 0.5 tend to mean-revert (SHORT).

The Hurst exponent captures an asset's intrinsic tendency to trend, independent
of the direction or magnitude of recent returns. This is conceptually different
from momentum (H-012) — an asset can have zero momentum but high Hurst if it's
been trending in both directions. It's also different from autocorrelation (H-115)
which measures lag-1 serial correlation; Hurst captures multi-scale persistence.

Estimation method: Rescaled range (R/S) method.

Parameter grid (48 combos):
  Lookback windows : [30, 40, 60, 80] days
...
def estimate_hurst_rs(series: np.ndarray) -> float:
    ...
    for size in [n // 4, n // 3, n // 2, n]:
        ...
        for b in range(n_blocks):
            ...
            if len(block) < 10:
                continue
            mean_r = np.mean(block)
            deviate = np.cumsum(block - mean_r)
...
def compute_hurst_matrix(returns: pd.DataFrame, lookback: int) -> pd.DataFrame:
    """Rolling Hurst exponent for each asset."""
    n_days = len(returns)
    n_assets = len(returns.columns)
    hurst_vals = np.full((n_days, n_assets), np.nan)
    ret_arr = returns.values

    for i in range(lookback, n_days):
        for j in range(n_assets):
            window = ret_arr[i - lookback:i, j]
            valid = window[~np.isnan(window)]
            if len(valid) >= 20:
                hurst_vals[i, j] = estimate_hurst_rs(valid)

    return pd.DataFrame(hurst_vals, index=returns.index, columns=returns.columns)


def run_hurst_factor(
    closes: pd.DataFrame,
    lookback: int,
    rebal_freq: int,
    n_long: int,
    ...
    if pre_computed is not None:
        hurst_matrix = pre_computed
    else:
        hurst_matrix = compute_hurst_matrix(returns, lookback)
...
def run_full_scan(closes: pd.DataFrame):
    print("\n" + "=" * 72)
    print("H-116: HURST EXPONENT FACTOR -- Full Param Scan")
    print("=" * 72)
    ...
    print(f"  Strategy : LONG high Hurst (trending), SHORT low Hurst (mean-reverting)")

    returns = closes.pct_change()

    print("\n  Pre-computing Hurst matrices...")
    hurst_cache = {}
    for lb in LOOKBACKS:
        print(f"    Lookback {lb}d ...", end=" ", flush=True)
        hurst_cache[lb] = compute_hurst_matrix(returns, lb)
        print("done")

    results = []
    print(f"\n  Running {len(LOOKBACKS) * len(REBAL_FREQS) * len(N_SIZES)} param combos...")
    for lb, rebal, n in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
        res = run_hurst_factor(closes, lb, rebal, n, pre_computed=hurst_cache[lb])
    ...
    for fold in range(n_folds):
        ...
        for lb, rebal, nn in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
            if len(train_closes) < lb + 30:
                continue
            res = run_hurst_factor(train_closes, lb, rebal, nn)
        ...
        lb, rebal, nn = best_p
        test_res = run_hurst_factor(test_closes, lb, rebal, nn)
        wf_results.append({
...
def split_half_test(closes):
    ...
    r1, r2 = [], []
    for lb, rebal, n in product(LOOKBACKS, REBAL_FREQS, N_SIZES):
        res1 = run_hurst_factor(h1, lb, rebal, n)
        res2 = run_hurst_factor(h2, lb, rebal, n)
...
def correlation_with_h012(closes, best_lb=40, best_rebal=5, best_n=4):
    res = run_hurst_factor(closes, best_lb, best_rebal, best_n)
