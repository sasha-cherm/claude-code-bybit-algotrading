#!/usr/bin/env python3
"""
H-227: Relative Strength vs BTC Factor (14 Assets)

Idea: Instead of absolute momentum, measure each altcoin's return RELATIVE to BTC
over a lookback period. This captures "alpha" vs the dominant crypto asset.
Altcoins outperforming BTC may have idiosyncratic catalysts; underperformers
may be losing market share.

Signal:
  1. Compute rolling return for each asset over lookback window.
  2. Subtract BTC's return over the same window.
  3. Rank by relative strength: high RS -> long, low RS -> short.
  (BTC excluded from trading universe to avoid self-reference.)

Parameter grid:
  Lookback  : [10, 15, 20, 30, 40, 60]
  Rebal freq: [3, 5, 7]
  N:          [3, 4]
  Direction:  [high_rs_long, low_rs_long]
...
TRADE_ASSETS = [a for a in ASSETS_ALL if a != "BTC/USDT"]  # 13 alts

LOOKBACKS  = [10, 15, 20, 30, 40, 60]
REBALS     = [3, 5, 7]
NS         = [3, 4]
DIRECTIONS = ["high_rs_long", "low_rs_long"]
...
def compute_relative_strength(closes, lookback):
    """Compute rolling return of each alt relative to BTC."""
    btc_ret = closes["BTC/USDT"].pct_change(lookback)
    rs = {}
    for sym in TRADE_ASSETS:
        if sym not in closes.columns:
            continue
        alt_ret = closes[sym].pct_change(lookback)
        rs[sym] = alt_ret - btc_ret
...
def backtest_rs(closes, lookback, rebal_freq, n, direction="high_rs_long"):
    trade_cols = [c for c in TRADE_ASSETS if c in closes.columns]
    if len(trade_cols) < 2 * n:
        return None
    returns = closes[trade_cols].pct_change().dropna()
    rs = compute_relative_strength(closes, lookback).reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        oos_r = backtest_rs(oo_c, lb, rf, nn, d)
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_rs(h1c, lb, rf, nn, d))
                    e2 = evaluate(backtest_rs(h2c, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-227: Relative Strength vs BTC Factor")
    print("=" * 60)
    ...
    if "BTC/USDT" not in closes.columns:
        print("BTC/USDT not found! Cannot compute relative strength. REJECTED.")
        return
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_rs(closes, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-227", "name": "Relative Strength vs BTC",
    ...
    best_r = backtest_rs(closes, int(best_row["lb"]), int(best_row["rf"]),
