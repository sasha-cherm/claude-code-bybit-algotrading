...
Idea: Compute residual volatility after removing BTC market factor.
  For each non-BTC asset, regress daily returns on BTC returns to get alpha + beta*BTC + epsilon.
  Idiosyncratic vol = std(epsilon) over lookback window.
  In equities, low idio-vol outperforms (the "idiosyncratic volatility puzzle").
  Test if this holds in crypto cross-section.

Different from:
- H-019 (Low-Vol Anomaly): uses total volatility, not residual
- H-083 (prior idio-vol attempt): was rejected IS<80% with 1d data only, now using refined grid
- H-144 (another idio-vol): different lookback/rebal grid, also rejected
- H-076 (Efficiency): measures path quality, not residual volatility
- H-112 (Downside Beta): separates upside/downside beta but not residual vol

Key difference from total vol: BTC-hedged residual captures truly asset-specific risk.
Large-cap alts with low idio-vol are "cleaner" BTC proxies, while high-idio-vol
alts have unpredictable idiosyncratic moves.

Signal construction:
  1. For each non-BTC asset, regress returns on BTC returns over lookback
  2. Compute std(residuals) = idiosyncratic volatility
  3. Rank XS: test both directions

Parameter grid:
  Lookback: [15, 20, 30, 40, 60]
  Rebal freq: [3, 5, 7]
  N: [3, 4]
  Direction: [low_idiovol_long, high_idiovol_long]
...
REBALS = [3, 5, 7]
NS = [3, 4]
DIRECTIONS = ["low_idiovol_long", "high_idiovol_long"]
...
def compute_idiovol(closes, lookback):
    """Rolling idiosyncratic volatility: std of residuals from regression on BTC."""
    rets = closes.pct_change().dropna()
    btc_ret = rets["BTC/USDT"].values
    result = {}

    for asset in NON_BTC:
        if asset not in rets.columns:
            continue
        asset_ret = rets[asset].values
        vals = np.full(len(rets), np.nan)
        for i in range(lookback, len(rets)):
            wa = asset_ret[i - lookback:i]
            wb = btc_ret[i - lookback:i]
            X = np.column_stack([np.ones(lookback), wb])
            try:
                beta = np.linalg.lstsq(X, wa, rcond=None)[0]
                residuals = wa - X @ beta
                vals[i] = np.std(residuals)
            except Exception:
                pass
        result[asset] = vals

    idiovol = pd.DataFrame(result, index=rets.index)
    return idiovol
...
def backtest_factor(closes, signal_df, rebal_freq, n, direction):
    cols = [c for c in NON_BTC if c in signal_df.columns and c in closes.columns]
    if len(cols) < 2 * n:
        return None
    ...
    for i in range(1, len(dates)):
        ...
        if len(valid) < 2 * n:
            continue
        if direction == "low_idiovol_long":
            ranked = valid.sort_values(ascending=True)  # low idiovol first
        else:
            ranked = valid.sort_values(ascending=False)  # high idiovol first
...
def run_walk_forward(closes):
    ...
    for fold in range(WF_FOLDS):
        ...
        full_sig = compute_idiovol(closes.iloc[:oos_end], lb)
...
def run_split_half(closes):
    ...
    for lb in LOOKBACKS:
        h1_sig = compute_idiovol(h1c, lb)
        h2_sig = compute_idiovol(h2c, lb)
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
...
def main():
    print("=" * 60)
    print("  H-272: Idiosyncratic Volatility Factor")
    print("=" * 60)
    ...
    # Pre-compute signals
    print("\nPre-computing idiosyncratic vol signals...")
    sig_cache = {}
    for lb in LOOKBACKS:
        sig_cache[lb] = compute_idiovol(closes, lb)
        print(f"  LB={lb} done")
    ...
    rf = int(parts[1][1:])
    nn = int(parts[2][1:])
    d = "low_idiovol_long" if "low_idiovol_long" in best["label"] else "high_idiovol_long"
    sig = sig_cache[lb]
