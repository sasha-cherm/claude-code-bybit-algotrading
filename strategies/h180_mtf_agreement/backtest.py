...
Concept: For each asset, check if momentum signals agree across multiple
timeframes (5d, 10d, 20d, 40d, 60d). Count how many of these 5 lookbacks
show positive returns. Assets where ALL timeframes agree on uptrend (count=5)
get ranked highest. Assets where all agree on downtrend (count=0) get ranked
lowest.

This captures "trend breadth" — how broadly the trend is confirmed across
timescales. The key insight is this uses BINARY agreement, not magnitude.
An asset up 0.1% on all 5 lookbacks scores higher than one up 50% on 4
lookbacks but down on 1.

Logic:
  For each asset on each rebalance day:
    1. Compute returns over 5 lookback windows: [5, 10, 20, 40, 60] days
    2. Count how many are positive -> agreement_score in {0, 1, 2, 3, 4, 5}
    3. Rank assets cross-sectionally by agreement_score
       (with tiebreaking by average return magnitude or sum of returns)
    4. Two directions:
       - agreement_long: Long top-N (strong multi-TF agreement UP),
                         Short bottom-N (strong agreement DOWN)
       - contrarian_long: opposite
    5. Dollar-neutral, equal weight within legs

Transaction costs: 0.06% per side (0.12% round trip) on each rebalance.

Parameter grid:
  Lookback windows    : FIXED at [5, 10, 20, 40, 60]
  Rebalance (R)       : [3, 5, 7]
  N (top/bottom)      : [3, 4]
  Direction           : [agreement_long, contrarian_long]
  Tiebreak            : [avg_return, sum_return]
  Total: 3 x 2 x 2 x 2 = 24 combos

Validation (4-stage):
  1. IS: >= 80% of combos in the better direction must have positive Sharpe
  2. Walk-Forward: 6 folds, best IS params per fold, >= 4/6 positive OOS
...
# Fixed lookback windows for multi-timeframe agreement
LOOKBACK_WINDOWS = [5, 10, 20, 40, 60]
MAX_LOOKBACK = max(LOOKBACK_WINDOWS)  # 60

# Parameter grid
REBALS      = [3, 5, 7]
NS          = [3, 4]
DIRECTIONS  = ["agreement_long", "contrarian_long"]
TIEBREAKS   = ["avg_return", "sum_return"]
...
# Walk-forward config
WF_FOLDS     = 6
WF_TEST_DAYS = 120  # ~4 months per fold (needs to be > warmup+30 for OOS eval)
WF_TRAIN_MIN = 150  # minimum IS training days
...
def backtest_mtf_agreement(closes, rebal_freq, n, direction, tiebreak):
    """
    Multi-Timeframe Momentum Agreement Factor backtest.

    Args:
        closes: DataFrame of daily closes for all assets
        rebal_freq: rebalance every N days
        n: number of longs and shorts
        direction: "agreement_long" or "contrarian_long"
        tiebreak: "avg_return" or "sum_return"
    Returns:
    ...
    dates = returns.index
    warmup = MAX_LOOKBACK + 5  # need enough data for the longest lookback
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute agreement score for each asset
            scores = {}
            tiebreak_vals = {}
            ...
            for sym in trade_cols:
                ...
                # Count how many lookback windows show positive returns
                agreement = 0
                ret_values = []
                valid = True
                ...
                for lb in LOOKBACK_WINDOWS:
                    if i < lb:
                        valid = False
                        break
                    # Return over this lookback
                    cur_price = closes[sym].iloc[i]
                    past_price = closes[sym].iloc[i - lb]
                    if past_price <= 0 or np.isnan(past_price) or np.isnan(cur_price):
                        valid = False
                        break
                ...
                if not valid or len(ret_values) != len(LOOKBACK_WINDOWS):
                    continue

                scores[sym] = agreement
                if tiebreak == "avg_return":
                    tiebreak_vals[sym] = np.mean(ret_values)
                else:  # sum_return
                    tiebreak_vals[sym] = np.sum(ret_values)

            if len(scores) < 2 * n:
                continue

            # Rank: primary by agreement score, secondary by tiebreak value
            # Create a combined ranking key: (agreement_score, tiebreak_val)
            ranking = pd.DataFrame({
                "score": pd.Series(scores),
                "tiebreak": pd.Series(tiebreak_vals),
            })
            # Sort descending by score, then descending by tiebreak
            ranking = ranking.sort_values(
                ["score", "tiebreak"], ascending=[False, False]
            )

            if direction == "agreement_long":
                # Long top-N (highest agreement), Short bottom-N (lowest agreement)
                longs = ranking.index[:n]
                shorts = ranking.index[-n:]
            else:
                # Contrarian: Long bottom-N (lowest agreement), Short top-N (highest)
                longs = ranking.index[-n:]
                shorts = ranking.index[:n]
...
def run_walk_forward(closes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for rf in REBALS:
            for nn in NS:
                for tb in TIEBREAKS:
                    r = backtest_mtf_agreement(train_closes, rf, nn, best_direction, tb)
                    ev = evaluate(r)
                    if ev and ev["sharpe"] > best_sharpe:
                        best_sharpe = ev["sharpe"]
                        best_params = (rf, nn, tb)
        ...
        rf, nn, tb = best_params
        # OOS evaluation
        oos_r = backtest_mtf_agreement(oos_closes, rf, nn, best_direction, tb)
...
def run_split_half(closes, best_direction):
    ...
    for rf in REBALS:
        for nn in NS:
            for tb in TIEBREAKS:
                r1 = backtest_mtf_agreement(h1, rf, nn, best_direction, tb)
                r2 = backtest_mtf_agreement(h2, rf, nn, best_direction, tb)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-180: Multi-Timeframe Momentum Agreement Factor")
    print("=" * 60)
    ...
    print(f"Lookback windows: {LOOKBACK_WINDOWS} (FIXED)")
    ...
    all_results = []
    count = 0
    for rf, nn, d, tb in valid_grid:
        count += 1
        r = backtest_mtf_agreement(closes, rf, nn, d, tb)
        ev = evaluate(r, f"R{rf}_N{nn}_{d}_{tb}")
        if ev:
            ev["rf"] = rf
            ev["n"] = nn
            ev["direction"] = d
    ...
    for _, row in df_best.head(10).iterrows():
        print(f"    R{int(row['rf'])}_N{int(row['n'])}_{row['tiebreak']}: "
              f"Sharpe {row['sharpe']:.3f}, Ann {row['annual']:.1f}%, DD {row['dd']:.1f}%")

    # Bottom 5
    print(f"\n  Bottom combos ({best_dir}):")
    ...
    best_tb = best_row["tiebreak"]

    print(f"\n  Best combo: R{best_rf}_N{best_n}_{best_tb} ({best_dir})")
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-180",
            "name": "Multi-Timeframe Momentum Agreement Factor",
            "status": "REJECTED",
            ...
            "best_params": {"rebal": best_rf, "n": best_n, "direction": best_dir, "tiebreak": best_tb},
    ...
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with Existing Factors ---")
    best_rets = backtest_mtf_agreement(closes, best_rf, best_n, best_dir, best_tb)
