"""
H-085: Turnover Velocity Factor (Cross-Sectional)

Assets experiencing increasing trading activity (volume relative to historical
average) may signal growing institutional interest.
- Compute dollar volume = close * volume for each asset/day
- Short-term avg dollar volume (5-day) / long-term avg dollar volume (lookback)
- This turnover ratio captures whether recent trading activity is above/below norm
- Long top N (highest turnover surge), short bottom N (lowest turnover)
- Dollar-neutral: equal $ on long side and short side

Validation: walk-forward (6 folds, 90d each), split-half, 70/30 train/test,
...
# Parameter grid
LOOKBACKS = [20, 30, 40, 60]       # long-term volume lookback
REBAL_FREQS = [3, 5, 7, 10]       # rebalance every N days
N_LONGS = [3, 4, 5]               # top/bottom N

SHORT_VOL_WINDOW = 5               # short-term volume window (fixed)
...
def turnover_ratio(closes, volumes, long_lookback):
    """
    Compute turnover ratio for each asset/day.
    turnover_ratio = 5-day mean dollar volume / long_lookback-day mean dollar volume
    Higher value = recent trading activity surging relative to history.
    """
    dollar_vol = closes * volumes
    short_avg = dollar_vol.rolling(SHORT_VOL_WINDOW, min_periods=SHORT_VOL_WINDOW).mean()
    long_avg = dollar_vol.rolling(long_lookback, min_periods=long_lookback).mean()
...
def run_full_scan(closes, volumes):
    ...
    print("\n" + "=" * 70)
    print("H-085: TURNOVER VELOCITY FACTOR -- Full Parameter Scan")
    print("=" * 70)
    ...
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking = turnover_ratio(closes, volumes, lookback)
        warmup = lookback + 5  # need at least lookback days for long-term avg
...
def run_train_test_split(closes, volumes, split_ratio=0.7):
    ...
    train_results = []
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking = turnover_ratio(train_closes, train_volumes, lookback)
        warmup = lookback + 5
    ...
    test_ranking = turnover_ratio(test_closes, test_volumes, lookback)
    warmup = lookback + 5
...
def run_split_half(closes, volumes):
    ...
    results_h2 = []
    for lookback, rebal, n_long in product(LOOKBACKS, REBAL_FREQS, N_LONGS):
        ranking1 = turnover_ratio(half1_closes, half1_volumes, lookback)
        warmup = lookback + 5
        res1 = run_xs_factor(half1_closes, ranking1, rebal, n_long, warmup=warmup)

        ranking2 = turnover_ratio(half2_closes, half2_volumes, lookback)
...
def compute_h012_correlation(closes, volumes, lookback, rebal, n_long):
    """Compute daily return correlation between turnover factor and H-012 momentum."""
    print(f"\n  Correlation with H-012 (60d Momentum)")

    # Run turnover factor
    tv_ranking = turnover_ratio(closes, volumes, lookback)
    warmup_tv = lookback + 5
    res_tv = run_xs_factor(closes, tv_ranking, rebal, n_long, warmup=warmup_tv)
    ...
    rets_tv = eq_tv.pct_change().dropna()
    rets_mom = eq_mom.pct_change().dropna()

    common = rets_tv.index.intersection(rets_mom.index)
    if len(common) < 50:
        print("    Insufficient overlap for correlation")
        return 0.0

    corr = rets_tv.loc[common].corr(rets_mom.loc[common])
    print(f"    Daily return correlation: {corr:.3f}")
    print(f"    H-012 Momentum:  Sharpe {res_mom['sharpe']:.3f}, Ann {res_mom['annual_ret']:.1%}")
    print(f"    H-085 Turnover:  Sharpe {res_tv['sharpe']:.3f}, Ann {res_tv['annual_ret']:.1%}")
    return round(corr, 3)
...
if __name__ == "__main__":
    print("H-085: Turnover Velocity Factor")
    print("=" * 70)
    ...
    # ===== 9. Summary =====
    print("\n" + "=" * 70)
    print("SUMMARY: H-085 Turnover Velocity Factor")
    print("=" * 70)
    ...
    results_data = {
        "hypothesis": "H-085",
        "name": "Turnover Velocity Factor",
        "description": "Cross-sectional factor: long high turnover-surge assets, short low turnover assets",
