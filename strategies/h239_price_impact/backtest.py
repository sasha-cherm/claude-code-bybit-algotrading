#!/usr/bin/env python3
"""
H-239: Price Impact Factor (Return-to-Volume Ratio, 14 Assets)

Idea: Measure the ratio of absolute return to dollar volume (Amihud-like but
using dollar volume instead of raw volume). This captures "price impact" or
market depth — assets where large volume moves price less are deeper/more
resilient and may attract institutional flow.

Distinction from H-197 (Amihud): H-197 uses raw Amihud illiquidity (|ret|/volume).
This uses |ret| / dollar_volume which normalizes across different price levels.
Also tests the inverse: high-impact assets may have momentum amplification.

Signal:
  1. Compute rolling average of |daily_return| / dollar_volume over lookback.
  2. Rank cross-sectionally: low impact -> long (deep market), high -> short.
  3. Also test reverse direction.

Parameter grid:
  Lookback  : [10, 15, 20, 30, 40, 60]
  Rebal freq: [3, 5, 7]
  N:          [3, 4]
  Direction:  [low_impact_long, high_impact_long]
...
REBALS     = [3, 5, 7]
NS         = [3, 4]
DIRECTIONS = ["low_impact_long", "high_impact_long"]
...
def compute_price_impact(closes, volumes, lookback):
    """Compute rolling average of |return| / dollar_volume for each asset."""
    returns = closes.pct_change()
    dollar_vol = closes * volumes  # dollar volume
    abs_ret = returns.abs()
    # price impact = |ret| / dollar_vol (small number, normalize later via ranking)
    impact_raw = abs_ret / dollar_vol
    # Replace inf/nan with nan
    impact_raw = impact_raw.replace([np.inf, -np.inf], np.nan)
    # Rolling mean
    impact = impact_raw.rolling(lookback, min_periods=max(lookback // 2, 5)).mean()
    return impact
...
def backtest_factor(closes, volumes, lookback, rebal_freq, n, direction="low_impact_long"):
    cols = list(closes.columns)
    if len(cols) < 2 * n:
        return None
    returns = closes.pct_change().dropna()
    signal = compute_price_impact(closes, volumes, lookback).reindex(returns.index)
    dates = returns.index
    warmup = lookback + 5
    ...
    for i in range(warmup, len(dates)):
        ...
        if len(valid) < 2 * n:
            continue
        if direction == "low_impact_long":
            ranked = valid.sort_values(ascending=True)  # low impact first
        else:
            ranked = valid.sort_values(ascending=False)  # high impact first
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    for d in DIRECTIONS:
                        r = backtest_factor(tr_c, tr_v, lb, rf, nn, d)
                        ev = evaluate(r)
        ...
        oos_r = backtest_factor(oo_c, oo_v, lb, rf, nn, d)
...
def run_split_half(closes, volumes):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    e1 = evaluate(backtest_factor(h1c, h1v, lb, rf, nn, d))
                    e2 = evaluate(backtest_factor(h2c, h2v, lb, rf, nn, d))
                    if e1:
...
def main():
    print("=" * 60)
    print("  H-239: Price Impact Factor (Return-to-Dollar-Volume)")
    print("=" * 60)
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                for d in DIRECTIONS:
                    count += 1
                    r = backtest_factor(closes, volumes, lb, rf, nn, d)
    ...
    if not is_pass and not dom_dir_pass:
        ...
        result = {
            "hypothesis": "H-239", "name": "Price Impact Factor",
    ...
    best_r = backtest_factor(closes, volumes, int(best_row["lb"]), int(best_row["rf"]),
    ...
    print(f"  Note: Related to H-197 (Amihud). Check deployment-time correlation if confirmed.")
