#!/usr/bin/env python3
"""
H-203: Kurtosis / Excess Tail Risk Factor (Cross-Sectional)

Concept: Rolling kurtosis of daily returns measures tail risk / fat-tail behavior.
  - Low kurtosis => well-behaved, thin tails, more predictable
  - High kurtosis => fat tails, crash-prone, more extreme moves

Two directions tested:
  - low_kurt_long:  Long thin-tail assets, Short fat-tail assets
  - high_kurt_long: Long fat-tail assets, Short thin-tail assets

Logic:
  1. Compute rolling kurtosis of daily returns over lookback window
  2. Rank cross-sectionally each day
  3. Long top N, Short bottom N (dollar-neutral, equal weight)
  4. Rebalance every rebal_freq days
...
Validation (4-stage):
  ...
  4. Correlation: corr with H-012 cross-sectional momentum factor
"""
...
def load_daily_data():
    ...
    # Keep rows with at least half the assets present
    min_assets = max(2, len(closes.columns) // 2)
    closes = closes.dropna(thresh=min_assets)
    return closes
...
def backtest_kurtosis(closes, lookback, rebal_freq, n, direction):
    """
    Kurtosis Factor backtest.

    Args:
        closes:     DataFrame of daily closes (columns = assets)
        lookback:   rolling window for kurtosis calculation
        rebal_freq: rebalance every N days
        n:          number of longs and shorts
        direction:  "low_kurt_long" or "high_kurt_long"
    Returns:
    ...
    for i in range(warmup, len(dates)):
        ...
        if i - last_rebal >= rebal_freq:
            # Compute rolling kurtosis for each asset
            kurt_scores = {}
            for sym in trade_cols:
                ...
                if len(valid) < max(8, lookback // 3):
                    continue
                # scipy-style excess kurtosis (Fisher): normal = 0
                # Using pandas kurtosis which is excess kurtosis (Fisher definition)
                try:
                    k = pd.Series(valid).kurt()
                    if np.isfinite(k):
                        kurt_scores[sym] = k
                except Exception:
                    continue
            ...
            # ranked ascending: lowest kurtosis (thin tails) first
            ranked = pd.Series(kurt_scores).sort_values(ascending=True)

            if direction == "low_kurt_long":
                # Long low kurtosis (thin tails), Short high kurtosis (fat tails)
                longs = ranked.index[:n]
                shorts = ranked.index[-n:]
            else:
                # Long high kurtosis (fat tails), Short low kurtosis (thin tails)
                longs = ranked.index[-n:]
                shorts = ranked.index[:n]
...
# -- Reference strategy returns (H-012 momentum for correlation) -----------
...
def run_walk_forward(closes, best_direction):
    ...
    for fold in range(WF_FOLDS):
        ...
        for lb in LOOKBACKS:
            for rf in REBALS:
                for nn in NS:
                    r = backtest_kurtosis(train_closes, lb, rf, nn, best_direction)
                    ev = evaluate(r)
        ...
        # OOS evaluation
        oos_r = backtest_kurtosis(oos_closes, lb, rf, nn, best_direction)
...
def run_split_half(closes, best_direction):
    ...
    for lb in LOOKBACKS:
        for rf in REBALS:
            for nn in NS:
                r1 = backtest_kurtosis(h1_closes, lb, rf, nn, best_direction)
                r2 = backtest_kurtosis(h2_closes, lb, rf, nn, best_direction)
                e1 = evaluate(r1)
                e2 = evaluate(r2)
                if e1:
...
def main():
    print("=" * 60)
    print("  H-203: Kurtosis / Excess Tail Risk Factor")
    print("=" * 60)
    ...
    # Select the better direction (higher positive rate; tie-break by mean sharpe)
    best_dir = max(dir_stats, key=lambda d: (dir_stats[d]["pct_positive"], dir_stats[d]["mean_sharpe"]))
    ...
    if not is_pass:
        ...
        result_json = {
            "hypothesis": "H-203",
            "name": "Kurtosis / Excess Tail Risk Factor",
            "status": "REJECTED",
    ...
    # ==================================================================
    # Stage 4: Correlation with H-012 Momentum Factor
    # ==================================================================
    print(f"\n--- Stage 4: Correlation with H-012 Momentum ---")
    best_rets = backtest_kurtosis(closes, best_lb, best_rf, best_n, best_dir)
